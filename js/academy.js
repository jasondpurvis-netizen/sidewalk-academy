...
function vAcat(v){
  ...
  const _tcard=t=>{ const done=trackDone(t.id); const ls=trackLessons(t.id); const dn=ls.filter(l=>isDone(l.id)).length; const pct=trackPct(t.id);
    return `<div class="card" style="padding:0;overflow:hidden;cursor:pointer" onclick="go('track',{tid:'${t.id}'})">${trackBanner(t,pct,done)}<div style="padding:15px 17px 17px"><div style="font-weight:600;font-size:15px;margin:0 0 3px">${esc(t.name)}</div><div class="muted" style="font-size:12.5px;margin-bottom:11px">${done?esc(t.cert):dn+' of '+ls.length+' modules'}</div><div class="bar"><i style="width:${pct}%"></i></div></div></div>`; };
  let h=`<div class="crumb" onclick="go('home')">← Academy</div>`;
  h+=`<div style="position:relative;margin:0 0 16px"><i class="ti ti-search" style="position:absolute;left:13px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:16px;pointer-events:none"></i><input id="acatSearch" type="search" autocomplete="off" oninput="acatFilter()" placeholder="Search ${esc(label)} — a station, a step, a word…" style="width:100%;padding:12px 14px 12px 38px;border:1px solid var(--line2);border-radius:11px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14.5px"/></div>`;
  ...
  if(list.length){ h+=`<div class="sec">Badges</div><div class="card" style="padding:22px 18px"><div class="row" style="flex-wrap:wrap;gap:24px;justify-content:flex-start;align-items:flex-start">`+
    list.map((t,i)=>{ const done=trackDone(t.id); const bt=BADGE_THEMES[i%BADGE_THEMES.length];
      return `<div style="text-align:center;width:100px"><div style="width:70px;height:70px;margin:0 auto;position:relative;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:31px;${done?`color:#fff;background:${bt.g};box-shadow:0 8px 22px rgba(${bt.s},.55),inset 0 2px 6px rgba(255,255,255,.45);border:3px solid #fff`:'color:var(--faint);background:var(--bg);border:2px dashed var(--line2)'}"><i class="ti ${done?bt.ic:'ti-lock'}"></i>${done?`<span style="position:absolute;top:-3px;right:-3px;width:23px;height:23px;border-radius:50%;background:linear-gradient(135deg,#FFE07A,#F2A104);color:#7a4d00;display:flex;align-items:center;justify-content:center;font-size:12px;box-shadow:0 2px 7px rgba(242,161,4,.6);border:2px solid #fff"><i class="ti ti-star-filled"></i></span>`:''}</div><div style="font-size:11.5px;margin-top:9px;line-height:1.3;font-weight:${done?'600':'400'};color:${done?'var(--ink)':'var(--faint)'}">${esc(t.cert)}</div></div>`;
    }).join("")+`</div></div>`; }
  h+=`</div>`;
  v.innerHTML=h;
...
window.acatFilter=function(){ const el=document.getElementById('acatSearch'); const main=document.getElementById('acatMain'); const res=document.getElementById('acatResults'); if(!el||!res)return; const q=(el.value||'').trim().toLowerCase(); if(!q){ res.innerHTML=''; if(main)main.style.display=''; return; } if(main)main.style.display='none'; const idx=window._acatIdx||[]; const terms=q.split(/\s+/).filter(Boolean); const hits=idx.filter(function(r){ return terms.every(function(tm){ return window.fuzzyHas(r.txt,tm); }); }).slice(0,40);
  if(!hits.length){ res.innerHTML='<div class="card" style="padding:22px 18px;text-align:center"><div class="muted" style="font-size:13.5px">No modules match &ldquo;'+esc(q)+'&rdquo;. Try a station name or a single word.</div></div>'; return; }
  res.innerHTML='<div class="faint" style="font-size:12px;margin:0 0 8px">'+hits.length+' match'+(hits.length===1?'':'es')+'</div><div class="card" style="padding:4px 0">'+hits.map(function(r){ const done=isDone(r.lid); return '<div class="lesson-row" onclick="go(\'lesson\',{tid:\''+r.tid+'\',lid:\''+r.lid+'\'})"><div class="dot '+(done?'done':'')+'">'+(done?'✓':'')+'</div><div style="flex:1"><div style="font-weight:500">'+esc(r.title)+'</div><div class="faint" style="font-size:12px">'+esc(r.track)+' &middot; Module '+r.n+'</div></div><span class="faint">&rsaquo;</span></div>'; }).join('')+'</div>'; };
function vTrack(v){
  ...
  h+=trackLessons(t.id).map(l=>{ const d=isDone(l.id);
    return `<div class="lesson-row" onclick="go('lesson',{tid:'${t.id}',lid:'${l.id}'})"><div class="dot ${d?'done':''}">${d?'✓':''}</div><div style="flex:1"><div style="font-weight:500">${esc(l.title)}</div><div class="faint" style="font-size:12px">Module ${l.n}</div></div><span class="faint">›</span></div>`;
  }).join("")+`</div>`;
  v.innerHTML=h;
}
function vLesson(v){
  ...
  else main+=`<div class="card" style="padding:24px 22px;text-align:center"><div style="font-size:24px;margin-bottom:8px">📖</div><div style="font-weight:600;margin-bottom:6px">Full module text coming in</div><div class="muted" style="font-size:13px;max-width:430px;margin:0 auto;line-height:1.6">This module's written lesson is being loaded into the academy.</div></div>`;
  if((t.category||'development')!=='operations') main+=`<div class="card" style="padding:18px 20px;margin-top:24px"><div style="font-weight:600;margin-bottom:4px">📓 Your reflection</div><div class="muted" style="font-size:13px;margin-bottom:10px">Answer this module's questions here. It saves to your journal for your next conversation.</div><textarea id="jr" onblur="saveJournal('${l.id}')" style="width:100%;min-height:130px;padding:12px 13px;border:1px solid var(--line2);border-radius:9px;font-size:14px;font-family:inherit;line-height:1.65;color:var(--ink);background:var(--card)" placeholder="Write your thoughts...">${esc(state.responses[l.id]||'')}</textarea><div class="row" style="margin-top:9px"><button class="btn pri" style="width:auto" onclick="saveJournal('${l.id}')">Save reflection</button><span class="muted" id="jrmsg" style="font-size:12px"></span></div></div>`;
  ...
  const rail = dls.length ? `<div class="card" style="padding:14px 15px;background:var(--brand-soft);border-color:var(--brand-line)"><div style="font-size:11px;font-weight:600;color:var(--brand);text-transform:uppercase;letter-spacing:.06em;margin-bottom:11px">Downloads</div>`+dls.map(d=>`<a href="${d.url}" target="_blank" style="display:flex;align-items:flex-start;gap:9px;text-decoration:none;margin-bottom:12px"><div style="color:var(--brand);font-size:18px;flex-shrink:0">📄</div><div style="font-size:13px;color:var(--ink);line-height:1.4">${esc(d.name)}<div style="color:var(--brand);font-size:12px;margin-top:3px;font-weight:500">Download ↓</div></div></a>`).join('')+`</div>` : '';
...
async function vSummary(v){
  ...
  const stat=(n,l,c)=>`<div style="flex:1;text-align:center;padding:14px 8px"><div style="font-size:24px;font-weight:700;color:${c}">${n}</div><div class="faint" style="font-size:12px">${l}</div></div>`;
  ...
  h+=`<div class="card" style="padding:12px 15px;margin-bottom:16px"><b>This week</b> <span class="muted" style="font-size:13px">— ${weekDone} lesson${weekDone===1?'':'s'} completed across the team.${nStall?'':' Nice momentum.'}</span></div>`;
  if(nStall) h+=`<div class="card" style="padding:12px 15px;margin-bottom:16px;background:var(--amber-soft);border-color:var(--amber)"><b style="color:var(--amber)">⚠ ${nStall} ${nStall===1?'person has':'people have'} stalled</b> <span class="muted" style="font-size:13px">— no progress in ${STALL_DAYS}+ days. A quick check-in goes a long way.</span></div>`;
  ...
  h2+=learners.map(p=>`<div class="lesson-row" style="cursor:default"><div style="width:32px;height:32px;border-radius:50%;background:var(--brand-soft);color:var(--brand);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700">${esc((p.name||'?').charAt(0).toUpperCase())}</div><div style="flex:1;min-width:0"><div style="font-weight:500">${esc(p.name)}</div><div class="faint" style="font-size:12px">${esc(p.title||'Team member')}</div></div><div style="margin-right:14px">${pill(p)}</div><div style="width:130px;text-align:right"><div class="muted" style="font-size:12px;margin-bottom:5px">${p.dn} of ${total}</div><div class="bar"><i style="width:${p.pct}%"></i></div></div></div>`).join("")||`<div style="padding:30px;text-align:center" class="faint">No team members yet. Share your join code from Settings to get them in.</div>`;
  h2+=`</div>`;
  v.innerHTML=h+h2;
}
...
async function vCommunity(v){
  ...
  let h=`<div style="display:flex;gap:8px;overflow-x:auto;margin-bottom:18px;padding-bottom:5px;scrollbar-width:thin;white-space:nowrap">`+visible.map(c=>{ const on=ch===c.id; const u=unreadOf(c.id); const bdg=(u&&!on)?`<span style="background:var(--accent);color:#3A2B00;border-radius:999px;font-size:10px;font-weight:800;padding:1px 6px;margin-left:5px">${u>9?'9+':u}</span>`:''; const ic=c.icon||'ti-hash'; return `<button onclick="go('community',{ch:'${c.id}'})" style="display:inline-flex;align-items:center;gap:7px;padding:9px 16px;flex-shrink:0;border-radius:999px;font-weight:700;font-size:13px;cursor:pointer;font-family:inherit;border:1px solid ${on?'transparent':'var(--line2)'};${on?'background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff;box-shadow:0 6px 15px rgba(46,125,138,.32)':'background:var(--card);color:var(--ink)'}"><i class="ti ${ic}" style="font-size:15px"></i>${esc(c.label)}${bdg}</button>`; }).join('')+(isAdmin?`<button onclick="manageChannels()" title="Add, rename, reorder or hide channels" style="display:inline-flex;align-items:center;gap:6px;padding:9px 14px;flex-shrink:0;border-radius:999px;font-weight:700;font-size:13px;cursor:pointer;font-family:inherit;border:1px dashed var(--line2);background:var(--card);color:var(--muted)"><i class="ti ti-settings" style="font-size:15px"></i>Manage</button><button onclick="importChats()" title="Import an old group's chat history from a message.json export" style="display:inline-flex;align-items:center;gap:6px;padding:9px 14px;flex-shrink:0;border-radius:999px;font-weight:700;font-size:13px;cursor:pointer;font-family:inherit;border:1px dashed var(--line2);background:var(--card);color:var(--muted)"><i class="ti ti-upload" style="font-size:15px"></i>Import</button>`:'')+`</div>`;
  ...
  if(q){
    ...
    h+=`<div class="faint" style="font-size:13px;margin-bottom:12px">${res.length} result${res.length===1?'':'s'} for "${esc(q)}" · <span style="color:var(--brand);cursor:pointer;font-weight:600" onclick="comSearch('')">clear</span></div>`;
    ...
    else h+=res.map(p=>`<div class="card" style="padding:14px 16px;margin-bottom:10px;cursor:pointer" onclick="go('community',{ch:'${p.channel}'})"><div class="row" style="gap:8px;margin-bottom:6px;flex-wrap:wrap"><span class="pill" style="background:var(--brand-soft);color:var(--brand);font-size:11px">${esc(chName(p.channel))}</span><span style="font-weight:600;font-size:13px">${esc(p.author_name||'Someone')}</span><span class="faint" style="font-size:12px">${timeAgo(p.created_at)}</span></div><div class="muted" style="font-size:13.5px;line-height:1.5;white-space:pre-wrap">${esc((p.body||'').slice(0,240))}${(p.body||'').length>240?'…':''}</div>${p.media_url?`<div class="faint" style="font-size:12px;margin-top:5px"><i class="ti ti-${p.media_type==='video'?'video':'photo'}"></i> ${p.media_type==='video'?'video':'photo'} attached</div>`:''}</div>`).join('');
    v.innerHTML=h; return;
  }
  if(mediaMode){
    const media=state.community.posts.filter(p=>p.channel===ch && p.media_url);
    h+=`<div class="faint" style="font-size:13px;margin-bottom:12px">${media.length} photo/video${media.length===1?'':'s'} shared in ${esc(cur.label)} — only people with access to this channel see these.</div>`;
  ...
  if(!posts||!posts.length) h+=`<div class="card" style="padding:34px;text-align:center"><div style="font-size:26px;margin-bottom:8px">💬</div><div class="muted" style="font-size:14px">Nothing here yet.${canPost?' Start the conversation.':''}</div></div>`;
    ...
    const cs=cBy[p.id]||[];
    return `<div class="card" style="padding:15px 17px;margin-bottom:12px"><div class="row" style="gap:10px;margin-bottom:9px"><div style="width:34px;height:34px;border-radius:50%;overflow:hidden;background:#EFE9E2;color:#7a6f63;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;flex-shrink:0">${(function(){var pr=(state.community.profiles||[]).find(function(x){return x.id===p.author_id;});return pr&&pr.avatar_url&&pr.name===p.author_name?'<img src="'+pr.avatar_url+'" style="width:100%;height:100%;object-fit:cover"/>':esc((p.author_name||'?').charAt(0).toUpperCase());})()}</div><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(p.author_name||'Someone')}</div><div class="faint" style="font-size:12px">${timeAgo(p.created_at)}${p.edited?' · edited':''}</div></div><button onclick="markUnreadFrom(${p.id})" class="no-print" title="Mark unread from here — flags it so you come back to it" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:15px;line-height:1;align-self:flex-start"><i class="ti ti-bookmark"></i></button>${(((state.profile||{}).name||'').trim().toLowerCase()===((p.author_name||'').trim().toLowerCase()))?`<button onclick="editPost(${p.id})" class="no-print" title="Edit" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:12.5px;align-self:flex-start">Edit</button>`:''}${(p.author_id===state.user.id||isAdmin)?`<button onclick="delPostC(${p.id})" class="no-print" title="Delete" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:16px;line-height:1;align-self:flex-start">×</button>`:''}</div>${p.recognized_name?`<div style="display:flex;align-items:center;gap:8px;background:#FFF7E0;border:1px solid #F0D98A;border-radius:9px;padding:8px 11px;margin-bottom:8px"><span style="font-size:16px">👏</span><span style="font-size:13px;font-weight:700;color:#8A5A00">Shout-out to ${esc(p.recognized_name)}</span></div>`:''}<div class="muted" style="line-height:1.55;white-space:pre-wrap">${linkifyTags(p.body)}</div>`+
      (cs.length?`<div style="margin-top:10px;border-top:1px solid var(--line)">`+cs.map(c=>`<div style="padding:9px 0 0"><span style="font-weight:600;font-size:13px">${esc(c.author_name||'Someone')}</span> <span class="muted" style="font-size:13px;white-space:pre-wrap">${esc(c.body)}</span>${(((state.profile||{}).name||'').trim().toLowerCase()===((c.author_name||'').trim().toLowerCase()))?` <button onclick="editComment(${c.id})" class="no-print" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:11px">edit</button>`:''}${(c.author_id===state.user.id||isAdmin)?`<button onclick="delComment(${c.id})" class="no-print" title="Delete" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:13px;line-height:1">×</button>`:''}</div>`).join('')+`</div>`:``)+
      `${p.media_url?(p.media_type==='video'?`<video src="${p.media_url}" controls playsinline style="width:100%;max-height:420px;margin-top:11px;border-radius:11px;background:#000"></video>`:`<img src="${p.media_url}" loading="lazy" style="width:100%;margin-top:11px;border-radius:11px;display:block"/>`):''}`+
      reactionBar(p)+
      ((twoWay||isAdmin)?`<div class="row" style="margin-top:11px;gap:8px"><input id="rc${p.id}" spellcheck="true" autocorrect="on" autocapitalize="sentences" placeholder="Reply…" style="flex:1;padding:8px 11px;border:1px solid var(--line2);border-radius:8px;font-size:13px;font-family:inherit;color:var(--ink);background:var(--card)" onkeydown="if(event.key==='Enter')commentNew(${p.id})"/><button class="btn" style="width:auto" onclick="commentNew(${p.id})">Reply</button></div>`:`<div class="faint" style="margin-top:10px;font-size:12px;font-style:italic;border-top:1px solid var(--line);padding-top:9px">📣 One-way channel — replies are off here.</div>`)+`</div>`;
  }).join('');
  v.innerHTML=h;
}
