...
async function vCalendar(v){
  ...
  h+=`<div class="card" style="padding:14px;margin-top:10px"><div style="font-weight:600;font-size:14px;margin-bottom:8px">Add to ${esc(selLbl)}</div><div class="row" style="gap:8px;flex-wrap:wrap"><select id="calkind" onchange="calKindChanged()" style="">`+Object.keys(CAL_KINDS).map(k=>`<option value="${k}">${CAL_KINDS[k][1]}</option>`).join('')+`</select><input id="caltitle" placeholder="What's happening?" onkeydown="if(event.key==='Enter')calAdd('${sel}')" style="flex:1;min-width:150px"/></div><div class="row" style="gap:8px;margin-top:8px"><input id="caldetail" placeholder="Details (optional)" onkeydown="if(event.key==='Enter')calAdd('${sel}')" style="flex:1"/><button class="btn pri" style="width:auto" onclick="calAdd('${sel}')">Add</button></div>
    <div id="calPrepWrap" class="row" style="gap:9px;margin-top:9px;align-items:center;flex-wrap:wrap"><span style="font-size:14px;color:var(--ink2)">Start getting ready</span><select id="calprep" style=""><option value="0">on the day</option><option value="1" selected>1 day before</option><option value="2">2 days before</option><option value="3">3 days before</option><option value="7">a week before</option></select><span class="faint" style="font-size:13px;flex:1;min-width:170px">It shows up on Today that morning, so a future order stops being a surprise on the day.</span></div><div class="faint" style="font-size:12.5px;margin-top:7px">A holiday auto-adds a prep &amp; staffing heads-up to the Lineup the day before and the day of.</div></div>`;
  v.innerHTML=h;
}
...
async function vCatering(v){
  ...
  h+=`<div class="card" style="padding:14px;margin-bottom:12px">
    ...
    <div class="row" style="gap:8px;flex-wrap:wrap">
      <input id="catWhat" placeholder="What they ordered — e.g. 60 turkey sandwiches" style="flex:2;min-width:190px"/>
      <input id="catWho" placeholder="Who it's for" style="flex:1;min-width:130px"/>
    </div>
    <div class="row" style="gap:8px;margin-top:8px;flex-wrap:wrap">
      <input id="catDate" type="date" value="${today}" style=""/>
      <input id="catTime" type="time" style=""/>
      <input id="catContact" placeholder="Phone or email (optional)" style="flex:1;min-width:150px"/>
    </div>
...
async function vSettings(v){
  ...
  const _sx=`<!--SEC:shop--><div class="card" style="padding:22px;max-width:480px">
    <div style="margin-bottom:18px"><label style="display:block;font-size:14px;font-weight:600;margin-bottom:6px">What your team calls the training programme</label><input id="setname" type="text" value="${esc(s.academy_name||'')}" style="width:100%"/></div>
