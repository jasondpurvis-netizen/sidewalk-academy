...
window.openRosterImport=function(){
  ...
  w.innerHTML='<div style="background:var(--card,#fff);color:var(--ink,#111);border-radius:12px;max-width:620px;width:100%;max-height:88vh;overflow:auto;padding:22px 24px;box-shadow:0 20px 60px rgba(0,0,0,.3)">'
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">'
    +'<div><div style="font-weight:800;font-size:18px">Bring your team over</div>'
    +'<div class="muted" style="font-size:14px;margin-top:3px">Export your staff list from your current scheduler and drop the file here. Nothing is typed by hand.</div></div>'
    +'<button onclick="var m=document.getElementById(\'impModal\');if(m)m.remove()" style="border:none;background:transparent;font-size:26px;cursor:pointer;line-height:1;color:inherit">&times;</button></div>'
    ...
    +'<div style="font-weight:700;font-size:14px">Choose your CSV file</div>'
...
window._impRead=function(file){
  ...
  fr.onload=function(){
    ...
    if(res.error){ body.innerHTML='<div style="color:#B32D2D;font-size:14px">'+esc(res.error)+'</div>'; return; }
    ...
    var h='<div style="background:var(--bg,#f4f7f8);border-radius:12px;padding:14px 16px">'
      +'<div style="font-weight:800;font-size:15.5px">'+fresh.filter(function(p){return !p.already;}).length+' people ready to import</div>'
    ...
    if(window._impStale && window._impStale.length){
      h+='<div style="margin-top:12px;background:var(--bg,#f4f7f8);border-left:3px solid #7A6224;border-radius:8px;padding:12px 14px">'
        +'<div style="font-weight:700;font-size:14px">'+window._impStale.length+' on your roster are not in this file</div>'
    ...
    if(stationList.length){
      h+='<div style="margin-top:14px"><div style="font-weight:700;font-size:14px;margin-bottom:6px">Stations found</div>'
        +'<div style="display:flex;flex-wrap:wrap;gap:6px">'
        + stationList.map(function(k){ return '<span style="background:var(--bg,#eef3f4);border-radius:999px;padding:4px 10px;font-size:12.5px">'+esc(k)+' &middot; '+stations[k]+'</span>'; }).join('')
        +'</div><div class="muted" style="font-size:12.5px;margin-top:6px">These become skills on each person, so the schedule builder already knows who can work where.</div></div>';
    }
    if(fresh.length){
      ...
      h+='<div style="margin-top:16px;display:flex;gap:9px;align-items:center;flex-wrap:wrap">'
        +'<input id="impSearch" placeholder="Search names\u2026" oninput="_impFilter()" style="flex:1;min-width:170px;padding:9px 12px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:transparent;color:inherit">'
      ...
      h+='<div id="impList" style="margin-top:10px;max-height:290px;overflow:auto;border:1px solid var(--line2,#e2e9ea);border-radius:12px">'
        + fresh.map(function(p,i){ return '<label data-improw="'+i+'" data-impname="'+esc(p.name.toLowerCase())+'" style="display:flex;gap:10px;align-items:center;padding:9px 12px;border-bottom:1px solid var(--line,#eef2f3);cursor:'+(p.already?'default':'pointer')+';opacity:'+(p.already?'.55':'1')+'">'
            ...
            +'<span style="flex:1;min-width:0;font-size:14px">'+esc(p.name)+'</span>'
            +(p.already?'<span class="muted" style="font-size:12.5px">already added</span>':'')
            +'<span class="muted" style="font-size:12.5px">'+esc(p.stations&&p.stations.length?(p.stations.length+' stations'):(p.station||p.rank))+'</span>'
            +'<span class="muted" style="font-size:12.5px;min-width:74px;text-align:right">'+esc(p.hire||'')+'</span></label>'; }).join('')
        +'</div>';
      h+='<div style="display:flex;gap:9px;margin-top:16px;align-items:center;flex-wrap:wrap">'
        +'<button class="btn" onclick="_impCommit()" id="impGo" disabled style="background:var(--brand,#4a9cad);color:#fff;border:none;border-radius:8px;padding:11px 18px;font-weight:700;cursor:pointer;opacity:.5">Import 0 people</button>'
        +'<button onclick="var m=document.getElementById(\'impModal\');if(m)m.remove()" style="background:transparent;border:1px solid var(--line2,#d5dde0);border-radius:8px;padding:11px 16px;cursor:pointer;color:inherit">Cancel</button>'
        +'<span id="impMsg" class="muted" style="font-size:12.5px"></span></div>';
    } else {
      h+='<div class="muted" style="margin-top:14px;font-size:14px">Everyone in this file is already on your roster. Nothing to import.</div>';
    }
    body.innerHTML=h;
  };
...
window.openConceptSetup=function(){
  ...
  w.style.cssText='position:fixed;inset:0;z-index:10060;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;padding:18px';
  w.innerHTML='<div id="cxCard" style="background:var(--card,#fff);color:var(--ink,#111);border-radius:12px;max-width:600px;width:100%;max-height:88vh;overflow:auto;padding:22px 24px;box-shadow:0 20px 60px rgba(0,0,0,.3)"></div>';
...
window._cxPick=function(){
  ...
  var h='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">'
    +'<div><div style="font-weight:800;font-size:18px">What kind of restaurant is this?</div>'
    +'<div class="muted" style="font-size:14px;margin-top:4px">Pick the closest one. It just fills in a starting set of stations — you can rename or remove any of them next.</div></div>'
    +'<button onclick="var m=document.getElementById(\'cxModal\');if(m)m.remove()" style="border:none;background:transparent;font-size:26px;cursor:pointer;line-height:1;color:inherit">&times;</button></div>';
  h+='<div style="display:grid;gap:9px;margin-top:16px">'
    + CONCEPTS.map(function(k,i){ return '<button onclick="_cxChoose('+i+')" style="text-align:left;background:transparent;border:1.5px solid var(--line2,#d5dde0);border-radius:12px;padding:13px 15px;cursor:pointer;color:inherit">'
        +'<div style="font-weight:700;font-size:15.5px">'+esc(k.name)+'</div>'
...
window._cxEdit=function(){
  ...
  var h='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">'
    +'<div><div style="font-weight:800;font-size:18px">Make these your own</div>'
    +'<div class="muted" style="font-size:14px;margin-top:4px">Use the words your team actually says on the floor. Remove anything you do not run.</div></div>'
    +'<button onclick="var m=document.getElementById(\'cxModal\');if(m)m.remove()" style="border:none;background:transparent;font-size:26px;cursor:pointer;line-height:1;color:inherit">&times;</button></div>';
  h+='<div style="margin-top:16px;display:flex;flex-direction:column;gap:7px">'
    + k.stations.map(function(st,j){ return '<div style="display:flex;gap:8px;align-items:center">'
        +'<input value="'+esc(st)+'" oninput="_cxSet('+j+',this.value)" style="flex:1;padding:9px 12px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:transparent;color:inherit">'
        +'<button onclick="_cxDel('+j+')" title="Remove" style="border:none;background:transparent;font-size:18px;cursor:pointer;color:var(--muted,#6b8087);line-height:1">&times;</button></div>'; }).join('')
    +'</div>';
  h+='<button onclick="_cxAdd()" style="margin-top:10px;background:transparent;border:1px dashed var(--line2,#d5dde0);border-radius:8px;padding:9px 14px;font-size:14px;cursor:pointer;color:inherit">+ Add a station</button>';
  h+='<div style="display:flex;gap:9px;margin-top:18px;align-items:center">'
    +'<button onclick="_cxSave()" id="cxGo" style="background:var(--brand,#4a9cad);color:#fff;border:none;border-radius:8px;padding:11px 20px;font-weight:700;cursor:pointer">Save '+k.stations.length+' stations</button>'
    +'<button onclick="_cxPick()" style="background:transparent;border:1px solid var(--line2,#d5dde0);border-radius:8px;padding:11px 16px;cursor:pointer;color:inherit">Back</button>'
...
window._payRender = function(){
  ...
  let h='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">'
    +'<div>'+(inline?'':'<div style="font-weight:800;font-size:18px">Pay rates</div>')
    +'<div class="muted" style="font-size:14px;margin-top:3px">Everyone in one place — hourly or salaried. Use the button on the right of a row to switch someone between the two.</div></div>'
    + (inline?'':'<button onclick="var m=document.getElementById(\'payModal\');if(m)m.remove()" style="border:none;background:transparent;font-size:26px;cursor:pointer;line-height:1;color:inherit">&times;</button>')
    +'</div>';
  h+='<div style="display:flex;gap:8px;align-items:center;margin-top:14px;flex-wrap:wrap">'
    +'<input id="payAllVal" type="number" step="0.01" placeholder="15.15" style="width:110px;padding:8px 10px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:transparent;color:inherit">'
  ...
  people.forEach(function(n){
    ...
    h+='<div style="display:flex;gap:10px;align-items:center;border:1px solid '+(low?'#E4B8A8':'var(--line2,#d5dde0)')+';background:'+(low?'#F9EDE8':'transparent')+';border-radius:12px;padding:9px 12px">'
      ...
      +'<span style="flex:1;min-width:0;font-size:14px;font-weight:600">'+esc(n)+'</span>'
...
window.openContactEditor = async function(){
  ...
  w.innerHTML='<div id="ctCard" style="background:var(--card,#fff);color:var(--ink,#111);border-radius:12px;max-width:620px;width:100%;max-height:90vh;overflow:auto;padding:22px 24px;box-shadow:0 20px 60px rgba(0,0,0,.3)"></div>';
...
window._ctRender = function(){
  ...
  let h='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">'
    +'<div><div style="font-weight:800;font-size:18px">Contact details</div>'
    +'<div class="muted" style="font-size:14px;margin-top:3px">How your team hears about a new schedule. Without an email address, they do not.</div></div>'
    +'<button onclick="var m=document.getElementById(\'ctModal\');if(m)m.remove()" style="border:none;background:transparent;font-size:26px;cursor:pointer;line-height:1;color:inherit">&times;</button></div>';

  h+='<div style="display:flex;gap:18px;flex-wrap:wrap;margin-top:14px;padding:12px 14px;border:1px solid var(--line2,#d5dde0);border-radius:12px">'
    +'<div><div style="font-size:18px;font-weight:800;font-variant-numeric:tabular-nums">'+nEmail+' / '+people.length+'</div>'
      +'<div class="faint" style="font-size:12.5px">can be emailed</div></div>'
    +'<div><div style="font-size:18px;font-weight:800;font-variant-numeric:tabular-nums">'+nAcct+' / '+people.length+'</div>'
      +'<div class="faint" style="font-size:12.5px">have signed in</div></div>'
  ...
  people.forEach(function(n){
    ...
    const q=JSON.stringify(n).replace(/"/g,'&quot;');
    h+='<div style="border:1px solid '+bord+';background:'+bg+';border-radius:12px;padding:10px 12px">'
      +'<div style="display:flex;gap:9px;align-items:baseline;flex-wrap:wrap">'
        +'<span style="font-size:14px;font-weight:600">'+esc(n)+'</span>'
        +'<span class="muted" style="font-size:12.5px">'+esc(posOf(n))+'</span>'
        + (A.has(n) ? '<span style="font-size:11.5px;font-weight:700;color:#2C6E4B;border:1px solid #B9D6C4;border-radius:99px;padding:1px 7px">signed in</span>'
                    : '<span style="font-size:11.5px;font-weight:700;color:#7A5B1E;border:1px solid #E4CFA3;border-radius:99px;padding:1px 7px">no account yet</span>')
      +'</div>'
      +'<div style="display:flex;gap:8px;margin-top:7px;flex-wrap:wrap">'
        +'<input type="email" value="'+esc(v.email)+'" placeholder="email address" autocomplete="off"'
          ...
          +' style="flex:1;min-width:180px;padding:7px 9px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:var(--card,#fff);color:inherit">'
        +'<input type="tel" value="'+esc(v.mobile)+'" placeholder="mobile (optional)" autocomplete="off"'
          ...
          +' style="width:150px;padding:7px 9px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:var(--card,#fff);color:inherit">'
      +'</div>'
      + (badEmail? '<div style="font-size:12.5px;color:#A8401C;margin-top:5px">That does not look like an email address</div>':'')
      + (badPhone? '<div style="font-size:12.5px;color:#A8401C;margin-top:5px">A mobile number should be 10 digits</div>':'')
      +'</div>';
  });
  h+='</div>';
  h+='<div style="display:flex;gap:9px;margin-top:18px;align-items:center;flex-wrap:wrap">'
    +'<button onclick="_ctSave()" id="ctGo" style="background:var(--brand,#4a9cad);color:#fff;border:none;border-radius:8px;padding:11px 20px;font-weight:700;cursor:pointer">Save contacts</button>'
    +'<button onclick="var m=document.getElementById(\'ctModal\');if(m)m.remove()" style="background:transparent;border:1px solid var(--line2,#d5dde0);border-radius:8px;padding:11px 16px;cursor:pointer;color:inherit">Cancel</button>'
  ...
  h += `<div class="card" style="padding:18px 20px;margin-bottom:16px">
    <div style="display:flex;align-items:baseline;gap:12px;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:170px">
        <div style="font-weight:700;font-size:15.5px">${ready===100?'Auto-draft is ready':'Auto-draft is not ready yet'}</div>
...
window._asRender=function(){
  ...
  const cell=function(n,wd){
    ...
    return '<td style="padding:3px"><button onclick="_asOpen('+JSON.stringify(n).replace(/"/g,'&quot;')+','+wd+',event)" style="width:100%;border:none;border-radius:8px;padding:8px 4px;font-size:12.5px;font-weight:700;cursor:pointer;background:'+tone.bg+';color:'+tone.fg+'">'+esc(tone.t)+'</button></td>';
  };
  let h='<div class="muted" style="font-size:14px;margin:-4px 0 2px">Everyone can work all day unless you say otherwise. Tap any square to change it.</div>';
  h+='<div style="overflow-x:auto;margin-top:14px"><table style="width:100%;border-collapse:collapse;min-width:640px">'
    +'<thead><tr><th style="text-align:left;padding:6px 8px;font-size:12.5px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);position:sticky;left:0;background:var(--card,#fff)">Person</th>'
    + SHORT.map(d=>'<th style="padding:6px 4px;font-size:12.5px;font-weight:620;letter-spacing:.01em;text-transform:none;color:var(--muted)">'+d+'</th>').join('')
    +'<th style="padding:6px 8px;font-size:12.5px;font-weight:620;letter-spacing:.01em;text-transform:none;color:var(--muted);white-space:nowrap">Quick set</th></tr></thead><tbody>';
  people.forEach(function(n){
    const nm=JSON.stringify(n).replace(/"/g,'&quot;');
    h+='<tr style="border-top:1px solid var(--line,#eef2f3)">'
      +'<td style="padding:6px 8px;font-size:14px;font-weight:600;white-space:nowrap;position:sticky;left:0;background:var(--card,#fff)">'+esc(dispName(n))+'</td>'
      + [0,1,2,3,4,5,6].map(function(wd){ return cell(n,wd); }).join('')
      +'<td style="padding:3px 6px;white-space:nowrap">'
        +'<button onclick="_asPreset('+nm+',\'weekends\')" title="Weekends only" style="border:1px solid var(--line2,#d5dde0);background:transparent;color:inherit;border-radius:8px;padding:5px 7px;font-size:12.5px;cursor:pointer;margin-right:3px">Wknd</button>'
        +'<button onclick="_asPreset('+nm+',\'weekdays\')" title="Weekdays only" style="border:1px solid var(--line2,#d5dde0);background:transparent;color:inherit;border-radius:8px;padding:5px 7px;font-size:12.5px;cursor:pointer;margin-right:3px">Wkdy</button>'
        +'<button onclick="_asPreset('+nm+',\'all\')" title="Available every day" style="border:1px solid var(--line2,#d5dde0);background:transparent;color:inherit;border-radius:8px;padding:5px 7px;font-size:12.5px;cursor:pointer">All</button>'
      +'</td></tr>';
  });
  h+='</tbody></table></div>';
  ...
  h+='<div style="display:flex;gap:9px;margin-top:16px;align-items:center;flex-wrap:wrap">'
    +'<button onclick="_asFinish()" style="background:var(--brand,#4a9cad);color:#fff;border:none;border-radius:8px;padding:11px 20px;font-weight:700;cursor:pointer">Mark as reviewed</button>'
...
window._asOpen=function(name,wd,ev){
  ...
  pop.innerHTML='<div style="font-weight:700;font-size:14px;margin-bottom:8px">'+esc(dispName(name))+' &middot; '+['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'][wd]+'</div>'
    +'<div style="display:flex;gap:6px;flex-wrap:wrap">'
    +'<button onclick="_asSet('+nm+','+wd+',\'all\')" style="border:none;border-radius:8px;padding:7px 11px;font-size:12.5px;font-weight:600;cursor:pointer;background:'+(st.k==='all'?'#2C6E4B':'rgba(0,0,0,.06)')+';color:'+(st.k==='all'?'#fff':'inherit')+'">All day</button>'
    +'<button onclick="_asSet('+nm+','+wd+',\'part\')" style="border:none;border-radius:8px;padding:7px 11px;font-size:12.5px;font-weight:600;cursor:pointer;background:'+(st.k==='part'?'#7A5B1E':'rgba(0,0,0,.06)')+';color:'+(st.k==='part'?'#fff':'inherit')+'">Some hours</button>'
    +'<button onclick="_asSet('+nm+','+wd+',\'off\')" style="border:none;border-radius:8px;padding:7px 11px;font-size:12.5px;font-weight:600;cursor:pointer;background:'+(st.k==='off'?'#A8401C':'rgba(0,0,0,.06)')+';color:'+(st.k==='off'?'#fff':'inherit')+'">Can\'t work</button></div>'
    + (st.k==='part' ? '<div style="display:flex;gap:5px;align-items:center;margin-top:9px;flex-wrap:wrap"><span class="muted" style="font-size:12.5px">from</span>'
        +'<select onchange="_asWin('+nm+','+wd+',this.value,null)" style="padding:6px 7px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:12.5px;background:var(--card,#fff);color:inherit">'+_timeOpts(st.from)+'</select>'
        +'<span class="muted" style="font-size:12.5px">to</span>'
        +'<select onchange="_asWin('+nm+','+wd+',null,this.value)" style="padding:6px 7px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:12.5px;background:var(--card,#fff);color:inherit">'+_timeOpts(st.to)+'</select>'
        +'<button onclick="_asSpread('+nm+','+wd+')" style="border:none;background:transparent;font-size:12.5px;font-weight:600;cursor:pointer;color:#2F7F91;padding:0;margin-top:4px;width:100%;text-align:left">Use these hours every day they work</button></div>' : '')
    +'<button onclick="var p=document.getElementById(\'asPop\');if(p)p.remove()" style="margin-top:10px;border:none;background:transparent;font-size:12.5px;cursor:pointer;color:var(--muted,#6b8087);padding:0">Close</button>';
...
window.openStationClean=async function(){
  ...
  w.style.cssText='position:fixed;inset:0;z-index:10060;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;padding:18px';
  w.innerHTML='<div id="scCard" style="background:var(--card,#fff);color:var(--ink,#111);border-radius:12px;max-width:640px;width:100%;max-height:88vh;overflow:auto;padding:22px 24px;box-shadow:0 20px 60px rgba(0,0,0,.3)"></div>';
...
window._scRender=function(){
  ...
  let h='<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">'
    +'<div><div style="font-weight:800;font-size:18px">Tidy up your stations</div>'
    +'<div class="muted" style="font-size:14px;margin-top:4px;line-height:1.55">Rename anything to what your team actually calls it. Merge duplicates so nobody loses a skill. Drop the ones that are ranks or catch-alls rather than a job someone stands at.</div></div>'
    +'<button onclick="var m=document.getElementById(\'scModal\');if(m)m.remove()" style="border:none;background:transparent;font-size:26px;cursor:pointer;line-height:1;color:inherit">&times;</button></div>';
  ...
  L.forEach(function(r,i){
    ...
    h+='<div style="border:1px solid '+(r.drop?'#E4B8A8':'var(--line2,#d5dde0)')+';background:'+(r.drop?'#F9EDE8':'transparent')+';border-radius:12px;padding:11px 13px">'
      +'<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">'
      +'<input value="'+esc(r.name)+'" oninput="_scName('+i+',this.value)" '+(r.drop?'disabled':'')+' style="flex:1;min-width:150px;padding:8px 11px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:transparent;color:inherit">'
      +'<span class="muted" style="font-size:12.5px;white-space:nowrap">'+r.count+' '+(r.count===1?'person':'people')+'</span>'
      ...
      +'</div>'
      + (flag && !r.drop ? '<div style="font-size:12.5px;margin-top:6px;color:#7A5B1E">'+esc(flag)+'</div>' : '')
      + (r.mergeTo ? '<div style="font-size:12.5px;margin-top:6px;color:#2F7F91">Everyone here keeps the skill as <b>'+esc(r.mergeTo)+'</b>.</div>' : '')
      + (!r.drop && !r.mergeTo ? (r.splitTo!=null
          ? '<div style="display:flex;gap:7px;align-items:center;margin-top:8px;flex-wrap:wrap"><span class="muted" style="font-size:12.5px">and also</span>'
            +'<input value="'+esc(r.splitTo)+'" oninput="_scSplitName('+i+',this.value)" placeholder="second station" style="flex:1;min-width:130px;padding:7px 10px;border:1px solid var(--line2,#d5dde0);border-radius:8px;font-size:14px;background:transparent;color:inherit">'
            ...
            +'<div style="width:100%;font-size:12.5px;color:#2F7F91">Both stations are created, and everyone who had this one gets both. Prune the ones who should only do one.</div></div>'
  ...
  h+='</div>';
  h+='<button onclick="_scAdd()" style="margin-top:10px;background:transparent;border:1px dashed var(--line2,#d5dde0);border-radius:8px;padding:9px 14px;font-size:14px;cursor:pointer;color:inherit">+ Add a station</button>';
  h+='<div style="display:flex;gap:9px;margin-top:18px;align-items:center;flex-wrap:wrap">'
    +'<button onclick="_scSave()" id="scGo" style="background:var(--brand,#4a9cad);color:#fff;border:none;border-radius:8px;padding:11px 20px;font-weight:700;cursor:pointer">Save stations</button>'
    +'<button onclick="var m=document.getElementById(\'scModal\');if(m)m.remove()" style="background:transparent;border:1px solid var(--line2,#d5dde0);border-radius:8px;padding:11px 16px;cursor:pointer;color:inherit">Cancel</button>'
...
async function teamRoster(v){
  ...
  ordered.forEach(pos=>{ const c=POS_COL[pos]||'#94A3B8'; h+=`<div data-sgroup><div class="band" style="background:${c}14;color:${c};border-left:4px solid ${c};margin:14px 0 0">${esc(pos)} <span style="opacity:.7;font-weight:500">· ${groups[pos].length}</span></div><div class="card" style="margin-top:0">`+groups[pos].slice().sort().map(n=>{ const p=profileOf(n); const inits=(n||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase(); const sub=p.mobile?esc(p.mobile):'Tap to view profile'; return `<div data-srow class="row" style="padding:11px 15px;border-bottom:1px solid var(--line);cursor:pointer" onclick='openProfile(${JSON.stringify(n)})'><span class="av">${esc(inits)}</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(n)}</div><div class="faint" style="font-size:12.5px">${sub}</div></div><i class="ti ti-chevron-right" style="color:var(--faint)"></i></div>`; }).join('')+`</div></div>`; });
  h+=`</div>`;
  v.innerHTML=h;
}
async function teamFormer(v){
  ...
  h+=`<input id="fsearch" placeholder="Search former team by name…" oninput="var q=this.value.toLowerCase();var n=0;document.querySelectorAll('#formerlist>[data-fn]').forEach(function(el){var m=el.getAttribute('data-fn').indexOf(q)>=0;el.style.display=m?'':'none';if(m)n++;});var c=document.getElementById('fcount');if(c)c.textContent=q?(n+' of ${archRows.length}'):('${archRows.length} former');" style="width:100%;padding:11px 13px;border:1px solid var(--line2);border-radius:8px;font-size:14px;background:var(--card);color:var(--ink);font-family:inherit"/><div class="faint" id="fcount" style="font-size:12.5px;margin:8px 0 12px">${archRows.length} former</div>`;
...
async function vOnboarding(v){
  ...
  let h='';
  if(dueList.length){ h+=`<div class="card" style="padding:15px 17px;margin-bottom:18px;background:var(--brand-soft);border-color:var(--brand-line)"><div style="font-weight:600;color:var(--ink);font-size:15.5px;text-transform:none;letter-spacing:-.012em;margin-bottom:9px"><i class="ti ti-bell"></i> Reviews due now</div>`+dueList.slice(0,10).map(x=>`<div class="row" style="padding:6px 0;border-top:1px solid var(--line);align-items:center;gap:8px"><div style="flex:1;font-size:14px"><b>${esc(x.hh.name)}</b> — ${esc(x.m.l)} <span class="faint" style="font-size:12.5px">· due ${_d(x.due).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span></div><button class="btn pri" style="width:auto;padding:5px 12px" onclick="openReviewMenu(${x.hh.id},'${x.m.k}')">Start</button></div>`).join('')+`</div>`; }
  h+=`<div class="card" style="padding:16px;margin-bottom:18px"><div style="font-weight:700;margin-bottom:4px">Add a new hire</div><div class="faint" style="font-size:12.5px;margin-bottom:11px">Name + position puts them on the schedule, starts their checklist, and schedules their review milestones. Their full profile lives in <b style="color:var(--brand);cursor:pointer" onclick="state.ctx.ttab='roster';go('team')">Team</b>.</div><div class="row" style="gap:8px;flex-wrap:wrap"><input id="hname" placeholder="Full name" style="flex:1;min-width:150px;padding:10px 12px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><select id="hpos" style="padding:10px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${POS_PICK.map(o=>`<option value="${o}"${o==='Team Member'?' selected':''}>${o}</option>`).join('')}</select><input id="hdate" type="date" value="${isoDate(new Date())}" style="padding:10px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><button class="btn pri" style="width:auto" onclick="addHire()">Add hire</button></div><div class="faint" style="font-size:12.5px;margin-top:9px">Share your join code so they sign in and start training. Employment paperwork (I-9, W-4) is handled by your onboarding provider.</div></div>`;
...
window.viewReviews=function(hireId,mkey){ const revs=((window._revByHire||{})[hireId]||[]).filter(r=>r.milestone===mkey); const hh=(window._hires||[]).find(x=>x.id==hireId); const otLbl={on:'On track',watch:'Keep an eye',concern:'Needs support'}; let h=`<div style="font-weight:800;font-size:18px;margin-bottom:2px">${esc(hh?hh.name:'')}</div><div class="faint" style="font-size:14px;margin-bottom:14px">${esc(_milLabel(mkey))} · ${revs.length} submitted</div>`; if(revs.length>=2){ h+=`<button class="btn" style="width:100%;margin-bottom:6px" onclick="aiReviewSummary(${hireId},'${mkey}')">✨ AI summary + self-awareness gap check</button><div id="revAiBox" style="margin-bottom:12px"></div>`; } h+=revs.map(r=>{ const rd=Object.keys(r.ratings||{}).map(k=>`${esc(k)}: <b>${r.ratings[k]}</b>`).join(' · '); return `<div class="card" style="padding:13px 15px;margin-bottom:10px;box-shadow:none;border:1px solid var(--line)"><div style="font-size:15.5px;font-weight:600;color:${r.role==='self'?'var(--muted)':'var(--brand)'};text-transform:none;letter-spacing:-.012em;margin-bottom:5px">${r.role==='self'?'Self review':'Leadership'}${r.reviewer?' · '+esc(r.reviewer):''}${r.onTrack?' · '+(otLbl[r.onTrack]||''):''}</div>${rd?`<div style="font-size:12.5px;margin-bottom:6px">${rd}</div>`:''}${r.notes&&r.notes.a?`<div style="font-size:14px;margin-bottom:4px">${esc(r.notes.a)}</div>`:''}${r.notes&&r.notes.b?`<div style="font-size:14px;color:var(--muted)">${esc(r.notes.b)}</div>`:''}</div>`; }).join(''); h+=`<button class="btn" style="width:100%;margin-top:4px" onclick="openReviewMenu(${hireId},'${mkey}')">Back</button>`; _revOverlay(h); };
window.aiReviewSummary=async function(hireId,mkey){ const revs=((window._revByHire||{})[hireId]||[]).filter(r=>r.milestone===mkey); const box=document.getElementById('revAiBox'); if(!box)return; box.innerHTML='<div class="faint" style="font-size:14px;padding:6px 0">Reading the reviews…</div>'; const ctx=revs.map(r=>{ const rd=Object.keys(r.ratings||{}).map(k=>k+'='+r.ratings[k]).join(', '); return (r.role==='self'?'SELF':'LEADERSHIP'+(r.reviewer?' ('+r.reviewer+')':''))+' — ratings: '+(rd||'none')+'. Going well: '+((r.notes&&r.notes.a)||'-')+'. Work on: '+((r.notes&&r.notes.b)||'-')+'. Overall: '+(r.onTrack||'-'); }).join('\n'); const q='Combine these new team member reviews into one short manager-ready summary: (1) overall how they are doing, (2) the main strengths, (3) what to work on, and (4) any SELF-AWARENESS GAP — a dimension where the person rated themselves clearly higher or lower than leadership did (name the dimension and the direction). Concrete, brief, plain restaurant language. Do not invent anything not in the reviews.'; const ans=await aiCall({mode:'reviewsum',question:q,context:ctx}); box.innerHTML= ans ? `<div style="font-size:14px;line-height:1.6;white-space:pre-wrap;background:var(--brand-soft);border-radius:8px;padding:13px 15px">${esc(ans)}</div>` : '<div class="faint" style="font-size:14px">AI is busy — try again in a moment.</div>'; };
...
/* Catering leads the list because it is the one that goes wrong. A future order read off
   the POS screen as if it were today's, then forgotten on the morning it matters -- that is
   the specific failure this calendar exists to stop. */
const CAL_KINDS={catering:['🍱','Catering'],delivery:['🚚','Delivery'],holiday:['🎉','Holiday'],event:['⭐','Event'],rm:['🔧','Repair / maint.'],clean:['🧽','Deep clean'],order:['🛍️','Order'],note:['📝','Note'],task:['📌','Task'],people:['👋','People']};
...
async function vCalendar(v){
  ...
  if(hol[sel]) h+=`<div class="card" style="padding:12px 15px;margin-bottom:10px;background:var(--amber-soft);border-color:var(--amber)"><b style="color:var(--amber)">🎉 ${esc(hol[sel].name)}</b> <span class="muted" style="font-size:14px">${hol[sel].closed?"— closed. End of day: perishables prepped today are waste after close, so don't over-prep.":"— banks and government closed, so expect it busier. Bump prep and staffing."}</span></div>`;
  h+=`<div class="sec">${esc(selLbl)}</div><div class="card">`+(selIts.length?selIts.map(it=>`<div class="lesson-row"><div style="font-size:15.5px">${(CAL_KINDS[it.kind]||['•'])[0]}</div><div style="flex:1;min-width:0"><div style="font-weight:500;${it.done?'text-decoration:line-through;color:#999':''}">${esc(it.title)}</div>${it.detail?`<div class="faint" style="font-size:12.5px">${esc(it.detail)}</div>`:''}</div><button class="btn" style="width:auto" onclick="toggleDayItem(${it.id},${it.done?'false':'true'})">${it.done?'✓ Done':'Mark done'}</button><button class="btn" style="width:auto;padding:4px 9px" onclick="delDayItem(${it.id})">✕</button></div>`).join(''):`<div style="padding:20px;text-align:center" class="faint">Nothing scheduled.</div>`)+`</div>`;
  h+=`<div class="card" style="padding:14px;margin-top:10px"><div style="font-weight:600;font-size:14px;margin-bottom:8px">Add to ${esc(selLbl)}</div><div class="row" style="gap:8px;flex-wrap:wrap"><select id="calkind" onchange="calKindChanged()" style="padding:9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">`+Object.keys(CAL_KINDS).map(k=>`<option value="${k}">${CAL_KINDS[k][1]}</option>`).join('')+`</select><input id="caltitle" placeholder="What's happening?" onkeydown="if(event.key==='Enter')calAdd('${sel}')" style="flex:1;min-width:150px;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div><div class="row" style="gap:8px;margin-top:8px"><input id="caldetail" placeholder="Details (optional)" onkeydown="if(event.key==='Enter')calAdd('${sel}')" style="flex:1;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><button class="btn pri" style="width:auto" onclick="calAdd('${sel}')">Add</button></div>
    <div id="calPrepWrap" class="row" style="gap:9px;margin-top:9px;align-items:center;flex-wrap:wrap"><span style="font-size:14px;color:var(--ink2)">Start getting ready</span><select id="calprep" style="padding:8px 10px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px"><option value="0">on the day</option><option value="1" selected>1 day before</option><option value="2">2 days before</option><option value="3">3 days before</option><option value="7">a week before</option></select><span class="faint" style="font-size:13px;flex:1;min-width:170px">It shows up on Today that morning, so a future order stops being a surprise on the day.</span></div><div class="faint" style="font-size:12.5px;margin-top:7px">A holiday auto-adds a prep &amp; staffing heads-up to the Lineup the day before and the day of.</div></div>`;
  v.innerHTML=h;
}
...
/* Only catering, deliveries and orders have a useful "start getting ready" day. Showing a
   prep field on a birthday would be noise, so it appears for the kinds that earn it. */
const CAL_PREP_KINDS=new Set(['catering','delivery','order']);
window.calKindChanged=function(){
  const k=(document.getElementById('calkind')||{}).value;
  const w=document.getElementById('calPrepWrap');
  if(w) w.style.display = CAL_PREP_KINDS.has(k) ? 'flex' : 'none';
};
window.calAdd=async function(iso){ const kind=document.getElementById('calkind').value; const title=val('caltitle'); if(!title)return; const detail=val('caldetail'); await sb.from('day_items').insert({title,kind,detail,on_date:iso,created_by:state.user.id});
  /* The order sits on its own date. A separate heads-up sits on the day the work starts,
     which is the day somebody can still act on it. Two rows rather than one field, because
     Today reads by date and this way it needs no special case. */
  if(CAL_PREP_KINDS.has(kind)){
    const lead=+((document.getElementById('calprep')||{}).value||0);
    if(lead>0){
      ...
      await sb.from('day_items').insert({
        title:'Get ready: '+title,
        kind:'task',
        detail:(CAL_KINDS[kind]||['',''])[1]+' on '+when+(detail?' — '+detail:''),
        on_date:isoDate(d), created_by:state.user.id
      });
    }
...
async function vSettings(v){
  ...
  <div class="card" style="padding:14px 18px;max-width:480px;margin-top:16px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
    <span style="font-size:14px;font-weight:600;letter-spacing:-.012em">Version</span>
    <code style="font-size:14px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:var(--bg);border:1px solid var(--line);border-radius:5px;padding:4px 9px">${esc(window.BUILD||'unknown')}</code>
    <span class="faint" style="font-size:13px;flex:1;min-width:150px">If two devices disagree about what the app looks like, this tells us which one is behind.</span>
  </div>
  <div class="card" style="padding:22px;max-width:480px;margin-top:16px">
    ...
    <div class="faint" style="font-size:13px;line-height:1.6">
      <b style="color:var(--ink)">iPhone:</b> open the site in Safari &rarr; Share &rarr; Add to Home Screen.<br>
      <b style="color:var(--ink)">Android:</b> open in Chrome &rarr; menu &rarr; Install app.
    </div>
    <div class="faint" style="font-size:12.5px;margin-top:11px;line-height:1.5;border-top:1px solid var(--line);padding-top:11px">
      iPhone copies the icon once, when it is added. If you change your logo later, anyone who already saved it keeps the old one until they remove it and add it again.
    </div>
  </div>
  ${(state.profile&&state.profile.role==='admin')?`<div class="card" style="padding:22px;max-width:480px;margin-top:16px">
    <div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;margin-bottom:4px">See what your team sees</div>
    <div class="faint" style="font-size:14px;margin-bottom:13px;line-height:1.5">Switch the whole app to a team member's view so you can check what they can and cannot reach. It used to sit in the sidebar on every screen &mdash; it belongs where you come to change things.</div>
    <button class="btn" style="width:auto" onclick="togglePreviewLIT()"><i class="ti ti-eye"></i> Preview as Leader in Training</button>
  </div>`:''}
  <!--SEC:rules--><div class="card" style="padding:22px;max-width:480px;margin-top:16px"><div style="font-size:14px;font-weight:600;margin-bottom:6px">Stations &amp; skills</div><div class="faint" style="font-size:12.5px;margin-bottom:12px">These now live in <b>The Brain</b>, together with who can work each one, so there is one place to change them instead of two that can disagree.</div><button class="btn" style="width:auto" onclick="go('brain')">Open The Brain</button></div>
  <div class="card" data-savebar="saveScheduleRules|Scheduling and labor rules" style="padding:22px;max-width:480px;margin-top:16px"><div style="font-size:14px;font-weight:600;margin-bottom:6px">Scheduling &amp; labor rules</div><div class="faint" style="font-size:12.5px;margin-bottom:14px">Set your area so the schedule flags overtime, meal breaks, rest gaps, and advance-notice for you. This is a helpful heads-up, not legal advice or a compliance guarantee.</div>
    <div style="margin-bottom:16px"><label style="display:block;font-size:14px;font-weight:600;margin-bottom:6px">Your area (labor law)</label><select id="setlaw" style="width:100%;max-width:300px;padding:10px 12px;border:1px solid var(--line2);border-radius:8px;font-size:14px;background:var(--card);color:var(--ink);font-family:inherit">${LAW_OPTS.map(o=>`<option value="${o[0]}"${(s.law_jurisdiction||'AZ')===o[0]?' selected':''}>${esc(o[1])}</option>`).join('')}</select><div class="faint" style="font-size:12.5px;margin-top:5px">Pick the strictest that applies to your restaurant. Don't see yours? Start with your state.</div></div>
    <div style="margin-bottom:18px"><label style="display:block;font-size:14px;font-weight:600;margin-bottom:6px">Open floor — earliest start</label><input id="setfloor" type="time" value="${esc(s.open_floor||'05:30')}" style="padding:10px 12px;border:1px solid var(--line2);border-radius:8px;font-size:14px;background:var(--card);color:var(--ink);font-family:inherit"/><div class="faint" style="font-size:12.5px;margin-top:5px">The earliest anyone can be scheduled to start — the draft won't place a shift before this.</div></div>
    <div class="row"><button class="btn pri" style="width:auto" onclick="saveScheduleRules()">Save</button><span class="muted" id="schrulemsg" style="font-size:14px;margin-left:10px"></span></div></div>
  <!--SEC:access--><div class="card" style="padding:22px;max-width:520px;margin-top:16px"><div style="font-size:14px;font-weight:600;margin-bottom:6px">Access &amp; permissions</div><div class="faint" style="font-size:12.5px;margin-bottom:12px">Set the minimum role that can open each area. Owners always see everything. A person's role comes from their position on the team — Owner, Manager, Supervisor, or everyone else counts as Team member.</div><div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:14px"><thead><tr style="color:var(--muted)"><th style="text-align:left;padding:5px 8px">Area</th><th style="text-align:left;padding:5px 8px">Who can open it</th></tr></thead><tbody>${Object.keys(PERM_LABELS).map(pg=>`<tr style="border-top:1px solid var(--line)"><td style="padding:7px 8px;font-weight:600">${esc(PERM_LABELS[pg])}</td><td style="padding:7px 8px"><select class="permSel" data-pg="${pg}" onchange="savePerms()" style="padding:7px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${[5,4,3,2,1].map(r=>`<option value="${r}"${permOf(pg)===r?' selected':''}>${ROLE_LABELS[r]}${r===1?' (everyone)':(r===5?' only':' and up')}</option>`).join('')}</select></td></tr>`).join('')}</tbody></table></div><div style="border-top:1px solid var(--line);margin-top:14px;padding-top:14px"><div class="row" style="gap:10px;align-items:center;flex-wrap:wrap"><span style="font-size:14px;font-weight:600;flex:1;min-width:180px">Team members' schedule view</span><select id="teamViewSel" onchange="savePerms()" style="padding:7px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"><option value="team"${(!state.perms||state.perms._teamView!=='own')?' selected':''}>Whole team schedule (standard)</option><option value="own"${(state.perms&&state.perms._teamView==='own')?' selected':''}>Only their own shifts</option></select></div><div class="faint" style="font-size:12.5px;margin-top:5px">Supervisors and up always see the whole roster. This only changes what a Team member sees.</div></div><div class="row" style="margin-top:14px"><button class="btn pri" style="width:auto" onclick="savePerms()">Save access levels</button><span class="muted" id="permsg" style="font-size:14px;margin-left:10px"></span></div></div>
  <div class="card" style="padding:22px;max-width:520px;margin-top:16px"><div style="font-size:14px;font-weight:600;margin-bottom:6px">Give one person extra access</div><div class="faint" style="font-size:12.5px;margin-bottom:12px">On top of their role, let a specific person into one or more areas &mdash; like a scheduler who's still a team member. Pick the person, check the areas, Save. (Owners already see everything.)</div><div id="grantList" style="margin-bottom:14px">${_grantListHTML()}</div><div class="faint" style="font-size:12.5px;font-weight:700;margin-bottom:5px">Person</div><select id="grantWho" onchange="grantSel()" style="width:100%;max-width:300px;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;margin-bottom:14px">${_grantRoster().map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join('')}</select><div class="faint" style="font-size:12.5px;font-weight:700;margin-bottom:6px">Extra areas they can open</div><div id="grantPages" style="display:flex;flex-wrap:wrap;gap:13px;margin-bottom:14px">${_grantPageList().map(pg=>`<label style="display:inline-flex;align-items:center;gap:6px;font-size:14px;cursor:pointer"><input type="checkbox" class="grantPg" value="${pg}"/> ${esc(PERM_LABELS[pg]||pg)}</label>`).join('')}</div><div class="row"><button class="btn pri" style="width:auto" onclick="saveUserGrant()">Save this person's access</button><span class="muted" id="grantMsg" style="font-size:14px;margin-left:10px"></span></div></div>`;
  /* Settings was one scroll holding five unrelated jobs, three Save buttons and a wall of
     dropdowns. The markup was fine; the problem was that nothing told you which job you
     were doing. Same blocks, split into three, behind the tile pattern the Academy uses. */
  const _parts={}; _sx.split('<!--SEC:').slice(1).forEach(chunk=>{ const k=chunk.slice(0,chunk.indexOf('-->')); _parts[k]=chunk.slice(chunk.indexOf('-->')+3); });
  const SECTS=[
    ['shop','Your restaurant','Name, colour, logo and the code your team joins with','ti-building-store','linear-gradient(135deg,#4A9CAD,#2A6E7A)'],
    ['rules','Scheduling rules','Your state\u2019s labour law and the earliest anyone can start','ti-gavel','linear-gradient(135deg,#5C7CE0,#2F4BA8)'],
    ['access','Who can see what','Which roles open which areas, and extra access for one person','ti-lock','linear-gradient(135deg,#B0539B,#78256A)']
  ];
...
function _renderDirtyBar(){
  ...
  el.innerHTML='<span style="font-size:14px;font-weight:700;flex:1;min-width:150px">Unsaved changes'+(window._dirty.label?' &middot; '+esc(window._dirty.label):'')+'</span>'
    +'<button onclick="_dirtyDiscard()" style="border:1px solid rgba(255,255,255,.45);background:transparent;color:#fff;border-radius:8px;padding:8px 14px;font-size:14px;font-family:inherit;cursor:pointer">Discard</button>'
    +'<button onclick="_dirtySave()" style="border:none;background:var(--accent);color:#3A2B00;font-weight:800;border-radius:8px;padding:8px 16px;font-size:14px;font-family:inherit;cursor:pointer">Save now</button>';
}
...
function _grantListHTML(){ const g=state.grants||{}; const names=Object.keys(g).filter(n=>(g[n]||[]).length); if(!names.length) return '<div class="faint" style="font-size:12.5px">No one has extra access yet.</div>'; return names.sort().map(n=>`<div class="row" style="gap:8px;align-items:center;padding:7px 0;border-bottom:1px solid var(--line)"><span style="font-weight:600;font-size:14px;flex-shrink:0">${esc(n)}</span><span class="faint" style="font-size:12.5px;flex:1">${g[n].map(p=>esc(PERM_LABELS[p]||p)).join(', ')}</span><span style="color:#B32D2D;cursor:pointer;font-size:12.5px;flex-shrink:0" onclick='removeUserGrant(${JSON.stringify(n)})'>Remove</span></div>`).join(''); }
...
function renderPaywall(bs){ const admin=(state.profile&&state.profile.role==='admin'); const nm=(state.settings&&state.settings.academy_name)||'your academy'; const brand=(state.settings&&state.settings.brand_color)||DEFAULT_BRAND; const brandNew=bs.status==='needs_checkout'; const expired=bs.status==='trial_expired'; const head=brandNew?'Start your 14-day free trial':(expired?'Your 14-day trial has ended':'Your subscription is paused'); const sub=brandNew?('Add a card to start your free trial of '+esc(nm)+'. You won’t be charged until the 14 days are up — cancel anytime before then.'):(expired?('Thanks for trying '+esc(nm)+'. Add a payment method to keep your team, schedules, and training going.'):'Reactivate to restore full access. Your data is safe.'); const app=document.getElementById('app')||document.body;
  app.innerHTML=`<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--bg)"><div class="card" style="max-width:440px;width:100%;padding:30px 28px;text-align:center"><div style="width:54px;height:54px;border-radius:12px;margin:0 auto 16px;display:flex;align-items:center;justify-content:center;background:${brand};color:#fff;font-size:26px"><i class="ti ti-lock"></i></div><div style="font-weight:800;font-size:18px;margin-bottom:8px">${head}</div><div class="muted" style="font-size:14px;line-height:1.6;margin-bottom:20px">${sub}</div>`+
    (admin?`<div class="card" style="padding:14px 16px;margin-bottom:12px;text-align:left;border:1px solid var(--brand-line)"><div class="row" style="justify-content:space-between;align-items:center"><div><div style="font-weight:700">Monthly</div><div class="faint" style="font-size:12.5px">Billed each month</div></div><div style="font-weight:800;font-size:18px">$149<span class="faint" style="font-size:12.5px;font-weight:600">/location</span></div></div></div><div class="card" style="padding:14px 16px;margin-bottom:16px;text-align:left;border:1px solid var(--brand-line);position:relative"><div class="pill" style="position:absolute;top:-9px;right:12px;background:#1B7B3F;color:#fff;font-size:11.5px;font-weight:800;padding:2px 8px">2 MONTHS FREE</div><div class="row" style="justify-content:space-between;align-items:center"><div><div style="font-weight:700">Annual</div><div class="faint" style="font-size:12.5px">$1,490/location billed yearly</div></div><div style="font-weight:800;font-size:18px">$124<span class="faint" style="font-size:12.5px;font-weight:600">/mo</span></div></div></div><button id="bpaybtn" class="btn pri" style="width:100%;margin-bottom:8px" onclick="startCheckout('month')">${brandNew?'Start free trial — monthly':'Add payment — start monthly'}</button><button class="btn" style="width:100%" onclick="startCheckout('year')">${brandNew?'Start free trial — yearly':'Pay yearly &amp; save'}</button>`
...
function updateBillingBanner(){ let el=document.getElementById('billingBanner'); if(!BILLING_LIVE){ if(el)el.remove(); return; } const bs=billingState(); const admin=(state.profile&&state.profile.role==='admin'); let msg='',bg='',fg='',act=''; if(bs.status==='trialing'){ const d=bs.daysLeft; if(d==null||d>5){ if(el)el.remove(); return; } msg=(d<=0?'Your trial ends today':('Trial ends in '+d+' day'+(d===1?'':'s')))+'.'; bg='#FFF7E6'; fg='#8A5A00'; if(admin)act='<button class="btn pri" style="width:auto;padding:4px 12px;font-size:12.5px;margin-left:10px" onclick="startCheckout(\'month\')">Add payment</button>'; }
  else if(bs.status==='past_due'){ msg='Payment didn’t go through — update your card to avoid losing access.'; bg='#FCEBEB'; fg='#8A1C1C'; if(admin)act='<button class="btn" style="width:auto;padding:4px 12px;font-size:12.5px;margin-left:10px" onclick="openBillingPortal()">Update card</button>'; }
