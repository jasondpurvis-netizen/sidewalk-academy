...
async function vSettings(v){
  ...
  <div class="card" data-savebar="saveStations|Stations and skills" style="padding:22px;max-width:480px;margin-top:16px"><div style="font-size:13px;font-weight:600;margin-bottom:6px">Stations &amp; skills</div><div class="faint" style="font-size:12px;margin-bottom:12px">Your restaurant's own roles — bake, register, bar, grill, drive-thru, whatever fits your restaurant. These become checkboxes on each person's profile, so the schedule only puts people where they're trained. Open and close are built in, so no need to add them.</div><div id="stationList" style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:10px"></div><div class="row" style="gap:8px"><input id="stationNew" type="text" placeholder="Add a station…" style="flex:1;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit" onkeydown="if(event.key==='Enter'){addStation();event.preventDefault();}"/><button class="btn" style="width:auto" onclick="addStation()">Add</button></div><div class="row" style="margin-top:12px"><button class="btn pri" style="width:auto" onclick="saveStations()">Save stations</button><span class="muted" id="stmsg" style="font-size:13px;margin-left:10px"></span></div></div>
  ...
  ((state.settings&&state.settings.stations&&state.settings.stations.length)?state.settings.stations:['Bake / Prep','Register','Bar / Espresso']).forEach(st=>_addStationPill(st));
...
window.saveStations=async function(){ window.clearDirty&&window.clearDirty(); const arr=[...document.querySelectorAll('#stationList [data-st]')].map(e=>e.getAttribute('data-st')).filter(Boolean); const m=document.getElementById('stmsg'); if(m){m.style.color='';m.textContent='Saving…';}
