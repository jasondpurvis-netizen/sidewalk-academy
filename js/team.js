...
async function vCalendar(v){
  ...
  h+=`<div class="card" style="padding:14px;margin-top:10px"><div style="font-weight:600;font-size:14px;margin-bottom:8px">Add to ${esc(selLbl)}</div><div class="row" style="gap:8px;flex-wrap:wrap"><select id="calkind" style="padding:9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">`+Object.keys(CAL_KINDS).map(k=>`<option value="${k}">${CAL_KINDS[k][1]}</option>`).join('')+`</select><input id="caltitle" placeholder="What's happening?" onkeydown="if(event.key==='Enter')calAdd('${sel}')" style="flex:1;min-width:150px;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div><div class="row" style="gap:8px;margin-top:8px"><input id="caldetail" placeholder="Details (optional)" onkeydown="if(event.key==='Enter')calAdd('${sel}')" style="flex:1;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><button class="btn pri" style="width:auto" onclick="calAdd('${sel}')">Add</button></div><div class="faint" style="font-size:12.5px;margin-top:7px">A holiday auto-adds a prep &amp; staffing heads-up to the Lineup the day before and the day of.</div></div>`;
  v.innerHTML=h;
}
