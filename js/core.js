...
function buildTodoCard(d){ const dueRems=d.dueRems,todoItems=d.todoItems,allRems=d.allRems,openCount=d.openCount;
  ...
  const remPanel=`<div id="remPanel" style="display:none;margin-top:14px;padding-top:14px;border-top:1px dashed var(--line2)"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:10px"><i class="ti ti-repeat"></i> Recurring reminders</div>${remList}<div style="margin-top:12px;display:flex;flex-direction:column;gap:8px"><input id="remTitle" placeholder="Reminder — e.g. Change the water filter" onkeydown="if(event.key==='Enter')addReminder()" style="padding:9px 12px;border:1px solid var(--line2);border-radius:8px;font-family:inherit;font-size:14px;background:var(--card);color:var(--ink)"/><div class="row" style="gap:8px;flex-wrap:wrap;align-items:center"><span style="font-size:14px;color:var(--muted)">Repeats every</span><input id="remEvery" type="number" min="1" value="1" style="width:60px;padding:9px 10px;border:1px solid var(--line2);border-radius:8px;font-family:inherit;font-size:14px;background:var(--card);color:var(--ink)"/><select id="remUnit" style="padding:9px 10px;border:1px solid var(--line2);border-radius:8px;font-family:inherit;font-size:14px;background:var(--card);color:var(--ink)"><option value="day">Days</option><option value="week">Weeks</option><option value="month" selected>Months</option><option value="year">Years</option></select><span style="font-size:14px;color:var(--muted)">next due</span><input id="remNext" type="date" value="${isoDate(new Date())}" style="padding:9px 10px;border:1px solid var(--line2);border-radius:8px;font-family:inherit;font-size:14px;background:var(--card);color:var(--ink)"/><button class="btn pri" style="width:auto;padding:9px 17px;margin-left:auto" onclick="addReminder()">Add reminder</button></div></div></div>`;
...
async function vCostSmart(v){
  ...
  h+=`<div class="card" style="padding:16px 18px;margin-bottom:12px;border-color:var(--brand-line)"><div class="row" style="align-items:center;gap:12px"><div style="width:46px;height:46px;border-radius:12px;background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff;display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0"><i class="ti ti-calendar-plus"></i></div><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:15.5px;color:var(--ink)">Preview the leaner week</div><div style="font-size:14px;color:var(--muted);margin-top:2px">Builds a proposal to this target with your real team &mdash; <b>without touching your current schedule</b>. See it, then apply it or throw it away.</div></div></div><div class="row" style="align-items:center;gap:9px;margin-top:12px"><span style="font-size:14px;color:var(--muted);font-weight:700;white-space:nowrap">Preview for</span><select id="csBuildWkSel" onchange="state.ctx.csBuildWk=this.value" style="flex:1;padding:9px 11px;border:1px solid var(--line2);border-radius:8px;font-family:inherit;font-size:14px;background:var(--card);color:var(--ink)">${_wkOptHtml}</select></div><button class="btn pri" style="width:100%;margin-top:10px;padding:11px" onclick="csBuildSchedule()"><i class="ti ti-eye"></i> Preview my schedule</button></div>`;
