...
window._readFailed=function(table,err){
  ...
  box.innerHTML='<i class="ti ti-cloud-off" style="font-size:18px;flex:none;margin-top:1px"></i>'
    +'<div style="flex:1;min-width:0"><div style="font-weight:800;font-size:14px;margin-bottom:2px">Some of this page didn\'t load</div>'
    +'<div style="font-size:12.5px;line-height:1.5;opacity:.95;word-break:break-word">'+esc(msg)+'</div>'
    +'<div style="font-size:12.5px;opacity:.75;margin-top:4px">'+esc(table)+'. What you see may be incomplete &mdash; reload before acting on it.</div></div>'
...
window._writeFailed=function(table,op,err){
  ...
  box.innerHTML='<i class="ti ti-alert-triangle" style="font-size:18px;flex:none;margin-top:1px"></i>'
    +'<div style="flex:1;min-width:0"><div style="font-weight:800;font-size:14px;margin-bottom:2px">That didn\'t save</div>'
    +'<div style="font-size:12.5px;line-height:1.5;opacity:.95;word-break:break-word">'+esc(msg)+'</div>'
    +'<div style="font-size:12.5px;opacity:.75;margin-top:4px">'+esc(table)+' &middot; '+esc(op)+'. Your change was not stored, so try again.</div></div>'
...
function communityDigestCard(){
  ...
  return `<div class="card" style="padding:11px 15px;margin-bottom:14px;display:flex;align-items:center;gap:11px;cursor:pointer" onclick="go('community')"><div style="width:32px;height:32px;border-radius:8px;background:var(--brand-soft);color:var(--brand);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="ti ti-messages" style="font-size:18px"></i></div><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:14px">${tot} new in Community</div><div class="faint" style="font-size:12.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${parts}</div></div><i class="ti ti-chevron-right" style="color:var(--muted);flex-shrink:0"></i></div>`;
}
...
function renderApp(){
  ...
  root.innerHTML = `<div class="app">
    <aside class="side">
      ...
      ${(function(){
        ...
        return `<div style="padding:10px 14px 4px"><div class="faint" style="font-size:10.5px;letter-spacing:.14em;text-transform:uppercase;margin-bottom:5px">Restaurant</div>
          <select onchange="switchRestaurant(this.value)" style="width:100%;font-weight:600">
    ...
    <main class="main">${(typeof isQuiet==='function'&&isQuiet())?`<div style="background:#3A2B00;color:var(--accent);padding:9px 16px;font-size:14px;font-weight:600;display:flex;align-items:center;gap:9px;flex-wrap:wrap"><i class="ti ti-bell-off"></i><span>Practice mode \u2014 nobody is being notified of anything.</span>${(state.profile&&state.profile.role==='admin')||myRank()>=4?`<button onclick="toggleQuiet()" style="margin-left:auto;background:var(--accent);color:#3A2B00;border:none;border-radius:999px;padding:5px 14px;font-weight:800;font-size:13px;cursor:pointer;font-family:inherit">Start notifying the team</button>`:''}</div>`:''}${state.previewLIT?`<div style="background:var(--brand);color:#fff;padding:9px 16px;font-size:14px;font-weight:600;display:flex;align-items:center;gap:9px;flex-wrap:wrap"><i class="ti ti-eye"></i> Previewing as <select onchange="setPreviewRank(this.value)" style="padding:3px 7px;border-radius:8px;border:1px solid rgba(255,255,255,.5);background:transparent;color:#fff;font-family:inherit;font-size:14px;font-weight:700">${[[1,'Team member'],[2,'Supervisor'],[3,'Manager'],[4,'GM']].map(o=>`<option value="${o[0]}"${(state.previewRank||1)===o[0]?' selected':''} style="color:#1A1A1A">${o[1]}</option>`).join('')}</select> — this is what they see. <span onclick="togglePreviewLIT()" style="margin-left:auto;text-decoration:underline;cursor:pointer">Exit preview</span></div>`:''}<div class="mtop"><button class="mburger" onclick="toggleNav(true)" aria-label="Open menu"><i class="ti ti-menu-2"></i></button>${state.settings&&state.settings.logo_url?`<img class="mlogo" onclick="go('whiteboard')" style="cursor:pointer" src="${state.settings.logo_url}" alt="${esc((state.settings&&state.settings.academy_name)||'Academy')}"/>`:`<span class="mnm" onclick="go('whiteboard')" style="cursor:pointer">${esc((state.settings&&state.settings.academy_name)||'Academy')}</span>`}</div><div id="pagehero"></div><div class="wrap" id="view"></div></main>
...
function trackBanner(t,pct,done){ return `<div style="height:96px;background:${trackGrad(t)};position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden"><i class="ti ${trackIcon(t)}" style="font-size:40px;color:#fff;opacity:.97"></i><i class="ti ${trackIcon(t)}" style="position:absolute;right:-14px;bottom:-18px;font-size:96px;color:#fff;opacity:.11"></i>${done?`<span class="pill" style="position:absolute;top:10px;right:10px;background:rgba(255,255,255,.94);color:var(--green);font-size:12.5px;gap:4px;font-weight:600"><i class="ti ti-rosette-discount-check"></i>Certified</span>`:`<span style="position:absolute;top:10px;right:11px;background:rgba(255,255,255,.92);color:var(--brand);font-size:12.5px;font-weight:700;padding:3px 9px;border-radius:999px">${pct}%</span>`}</div>`; }
...
async function vWhiteboard(v){
  ...
  const wnote=(label,text)=>`<div style="flex:1;min-width:210px;background:#FBF9F5;border:1px solid #EBE5DB;border-radius:12px;padding:15px 17px;box-shadow:0 3px 12px rgba(60,50,30,.07);position:relative"><div style="position:absolute;top:-6px;left:20px;width:12px;height:12px;border-radius:50%;background:var(--brand);box-shadow:0 2px 4px rgba(0,0,0,.3)"></div><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:6px">${label}</div><div style="font-size:15.5px;line-height:1.45;font-weight:600;color:var(--ink)">${esc(text)}</div></div>`;
  let h='';
  if(mtext||vtext||isAdmin){
    if(mtext||vtext){
      h+=`<div class="phero" style="padding:26px 28px;margin-bottom:20px">${heroTile()}<div class="phero-in"><div class="row" style="align-items:center;margin-bottom:18px"><span style="font-size:12.5px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent)">Our Why</span>${isAdmin?`<button onclick="toggleWhyEdit()" style="margin-left:auto;border:1px solid rgba(255,255,255,.55);background:rgba(255,255,255,.15);color:#fff;border-radius:8px;padding:6px 12px;font-size:12.5px;cursor:pointer;font-family:inherit"><i class="ti ti-pencil"></i> Edit</button>`:''}</div>${mtext?`<div style="margin-bottom:${vtext?'20px':'0'}"><div style="font-size:12.5px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:7px">Mission</div><div class="mvtext" style="font-size:26px;font-weight:800;line-height:1.3;color:#fff">${esc(mtext)}</div></div>`:''}${vtext?`<div><div style="font-size:12.5px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:7px">Vision</div><div class="mvtext" style="font-size:26px;font-weight:800;line-height:1.3;color:#fff">${esc(vtext)}</div></div>`:''}</div></div>`;
    } else if(isAdmin){
      h+=`<div class="card" style="padding:16px 18px;margin-bottom:18px"><div class="row" style="align-items:center;margin-bottom:8px"><span style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink)">Our Why</span><button class="btn" style="width:auto;margin-left:auto;padding:5px 12px;font-size:12.5px" onclick="toggleWhyEdit()"><i class="ti ti-pencil"></i> Edit</button></div><div class="faint" style="font-size:14px">Add your mission and vision — they anchor the whole team, front and center.</div></div>`;
    }
  ...
  if(ops){
    ...
    else h+=`<div class="card">`+shifts.map(s=>{ const inits=(s.person_name||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase(); return `<div class="row" style="padding:10px 15px;border-bottom:1px solid var(--line)"><span class="av">${esc(inits)}</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(s.person_name||'—')}</div><div class="faint" style="font-size:12.5px">${esc(s.role||'Team')}</div></div><div style="font-size:14px;font-weight:600;color:var(--brand)">${s.start_time?fmtClock(s.start_time):''}${s.end_time?'–'+fmtClock(s.end_time):''}</div></div>`; }).join('')+`</div>`;
  } else {
    if(state.progress && state.progress.size===0){ h+=`<div class="card" style="padding:18px 20px;margin-bottom:16px;background:var(--brand-soft);border-color:var(--brand-line)"><div style="font-weight:700;font-size:15.5px;margin-bottom:5px">👋 Welcome to ${esc((state.settings&&state.settings.academy_name)||'the Academy')}</div><div class="muted" style="font-size:14px;line-height:1.62">This is your space to grow as a leader. Start with <b>Foundations of Leadership</b> below — read each module, jot a quick reflection at the end, and use <b>Community</b> to talk it through with the team. Take it at your own pace.</div></div>`; }
    let _c=null; for(const t of visibleTracks()){ for(const l of trackLessons(t.id)){ if(!isDone(l.id)){ _c={t,l}; break; } } if(_c) break; }
    if(_c){ h+=`<div class="sec">Continue your training</div><div class="card" style="padding:18px 20px;display:flex;align-items:center;gap:16px;margin-bottom:6px"><div style="width:46px;height:46px;border-radius:12px;background:var(--brand);color:#fff;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0"><i class="ti ti-player-play"></i></div><div style="flex:1;min-width:0"><div class="faint" style="font-size:15.5px;text-transform:none;letter-spacing:-.012em;font-weight:600">${esc(_c.t.name)}</div><div style="font-weight:600;font-size:15.5px;margin:3px 0 10px">${esc(_c.l.title)}</div><div class="bar"><i style="width:${trackPct(_c.t.id)}%"></i></div></div><button class="btn pri" style="width:auto;gap:6px" onclick="go('lesson',{tid:'${_c.t.id}',lid:'${_c.l.id}'})">Resume <i class="ti ti-arrow-right"></i></button></div>`; }
    else if(visibleTracks().length){ h+=`<div class="sec">Your training</div><div class="card" style="padding:22px;text-align:center"><div style="font-size:26px;margin-bottom:8px">🎓</div><div style="font-weight:600;margin-bottom:4px">You're all caught up</div><div class="muted" style="font-size:14px">Every module you've been assigned is complete. Nice work.</div></div>`; }
  }
...
function buildTodoCard(d){ const dueRems=d.dueRems,todoItems=d.todoItems,allRems=d.allRems,openCount=d.openCount;
  const remRows=dueRems.map(r=>`<div class="row" style="align-items:center;gap:11px;padding:8px 0;border-bottom:1px solid var(--line)"><span onclick="doneReminder('${r.id}')" title="Mark done for this cycle" style="width:22px;height:22px;border-radius:8px;border:2px solid var(--accent-2);background:transparent;cursor:pointer;flex-shrink:0"></span><span style="flex:1;min-width:0;font-size:14px;color:var(--ink)">${esc(r.title||'')}</span><span style="flex-shrink:0;display:inline-flex;align-items:center;gap:4px;font-size:12.5px;font-weight:700;color:var(--accent-ink);background:var(--accent-soft);border:1px solid var(--accent-line);border-radius:999px;padding:2px 8px"><i class="ti ti-repeat" style="font-size:12.5px"></i>${remCadenceLabel(r)}</span></div>`).join('');
  const todoRows=todoItems.map(t=>`<div class="row" style="align-items:center;gap:11px;padding:8px 0;border-bottom:1px solid var(--line)"><span onclick="toggleTodo('${t.id}',${t.done?0:1})" style="width:22px;height:22px;border-radius:8px;border:2px solid ${t.done?'var(--brand)':'var(--line2)'};background:${t.done?'var(--brand)':'transparent'};color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;font-size:14px">${t.done?'<i class="ti ti-check"></i>':''}</span><span style="flex:1;min-width:0;font-size:14px;${t.done?'text-decoration:line-through;color:var(--muted)':'color:var(--ink)'}">${esc(t.detail||'')}</span><span onclick="delTodo('${t.id}')" style="cursor:pointer;color:var(--muted);font-size:15.5px;flex-shrink:0" title="Remove"><i class="ti ti-x"></i></span></div>`).join('');
  const remList=allRems.length?allRems.map(r=>`<div class="row" style="align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid var(--line)"><span style="flex:1;min-width:0;font-size:14px;color:var(--ink)">${esc(r.title||'')}</span><span style="font-size:12.5px;color:var(--muted);flex-shrink:0">${remCadenceLabel(r)}${r.next_due?' · next '+remPrettyDate(r.next_due):''}</span><span onclick="delReminder('${r.id}')" style="cursor:pointer;color:var(--muted);font-size:15.5px;flex-shrink:0" title="Delete reminder"><i class="ti ti-trash"></i></span></div>`).join(''):`<div class="faint" style="font-size:14px">No reminders yet. Add one below.</div>`;
  const remPanel=`<div id="remPanel" style="display:none;margin-top:14px;padding-top:14px;border-top:1px dashed var(--line2)"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:10px"><i class="ti ti-repeat"></i> Recurring reminders</div>${remList}<div style="margin-top:12px;display:flex;flex-direction:column;gap:8px"><input id="remTitle" placeholder="Reminder — e.g. Change the water filter" onkeydown="if(event.key==='Enter')addReminder()" style=""/><div class="row" style="gap:8px;flex-wrap:wrap;align-items:center"><span style="font-size:14px;color:var(--muted)">Repeats every</span><input id="remEvery" type="number" min="1" value="1" style="width:60px"/><select id="remUnit" style=""><option value="day">Days</option><option value="week">Weeks</option><option value="month" selected>Months</option><option value="year">Years</option></select><span style="font-size:14px;color:var(--muted)">next due</span><input id="remNext" type="date" value="${isoDate(new Date())}" style=""/><button class="btn pri" style="width:auto;padding:9px 17px;margin-left:auto" onclick="addReminder()">Add reminder</button></div></div></div>`;
  const empty=(!todoItems.length&&!dueRems.length)?`<div class="faint" style="font-size:14px;padding:2px 0 4px">Nothing on the list. Add a to-do below, or set a recurring reminder.</div>`:'';
  return `<div id="todocard" style="border:1px solid #C0DD97;border-left:5px solid #639922;background:#F3F8EA;border-radius:12px;padding:15px 17px;margin-bottom:18px"><div class="row" style="align-items:center;margin-bottom:${(todoItems.length||dueRems.length)?'11':'6'}px"><span style="display:inline-flex;align-items:center;gap:7px;font-size:14px;font-weight:800;color:#27500A"><i class="ti ti-checkbox" style="font-size:18px;color:#3B6D11"></i>To-dos today</span><span style="margin-left:auto;display:inline-flex;gap:10px;align-items:center">${openCount?`<span style="font-size:12.5px;font-weight:700;color:var(--muted)">${openCount} open</span>`:''}<button onclick="toggleRemPanel()" class="btn" style="width:auto;padding:5px 11px;font-size:12.5px;gap:5px"><i class="ti ti-repeat"></i> Reminders</button></span></div>${empty}${remRows}${todoRows}<div class="row" style="gap:8px;margin-top:12px"><input id="todoinput" placeholder="Add a to-do for today…" onkeydown="if(event.key==='Enter')addTodo()" style="flex:1;min-width:0"/><button class="btn pri" style="width:auto;padding:9px 17px" onclick="addTodo()">Add</button></div>${remPanel}</div>`;
}
...
async function vCostSmart(v){
  ...
  h+=`<div class="card" style="padding:0;overflow:hidden;margin-bottom:12px;border-color:var(--brand-line)"><div style="padding:16px 18px 15px;background:linear-gradient(135deg,var(--brand-soft),#FFFDF9)"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">Your biggest opportunity</div>${!ld?`<div class="faint" style="font-size:14px">Labor history is still loading &mdash; your top move shows up once it's in.</div>`:(_big?`<div style="font-size:18px;font-weight:800;color:var(--ink);line-height:1.3">${dowShort[_big.dw]} ${hrRange(_big.startH,_big.endH)} &mdash; trim about ${_big.overSum.toFixed(1)} labor-hours</div><div style="font-size:15.5px;color:var(--accent-ink);font-weight:800;margin-top:4px">&asymp; ${fmt$(_big.saveWk)}/week &middot; ~${fmt$(_big.saveWk*52)}/yr</div>`:`<div style="font-size:18px;font-weight:800;color:var(--tealdark)"><i class="ti ti-circle-check"></i> You're dialed in &mdash; nothing worth trimming right now.</div>`)}${ld?`<div class="faint" style="font-size:12.5px;margin-top:10px">This week you're running <b>${wkPct.toFixed(0)}% of sales</b> on labor &middot; <b>$${Math.round(wkSPLH)}</b> per labor hour, against your ${tgtLabel}.</div>`:''}</div></div>`;
  h+=`<div class="card" style="padding:16px 18px;margin-bottom:12px;border-color:var(--brand-line)"><div class="row" style="align-items:center;gap:12px"><div style="width:46px;height:46px;border-radius:12px;background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff;display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0"><i class="ti ti-calendar-plus"></i></div><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:15.5px;color:var(--ink)">Preview the leaner week</div><div style="font-size:14px;color:var(--muted);margin-top:2px">Builds a proposal to this target with your real team &mdash; <b>without touching your current schedule</b>. See it, then apply it or throw it away.</div></div></div><div class="row" style="align-items:center;gap:9px;margin-top:12px"><span style="font-size:14px;color:var(--muted);font-weight:700;white-space:nowrap">Preview for</span><select id="csBuildWkSel" onchange="state.ctx.csBuildWk=this.value" style="flex:1">${_wkOptHtml}</select></div><button class="btn pri" style="width:100%;margin-top:10px;padding:11px" onclick="csBuildSchedule()"><i class="ti ti-eye"></i> Preview my schedule</button></div>`;
  let _csDetOpen=false; try{ _csDetOpen=localStorage.getItem('cs_detail_open')==='1'; }catch(e){}
  h+=`<div onclick="csToggleDetail()" style="cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;padding:11px;margin-bottom:10px;border:1px solid var(--line2);border-radius:12px;background:var(--card)"><span style="font-weight:700;font-size:14px;color:var(--brand)">Show the full breakdown</span><i id="csDetChev" class="ti ti-chevron-${_csDetOpen?'up':'down'}" style="color:var(--brand)"></i></div>`;
  h+=`<div id="csDetail" style="display:${_csDetOpen?'block':'none'}">`;
  h+=helpCard;
  h+=`<div class="card" style="padding:16px 18px;margin-bottom:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">Where you are</div><div style="font-size:15.5px;line-height:1.6;color:var(--ink)">In an average week you do <b>${fmt$(wkSalesTot)}</b> in sales on <b>${wkLaborTot.toFixed(0)} labor hours</b>${ld?` &mdash; about <b>${fmt$(wkLaborCost)}</b>, or <b>${wkPct.toFixed(0)}%</b> of sales, at <b>$${Math.round(wkSPLH)}</b> of sales for every hour of staff.`:`. <span class="faint">(Labor history is still loading.)</span>`}</div></div>`;
  h+=`<div class="card" style="padding:16px 18px;margin-bottom:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:10px">Your biggest fixes</div>`+(!ld?`<div class="faint" style="font-size:14px">Labor history is still loading &mdash; the ranked fixes show up once it's in.</div>`:(!topFixes.length?`<div style="font-size:14px;color:var(--tealdark)"><i class="ti ti-circle-check"></i> <b>You're dialed in.</b> Nothing worth changing this week. Cutting past this starts to risk your service and your growth &mdash; a schedule run this well is worth protecting.</div>`:topFixes.map((f,i)=>`<div class="row" style="align-items:flex-start;gap:11px;padding:9px 0;border-bottom:1px solid var(--line)"><div style="width:24px;height:24px;border-radius:50%;background:var(--accent-soft);color:var(--accent-ink);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12.5px;flex-shrink:0">${i+1}</div><div style="flex:1;min-width:0"><div style="font-size:14px;color:var(--ink);line-height:1.5"><b>${dowShort[f.dw]} ${hrRange(f.startH,f.endH)}</b> &mdash; you carry about <b>${f.overSum.toFixed(1)} more labor-hours</b> than the ${fmt$(f.sales)} of sales needs.</div><div style="font-size:14px;color:var(--accent-ink);font-weight:700;margin-top:2px">Trimming it &asymp; ${fmt$(f.saveWk)}/week (~${fmt$(f.saveWk*52)}/yr).</div></div></div>`).join('')))+`</div>`;
  h+=`<div class="card" style="padding:14px 16px;margin-bottom:14px;border-left:4px solid var(--brand)"><div style="font-size:14px;color:var(--ink);line-height:1.6"><b>Before you trim:</b> not all labor is cost. Some of it is an investment in growth &mdash; a daypart you're building, or service that turns guests into regulars. Cost-Smart sees the cost; it can't see your strategy. Cut the waste, and protect the bets on purpose.${protCount?` <b style="color:var(--tealdark)">Right now it's protecting ${protCount} investment hour${protCount!==1?'s':''} you've declared &mdash; those won't show as waste.</b>`:''}</div><div class="row" style="margin-top:9px;gap:16px;flex-wrap:wrap"><span onclick="csLearnLabor()" style="color:var(--brand);font-weight:700;cursor:pointer;font-size:14px">&#9788; Learn how to think about labor &rarr;</span><span onclick="csInvest()" style="color:var(--accent-ink);font-weight:700;cursor:pointer;font-size:14px">&#127793; Plan a labor investment &rarr;</span></div></div>`;
  if(ld){ h+=`<div class="row" style="margin-bottom:14px"><button class="btn" style="width:auto;padding:10px 16px;font-size:14px" onclick="csReport()"><i class="ti ti-file-text"></i> Print these as a report for your team</button></div>`; }
  if(ld && wkGap>1){ h+=`<div class="card" style="padding:16px 18px;margin-bottom:14px;background:linear-gradient(135deg,var(--accent-soft),#FFFDF6);border-color:var(--accent-line)"><div class="row" style="align-items:center;gap:12px"><div style="width:46px;height:46px;border-radius:12px;background:linear-gradient(135deg,var(--accent),var(--accent-2));color:#3A2B00;display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0"><i class="ti ti-coin"></i></div><div><div style="font-size:26px;font-weight:800;color:var(--accent-ink)">~${fmt$(wkGap)} a week</div><div style="font-size:14px;color:var(--ink)">of labor is above your ${tgtLabel} &mdash; roughly <b>${fmt$(wkGap*52)} a year</b>. Fix the spots above and that's what you get back.</div></div></div></div>`; }
  h+=`<div class="card" style="padding:14px 16px;margin-bottom:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:10px">What you're aiming for</div><div class="row" style="gap:7px;margin-bottom:11px"><button onclick="csSaveTarget('splh')" style="${_pill(mode==='splh')}">Sales / labor hr</button><button onclick="csSaveTarget('pct')" style="${_pill(mode==='pct')}">Labor % of sales</button><button onclick="csSaveTarget('tplh')" style="${_pill(mode==='tplh')}">Transactions / labor hr</button></div><div class="row" style="align-items:center;gap:8px;flex-wrap:wrap"><span id="csSplhWrap" style="display:${mode==='splh'?'inline-flex':'none'};align-items:center;gap:8px"><span style="font-size:14px;color:var(--muted)">Aim for $</span><input id="csTarget" type="number" min="1" value="${target}" style="width:64px;${_inp}"/><span style="font-size:14px;color:var(--muted)">/ labor hr</span></span><span id="csPctWrap" style="display:${mode==='pct'?'inline-flex':'none'};align-items:center;gap:8px"><span style="font-size:14px;color:var(--muted)">Keep labor at</span><input id="csPct" type="number" min="1" max="60" value="${pctTarget}" style="width:54px;${_inp}"/><span style="font-size:14px;color:var(--muted)">% of sales</span></span><span id="csTplhWrap" style="display:${mode==='tplh'?'inline-flex':'none'};align-items:center;gap:8px"><span style="font-size:14px;color:var(--muted)">Aim for</span><input id="csTplh" type="number" min="1" value="${tplhTarget}" style="width:54px;${_inp}"/><span style="font-size:14px;color:var(--muted)">transactions / labor hr</span></span><span style="font-size:14px;color:var(--muted)">&middot; never below</span><input id="csMin" type="number" min="1" value="${minCov}" style="width:40px;${_inp}"/><span style="font-size:14px;color:var(--muted)">&middot; close with</span><input id="csMinClose" type="number" min="1" value="${minClose}" style="width:40px;${_inp}"/><span style="font-size:14px;color:var(--muted)">&middot; wage $</span><input id="csWage" type="number" min="1" step="0.5" value="${avgWage}" style="width:58px;${_inp}"/><button class="btn pri" style="width:auto;padding:7px 15px;margin-left:auto" onclick="csSaveTarget('${mode}')">Update</button></div><div class="faint" style="font-size:12.5px;margin-top:8px">Amber hours run heavier than your ${tgtLabel}. &ldquo;Smart&rdquo; is the staffing that hits it.</div></div>`;
  if(mode==='tplh' && !tdc){ h+=`<div class="card" style="padding:13px 16px;margin-bottom:14px;background:var(--accent-soft);border-color:var(--accent-line)"><div style="font-size:14px;color:var(--accent-ink);line-height:1.5"><i class="ti ti-clock"></i> Transaction counts for ${dowName} are still loading from Toast — the backfill runs over the next couple hours. Sales-per-labor-hour and Labor-% work fully right now.</div></div>`; }
  ...
  const rows=openHours.map(H=>{ const s=avgSalesH[H], l=avgLaborH[H]; const splh=l>0?s/l:0; const need=idealH[H]; const w=Math.round(s/maxSales*100); const over=l>0&&(l-need)>=0.5; const good=l>0&&!over; const barc=good?'linear-gradient(90deg,var(--tealmid),var(--teallite))':(l>0?'linear-gradient(90deg,#E8A33C,#F0C05A)':'linear-gradient(90deg,var(--tealmid),var(--teallite))'); const splhTxt=l>0?'$'+Math.round(splh):'—'; const splhColor=l>0?(good?'var(--tealdark)':'#B45309'):'var(--muted)';
    return `<div class="row" style="align-items:center;gap:9px;padding:7px 0;border-bottom:1px solid var(--line)"><div style="width:40px;font-size:14px;font-weight:700;color:var(--ink);flex-shrink:0">${hourLbl(H)}</div><div style="flex:1;min-width:0;background:var(--bg);border-radius:8px;height:26px;position:relative;overflow:hidden"><div style="position:absolute;left:0;top:0;bottom:0;width:${w}%;background:${barc};border-radius:8px"></div><span style="position:absolute;left:8px;top:50%;transform:translateY(-50%);font-size:12.5px;font-weight:700;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,.3)">${fmt$(s)}</span></div><div style="width:44px;text-align:right;font-size:12.5px;flex-shrink:0;color:${over?'#B45309':'var(--muted)'};font-weight:${over?'700':'400'}">${l>0?l.toFixed(1):'—'}</div><div style="width:44px;text-align:right;font-size:14px;font-weight:800;color:var(--tealdark);flex-shrink:0">${need}</div><div style="width:44px;text-align:right;font-size:14px;font-weight:800;color:${splhColor};flex-shrink:0">${splhTxt}</div></div>`;
  }).join('');
  h+=`<div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:22px 0 10px">The hour-by-hour detail</div>`;
  ...
  h+=`<div class="card" style="padding:16px 18px"><div class="row" style="align-items:center;margin-bottom:8px;padding-bottom:8px;border-bottom:2px solid var(--line)"><div style="width:40px;font-size:15.5px;font-weight:600;color:var(--ink);text-transform:none;letter-spacing:-.012em">Hour</div><div style="flex:1;font-size:15.5px;font-weight:600;color:var(--ink);text-transform:none;letter-spacing:-.012em">Avg sales</div><div style="width:44px;text-align:right;font-size:15.5px;font-weight:600;letter-spacing:-.012em;color:var(--ink);text-transform:none">Now</div><div style="width:44px;text-align:right;font-size:15.5px;font-weight:600;letter-spacing:-.012em;color:var(--ink);text-transform:none">Smart</div><div style="width:44px;text-align:right;font-size:15.5px;font-weight:600;letter-spacing:-.012em;color:var(--ink);text-transform:none">$/LH</div></div>`+rows+`</div>`;
  if(ld){
    h+=`<div class="card" style="padding:0;margin-top:16px;overflow:hidden"><div style="padding:13px 18px;background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;opacity:.9">${dowName} — your schedule vs cost-smart</div></div><div class="row" style="align-items:stretch;gap:0;border-bottom:1px solid var(--line)"><div style="flex:1;padding:14px 18px;border-right:1px solid var(--line)"><div class="faint" style="font-size:15.5px;text-transform:none;letter-spacing:-.012em;font-weight:600;margin-bottom:6px">Your schedule now</div><div style="font-size:18px;font-weight:800;color:var(--ink)">${totLabor.toFixed(1)}h · ${fmt$(actualCost)}</div><div class="faint" style="font-size:12.5px;margin-top:2px">${actPct.toFixed(1)}% of sales</div></div><div style="flex:1;padding:14px 18px"><div style="font-size:15.5px;text-transform:none;letter-spacing:-.012em;font-weight:600;margin-bottom:6px;color:var(--ink)">Cost-smart to target</div><div style="font-size:18px;font-weight:800;color:var(--tealdark)">${idealPH}h · ${fmt$(idealCost)}</div><div class="faint" style="font-size:12.5px;margin-top:2px">${idealPct.toFixed(1)}% of sales</div></div></div><div style="padding:13px 18px;background:${gap>0?'var(--accent-soft)':'var(--bg)'};display:flex;align-items:center;gap:10px"><i class="ti ${gap>0?'ti-scissors':'ti-circle-check'}" style="font-size:18px;color:${gap>0?'var(--accent-ink)':'var(--tealdark)'};flex-shrink:0"></i><div style="font-size:14px;color:var(--ink);line-height:1.5">${gap>0?`<b>About ${fmt$(gap)} of labor this ${dowName}</b> is above what your ${tgtLabel} needs — mostly the amber hours.`:(gap<0?`This ${dowName} already runs leaner than your ${tgtLabel} — room to add ${Math.abs(gap/avgWage).toFixed(1)}h if service feels stretched.`:`This ${dowName} is right on your target.`)}</div></div></div>`;
  }
  h+=`<div class="faint" style="font-size:12.5px;margin-top:12px">&ldquo;Now&rdquo; = your real avg staffing (from clock-ins); &ldquo;Smart&rdquo; = people to hit your ${tgtLabel}, but never below ${minCov} on the floor and ${minClose} to close — a recommendation, not a rule. Based on ${sd} ${dowName}${sd>1?'s':''}${ld?' + '+ld+' with labor':''} of Toast history.</div>`;
  h+='</div>';
  v.innerHTML=h;
}
...
window.fcGoalInfo=async function(){
  ...
  const accLine = acc ? `<div style="margin-top:12px;padding:10px 12px;background:var(--brand-soft);border-radius:8px;font-size:12.5px;color:var(--ink)">Lately your forecast has landed within about <b>${acc.mape}%</b> of actual (last ${acc.n} days with sales)${acc.within>=50?`, inside 10% on <b>${acc.within}%</b> of them`:''}.</div>` : '';
  const html=`<div style="font-size:14px;line-height:1.65;color:var(--ink)"><div style="font-weight:800;font-size:15.5px;margin-bottom:8px">How the forecast works</div>We blend three things: last year's pattern for this same week, how you are running this year versus last (your last 4 weeks), and last week's trend.<div style="margin-top:10px">It will not nail every day. Big surprises happen, like a sudden 40% day, and that is fine. The goal is to get close, close enough that your first draft schedule is about <b>80%</b> right and you adjust from there.</div><div style="margin-top:10px">Type your own number on any day to override. Yours always wins.</div>${accLine}</div>`;
...
window.csRenderPreview=function(){
  ...
  if(dHrs>0.5){ h+=`<div class="card" style="padding:13px 16px;margin-bottom:14px"><div style="font-size:15.5px;color:var(--ink)">The proposal runs <b>${dHrs.toFixed(1)} fewer labor-hours</b> this week than your current schedule &mdash; the day-by-day cuts are shown below.</div></div>`; }
  ...
  if(p.flags&&p.flags.length){ h+=`<div class="card" style="padding:14px 16px;margin-bottom:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">A look at the proposal</div>`+p.flags.map(f=>`<div style="font-size:14px;color:var(--ink);white-space:pre-line;line-height:1.6;padding:2px 0">${esc(f)}</div>`).join('')+`</div>`; }
  h+=`<div class="sec" style="margin-bottom:8px">The proposed week <span class="faint" style="font-weight:400;font-size:12.5px">&mdash; same grid as your schedule; "was" shows what changed</span></div>`;
  ...
  h+=`<div class="bt bt-team" style="font-weight:600">On the floor</div>`+p.isoDays.map((iso,i)=>{ const pp=new Set(p.draft.filter(s=>s.on_date===iso&&s.person_name!=='__OPEN__').map(s=>s.person_name)).size; const cp=p.curByDay?p.curByDay[i].ppl:null; return `<div class="bt" style="font-weight:600">${pp||'—'}${(cp!=null&&cp!==pp)?`<div style="font-size:11.5px;font-weight:700;color:${pp<cp?'var(--tealdark)':'var(--muted)'}">was ${cp}</div>`:''}</div>`; }).join('');
  h+=`<div class="bt bt-team">Hours</div>`+p.isoDays.map((iso,i)=>{ const ph=p.draft.filter(s=>s.on_date===iso).reduce((a,s)=>a+shiftHours(s),0); const ch=p.curByDay?p.curByDay[i].hrs:null; return `<div class="bt">${ph?ph.toFixed(1)+'h':'—'}${(ch!=null&&Math.abs(ch-ph)>0.4)?`<div style="font-size:11.5px;font-weight:700;color:${ph<ch?'var(--tealdark)':'var(--muted)'}">was ${ch.toFixed(1)}h</div>`:''}</div>`; }).join('')+`</div></div>`;
...
window.csReport=async function(){
  ...
  h+=`<div id="csRep">`;
  h+=`<div style="border-bottom:3px solid var(--tealdark);padding-bottom:12px;margin-bottom:16px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink)">Labor Opportunities</div><div style="font-size:26px;font-weight:800;color:var(--ink);margin-top:2px">${esc(rest)}</div><div style="font-size:14px;color:var(--muted);margin-top:3px">${today} &middot; aiming for ${o.tgtLabel}</div></div>`;
  h+=`<div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">Where we are &mdash; an average week</div><div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(128px,1fr));gap:10px;margin-bottom:20px">`;
  const tile=(l,val)=>`<div style="border:1px solid var(--line);border-radius:12px;padding:11px 13px"><div style="font-size:15.5px;text-transform:none;letter-spacing:-.012em;color:var(--ink);font-weight:600;margin-bottom:5px">${l}</div><div style="font-size:18px;font-weight:800;color:var(--ink)">${val}</div></div>`;
  ...
  h+=`<div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:10px">Where we can tighten &mdash; start at the top</div>`;
  if(!o.hasLabor){ h+=`<div class="faint" style="font-size:14px">Labor history is still loading &mdash; opportunities show once it’s in.</div>`; }
  ...
  else { h+=top.map((f,i)=>`<div style="display:flex;gap:13px;align-items:flex-start;padding:11px 0;border-bottom:1px solid var(--line)"><div style="width:28px;height:28px;border-radius:50%;background:var(--accent-soft);color:var(--accent-ink);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;flex-shrink:0">${i+1}</div><div style="flex:1;min-width:0"><div style="font-size:15.5px;font-weight:700;color:var(--ink)">${o.dowShort[f.dw]} ${o.hrRange(f.startH,f.endH)}</div><div style="font-size:14px;color:var(--muted);margin-top:2px">About ${f.overSum.toFixed(1)} more labor-hours than the ${M$(f.sales)} of sales needs &mdash; room to trim a shift or send someone a little early.</div></div><div style="text-align:right;flex-shrink:0"><div style="font-size:15.5px;font-weight:800;color:var(--accent-ink)">${M$(f.saveWk)}</div><div style="font-size:12.5px;color:var(--faint);font-weight:700">per week</div></div></div>`).join(''); }
  if(o.hasLabor && top.length){ h+=`<div style="margin-top:16px;padding:14px 16px;border:2px solid var(--accent-line);border-radius:12px;background:var(--accent-soft)"><div style="font-size:15.5px;color:var(--ink)">Working through all of these is about <b style="color:var(--accent-ink)">${M$(o.totalSaveWk)} a week</b> in labor. We don’t do it all at once &mdash; pick the top one, trim gently, watch a week, then take the next.</div></div>`; }
  h+=`<div style="margin-top:20px;padding-top:14px;border-top:1px solid var(--line)"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">How we’ll work this together</div><div style="font-size:14px;color:var(--ink);line-height:1.7">Start with #1. Trim one shift or shorten it &mdash; don’t cut it all at once. Watch the next week: did service hold? If yes, take the next one. Protect the rush and the guest experience above the number. And remember &mdash; not every extra person is waste. Some labor is a bet on growth; cut the waste and protect the bets on purpose. This is about growing into it as a team, not slashing.</div></div>`;
  h+=`<div style="margin-top:18px;font-size:12.5px;color:var(--faint)">Generated by Cost-Smart &middot; ${today} &middot; Based on your real sales and clock-in history.</div></div>`;
  v.innerHTML=h; window.scrollTo(0,0);
};
window.csLearnLabor=function(){
  ...
  const hd=t=>`<div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:22px 0 10px">${t}</div>`;
  ...
  h+=`<div style="background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff;border-radius:12px;padding:20px 22px;margin-bottom:18px"><div style="font-size:12.5px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:6px">A short teaching</div><div style="font-size:26px;font-weight:800">The two sides of labor</div><div style="font-size:14px;opacity:.9;margin-top:5px">Why the leanest schedule isn't always the best one.</div></div>`;
...
window.csInvest=async function(){
  ...
  let h=`<div class="row" style="gap:10px;margin-bottom:14px"><button class="btn" style="width:auto;padding:9px 16px" onclick="go('costsmart')"><i class="ti ti-arrow-left"></i> Back to Cost-Smart</button></div><div style="max-width:720px">`;
  h+=`<div style="background:linear-gradient(135deg,var(--accent-2),var(--accent));color:#3A2B00;border-radius:12px;padding:20px 22px;margin-bottom:16px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:6px">The other side of labor</div><div style="font-size:26px;font-weight:800">Plan a labor investment</div><div style="font-size:14px;margin-top:5px;color:#5A4600">Not cutting cost &mdash; spending it on purpose, to grow.</div></div>`;
  ...
  if(bets.length){ h+=`<div class="sec" style="margin-bottom:8px">Your open bets</div>`; bets.forEach(b=>{ const d=b.d; const dc=(d.days||[]).length; const cw=(+d.people||1)*(+d.hours||0)*dc*(+d.wage||wage); const clkb=t=>{ if(!t)return''; let H=+t.slice(0,2); const m=t.slice(3,5); const ap=H<12?'a':'p'; let h12=H%12||12; return h12+(m==='00'?'':':'+m)+ap; }; h+=`<div class="card" style="padding:13px 15px;margin-bottom:10px"><div class="row" style="align-items:flex-start"><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:15.5px;color:var(--ink)">${esc(d.what||'Labor investment')}</div><div style="font-size:12.5px;color:var(--muted);margin-top:2px">${+d.people||1} person &middot; ${clkb(d.start)}&ndash;${clkb(d.end)} &middot; ${dc} day${dc!==1?'s':''}/wk &middot; ${M$(cw)}/week</div>${d.role?`<div style="font-size:14px;color:var(--ink);margin-top:5px"><b>The job:</b> ${esc(d.role)}</div>`:''}${d.belief?`<div style="font-size:14px;color:var(--ink);margin-top:3px"><b>The bet:</b> ${esc(d.belief)}</div>`:''}<div style="font-size:12.5px;color:var(--faint);margin-top:5px">Started ${d.created?new Date(d.created).toLocaleDateString():''} &middot; give it three months, check monthly</div></div><span onclick="csInvestDel('${b.id}')" style="cursor:pointer;color:var(--muted);font-size:15.5px;flex-shrink:0" title="Remove"><i class="ti ti-trash"></i></span></div></div>`; }); }
  ...
  h+=`<label style="font-size:12.5px;font-weight:700;color:var(--muted)">On which days?</label><div class="row" style="gap:6px;margin:6px 0 12px;flex-wrap:wrap">`+['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((L,i)=>`<label style="display:inline-flex;align-items:center;gap:5px;padding:6px 10px;border:1px solid var(--line2);border-radius:8px;font-size:14px;cursor:pointer;color:var(--ink)"><input type="checkbox" class="ivDay" data-jsdow="${(i+1)%7}" ${i<5?'checked':''} onchange="csInvestCalc()" style="width:15px;height:15px"/>${L}</label>`).join('')+`</div>`;
...
window.csInvestCalc=function(){
  ...
  el.innerHTML=`<div style="background:var(--accent-soft);border:1px solid var(--accent-line);border-radius:12px;padding:14px 16px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">What it costs</div><div class="row" style="gap:18px;flex-wrap:wrap"><div><div style="font-size:18px;font-weight:800;color:var(--ink)">${M$(cd)}</div><div style="font-size:12.5px;color:var(--muted)">per day</div></div><div><div style="font-size:18px;font-weight:800;color:var(--ink)">${M$(cw)}</div><div style="font-size:12.5px;color:var(--muted)">per week</div></div><div><div style="font-size:18px;font-weight:800;color:var(--ink)">${M$(cm)}</div><div style="font-size:12.5px;color:var(--muted)">per month</div></div><div><div style="font-size:18px;font-weight:800;color:var(--accent-ink)">${M$(c12)}</div><div style="font-size:12.5px;color:var(--muted)">a 12-week test</div></div></div>${guests?`<div style="font-size:14px;color:var(--ink);margin-top:10px;border-top:1px solid var(--accent-line);padding-top:9px">To break even it needs to add more than <b>${M$(cw)}/week</b> &mdash; about <b>${guests} more guest${guests>1?'s':''} a week</b> at your ~${M$(inv.avgTicket)} average ticket (more once you count product cost). Grow it past that and you're winning.</div>`:''}</div>`;
};
