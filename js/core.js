...
async function vWhiteboard(v){
  ...
  const wnote=(label,text)=>`<div style="flex:1;min-width:210px;background:#FBF9F5;border:1px solid #EBE5DB;border-radius:12px;padding:15px 17px;box-shadow:0 3px 12px rgba(60,50,30,.07);position:relative"><div style="position:absolute;top:-6px;left:20px;width:12px;height:12px;border-radius:50%;background:var(--brand);box-shadow:0 2px 4px rgba(0,0,0,.3)"></div><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:6px">${label}</div><div style="font-size:16.5px;line-height:1.45;font-weight:600;color:var(--ink)">${esc(text)}</div></div>`;
  let h='';
  if(mtext||vtext||isAdmin){
    if(mtext||vtext){
      h+=`<div class="phero" style="padding:26px 28px;margin-bottom:20px">${heroTile()}<div class="phero-in"><div class="row" style="align-items:center;margin-bottom:18px"><span style="font-size:11px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent)">Our Why</span>${isAdmin?`<button onclick="toggleWhyEdit()" style="margin-left:auto;border:1px solid rgba(255,255,255,.55);background:rgba(255,255,255,.15);color:#fff;border-radius:9px;padding:6px 12px;font-size:12.5px;cursor:pointer;font-family:inherit"><i class="ti ti-pencil"></i> Edit</button>`:''}</div>${mtext?`<div style="margin-bottom:${vtext?'20px':'0'}"><div style="font-size:11px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:7px">Mission</div><div class="mvtext" style="font-size:23px;font-weight:800;line-height:1.3;color:#fff">${esc(mtext)}</div></div>`:''}${vtext?`<div><div style="font-size:11px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:7px">Vision</div><div class="mvtext" style="font-size:23px;font-weight:800;line-height:1.3;color:#fff">${esc(vtext)}</div></div>`:''}</div></div>`;
    } else if(isAdmin){
      h+=`<div class="card" style="padding:16px 18px;margin-bottom:18px"><div class="row" style="align-items:center;margin-bottom:8px"><span style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink)">Our Why</span><button class="btn" style="width:auto;margin-left:auto;padding:5px 12px;font-size:12.5px" onclick="toggleWhyEdit()"><i class="ti ti-pencil"></i> Edit</button></div><div class="faint" style="font-size:13px">Add your mission and vision — they anchor the whole team, front and center.</div></div>`;
    }
  ...
  if(ops){
    ...
    if(_c){ h+=`<div class="sec">Continue your training</div><div class="card" style="padding:18px 20px;display:flex;align-items:center;gap:16px;margin-bottom:6px"><div style="width:46px;height:46px;border-radius:12px;background:var(--brand);color:#fff;display:flex;align-items:center;justify-content:center;font-size:21px;flex-shrink:0"><i class="ti ti-player-play"></i></div><div style="flex:1;min-width:0"><div class="faint" style="font-size:15px;text-transform:none;letter-spacing:-.012em;font-weight:600">${esc(_c.t.name)}</div><div style="font-weight:600;font-size:16px;margin:3px 0 10px">${esc(_c.l.title)}</div><div class="bar"><i style="width:${trackPct(_c.t.id)}%"></i></div></div><button class="btn pri" style="width:auto;gap:6px" onclick="go('lesson',{tid:'${_c.t.id}',lid:'${_c.l.id}'})">Resume <i class="ti ti-arrow-right"></i></button></div>`; }
...
window.showWhyMoment=function(mtext,vtext,force){
  ...
  m.innerHTML=_tile
    ...
    +'<div style="font-size:11px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:7px">&#10022; Every shift</div><div style="color:#DCEEF1;font-size:14.5px;margin-bottom:40px">'+esc(dstr)+'</div>'
...
async function vCostSmart(v){
  ...
  h+=`<div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:22px 0 10px">The hour-by-hour detail</div>`;
  ...
  h+=`<div class="card" style="padding:16px 18px"><div class="row" style="align-items:center;margin-bottom:8px;padding-bottom:8px;border-bottom:2px solid var(--line)"><div style="width:40px;font-size:15px;font-weight:600;color:var(--ink);text-transform:none;letter-spacing:-.012em">Hour</div><div style="flex:1;font-size:15px;font-weight:600;color:var(--ink);text-transform:none;letter-spacing:-.012em">Avg sales</div><div style="width:44px;text-align:right;font-size:15px;font-weight:600;letter-spacing:-.012em;color:var(--ink);text-transform:none">Now</div><div style="width:44px;text-align:right;font-size:15px;font-weight:600;letter-spacing:-.012em;color:var(--ink);text-transform:none">Smart</div><div style="width:44px;text-align:right;font-size:15px;font-weight:600;letter-spacing:-.012em;color:var(--ink);text-transform:none">$/LH</div></div>`+rows+`</div>`;
  if(ld){
    h+=`<div class="card" style="padding:0;margin-top:16px;overflow:hidden"><div style="padding:13px 18px;background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff"><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;opacity:.9">${dowName} — your schedule vs cost-smart</div></div><div class="row" style="align-items:stretch;gap:0;border-bottom:1px solid var(--line)"><div style="flex:1;padding:14px 18px;border-right:1px solid var(--line)"><div class="faint" style="font-size:15px;text-transform:none;letter-spacing:-.012em;font-weight:600;margin-bottom:6px">Your schedule now</div><div style="font-size:21px;font-weight:800;color:var(--ink)">${totLabor.toFixed(1)}h · ${fmt$(actualCost)}</div><div class="faint" style="font-size:12.5px;margin-top:2px">${actPct.toFixed(1)}% of sales</div></div><div style="flex:1;padding:14px 18px"><div style="font-size:15px;text-transform:none;letter-spacing:-.012em;font-weight:600;margin-bottom:6px;color:var(--ink)">Cost-smart to target</div><div style="font-size:21px;font-weight:800;color:var(--tealdark)">${idealPH}h · ${fmt$(idealCost)}</div><div class="faint" style="font-size:12.5px;margin-top:2px">${idealPct.toFixed(1)}% of sales</div></div></div><div style="padding:13px 18px;background:${gap>0?'var(--accent-soft)':'var(--bg)'};display:flex;align-items:center;gap:10px"><i class="ti ${gap>0?'ti-scissors':'ti-circle-check'}" style="font-size:20px;color:${gap>0?'var(--accent-ink)':'var(--tealdark)'};flex-shrink:0"></i><div style="font-size:14px;color:var(--ink);line-height:1.5">${gap>0?`<b>About ${fmt$(gap)} of labor this ${dowName}</b> is above what your ${tgtLabel} needs — mostly the amber hours.`:(gap<0?`This ${dowName} already runs leaner than your ${tgtLabel} — room to add ${Math.abs(gap/avgWage).toFixed(1)}h if service feels stretched.`:`This ${dowName} is right on your target.`)}</div></div></div>`;
  }
...
window.csRenderPreview=function(){
  ...
  let h=`<div style="background:linear-gradient(135deg,var(--tealmid),var(--tealdark));color:#fff;border-radius:16px;padding:18px 20px;margin-bottom:16px"><div style="font-size:11px;font-weight:640;letter-spacing:.06em;text-transform:uppercase;color:var(--accent);margin-bottom:6px">&#10022; Cost-Smart proposal</div><div style="font-size:24px;font-weight:800">Week of ${wkLab}</div><div style="font-size:13.5px;opacity:.9;margin-top:4px">Nothing has changed yet. Look it over, then apply it or throw it away.</div></div>`;
  const fb=window._csFcBasis; if(fb){ const yoyPct=Math.round((fb.yoy-1)*100); const ndgPct=Math.round((fb.nudge-1)*100); const parts=[fb.haveLY?"last year's shape for this week":'your recent same-day history']; if(fb.haveLY) parts.push('scaled to this year ('+(yoyPct>=0?'+':'')+yoyPct+'% vs last year)'); parts.push(Math.abs(ndgPct)<1?'last week held steady':('nudged '+(ndgPct>0?'up':'down')+' '+Math.abs(ndgPct)+'% for last week')); h+='<div class="card" style="padding:12px 15px;margin-bottom:14px;border-left:4px solid var(--tealmid)"><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:5px">Built to your forecast</div><div style="font-size:13px;color:var(--ink);line-height:1.55">'+parts.join(' &middot; ')+'.</div></div>'; }
  const col=(lbl,o,accent)=>`<div style="flex:1;padding:14px 16px${accent?';background:var(--brand-soft)':''}"><div style="font-size:15px;text-transform:none;letter-spacing:-.012em;font-weight:600;color:${accent?'var(--brand)':'var(--faint)'};margin-bottom:8px">${lbl}</div><div style="font-size:22px;font-weight:800;color:var(--ink)">${o.hrs?o.hrs.toFixed(1):'0'}h</div><div style="font-size:13px;color:var(--muted);margin-top:2px">${o.cost?M$(o.cost):'&mdash;'}${o.pct!=null?' &middot; '+o.pct.toFixed(1)+'% of sales':''}</div></div>`;
  ...
  if(p.flags&&p.flags.length){ h+=`<div class="card" style="padding:14px 16px;margin-bottom:14px"><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">A look at the proposal</div>`+p.flags.map(f=>`<div style="font-size:13px;color:var(--ink);white-space:pre-line;line-height:1.6;padding:2px 0">${esc(f)}</div>`).join('')+`</div>`; }
...
window.csReport=async function(){
  ...
  h+=`<div id="csRep">`;
  h+=`<div style="border-bottom:3px solid var(--tealdark);padding-bottom:12px;margin-bottom:16px"><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink)">Labor Opportunities</div><div style="font-size:26px;font-weight:800;color:var(--ink);margin-top:2px">${esc(rest)}</div><div style="font-size:13px;color:var(--muted);margin-top:3px">${today} &middot; aiming for ${o.tgtLabel}</div></div>`;
  h+=`<div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">Where we are &mdash; an average week</div><div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(128px,1fr));gap:10px;margin-bottom:20px">`;
  const tile=(l,val)=>`<div style="border:1px solid var(--line);border-radius:10px;padding:11px 13px"><div style="font-size:15px;text-transform:none;letter-spacing:-.012em;color:var(--ink);font-weight:600;margin-bottom:5px">${l}</div><div style="font-size:19px;font-weight:800;color:var(--ink)">${val}</div></div>`;
  ...
  h+=`<div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:10px">Where we can tighten &mdash; start at the top</div>`;
...
window.csLearnLabor=function(){
  ...
  const hd=t=>`<div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:22px 0 10px">${t}</div>`;
...
window.csInvest=async function(){
  ...
  let h=`<div class="row" style="gap:10px;margin-bottom:14px"><button class="btn" style="width:auto;padding:9px 16px" onclick="go('costsmart')"><i class="ti ti-arrow-left"></i> Back to Cost-Smart</button></div><div style="max-width:720px">`;
  h+=`<div style="background:linear-gradient(135deg,var(--accent-2),var(--accent));color:#3A2B00;border-radius:16px;padding:20px 22px;margin-bottom:16px"><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:6px">The other side of labor</div><div style="font-size:26px;font-weight:800">Plan a labor investment</div><div style="font-size:14px;margin-top:5px;color:#5A4600">Not cutting cost &mdash; spending it on purpose, to grow.</div></div>`;
