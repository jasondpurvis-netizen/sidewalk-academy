...
window.openArchive=function(name,editId){
  ...
  m.innerHTML=`<div style="background:var(--card);width:100%;max-width:460px;border-radius:12px 16px 0 0;padding:20px 20px 28px;box-shadow:0 -8px 40px rgba(0,0,0,.22)"><div style="font-weight:700;font-size:15.5px">${cur?'Edit record':'No longer employed'}</div><div class="faint" style="font-size:12.5px;margin-bottom:15px">${esc(name)} · drops off active views — history stays on record</div><label style="font-size:12.5px;color:var(--muted)">Reason for leaving</label><select id="arReason" style="width:100%;padding:11px;margin:4px 0 12px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${reasons.map(r=>`<option${d.reason===r?' selected':''}>${r}</option>`).join('')}</select><label style="font-size:12.5px;color:var(--muted)">Rehire status</label><select id="arRehire" style="width:100%;padding:11px;margin:4px 0 12px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${rehires.map(o=>`<option value="${o[0]}"${d.rehire===o[0]?' selected':''}>${o[1]}</option>`).join('')}</select><label style="font-size:12.5px;color:var(--muted)">Notes — why they left, anything a future you should know</label><textarea id="arNotes" placeholder="e.g. Great closer, left for school. Would take back in a heartbeat." style="width:100%;min-height:84px;padding:11px;margin:4px 0 2px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px;line-height:1.5">${esc(d.notes||'')}</textarea><div class="faint" style="font-size:12.5px;margin:14px 0 2px;line-height:1.5">Their upcoming shifts come off the schedule automatically. Past shifts stay on the record.</div><div class="row" style="gap:8px;margin-top:14px"><button class="btn" style="width:auto;margin-left:auto" onclick="document.getElementById('archm').remove()">Cancel</button><button class="btn pri" style="width:auto" onclick='doArchive(${JSON.stringify(name)},${editId!=null?editId:'null'})'>${cur?'Save':'Archive'}</button></div></div>`;
...
window.openProfile=function(name){
  ...
  const stationBoxes=STATIONS.map(s=>`<label style="display:inline-flex;align-items:center;gap:7px;font-size:14px;background:var(--bg);border:1px solid var(--line2);border-radius:8px;padding:7px 11px;cursor:pointer"><input type="checkbox" class="prRoleCb" value="${esc(s)}" ${myRoles.includes(s)?'checked':''}/> ${esc(s)}</label>`).join('');
  const F=(id,lbl,type,val,ph)=>`<div style="flex:1;min-width:120px"><label style="font-size:12.5px;color:var(--muted)">${lbl}</label><input id="${id}" type="${type||'text'}" value="${val?esc(String(val)):''}" placeholder="${ph||''}" style="width:100%;padding:9px 10px;margin-top:3px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px"/></div>`;
  ...
  m.innerHTML=`<div style="background:var(--card);width:100%;max-width:520px;border-radius:12px 16px 0 0;padding:20px 20px 26px;box-shadow:0 -8px 40px rgba(0,0,0,.22);max-height:88vh;overflow:auto"><div style="font-weight:700;font-size:18px">${esc(name)}</div><div class="faint" style="font-size:12.5px;margin-bottom:14px">Profile — kept on record</div><label style="font-size:12.5px;color:var(--muted)">Position</label><select id="prPos" style="width:100%;padding:10px;margin:3px 0 6px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${POS_PICK.map(o=>`<option${o===pos?' selected':''}>${o}</option>`).join('')}</select><div class="sec" style="margin-top:6px">Dates</div><div class="row" style="gap:10px;flex-wrap:wrap">${F('prBday','Birthday','date',p.bday?('2000-'+p.bday):'')}${F('prHired','Hire date','date',p.hired||'')}</div><div class="sec">Contact</div><div class="row" style="gap:10px;flex-wrap:wrap">${F('prMobile','Mobile phone','tel',p.mobile,'(555) 555-5555')}${F('prHome','Home phone','tel',p.home)}</div><div class="row" style="gap:10px;flex-wrap:wrap;margin-top:8px">${F('prAddr','Address','text',p.address)}</div><div class="row" style="gap:10px;flex-wrap:wrap;margin-top:8px">${F('prCity','City','text',p.city)}${F('prState','State','text',p.state)}${F('prZip','Zip','text',p.zip)}</div><div class="sec">Emergency contact</div><div class="row" style="gap:10px;flex-wrap:wrap">${F('prEcName','Name','text',p.ec_name)}${F('prEcPhone','Phone','tel',p.ec_phone)}</div><div class="sec">Pay</div><div class="faint" style="font-size:12.5px;margin:-4px 0 8px;line-height:1.55">Pay is set in one place so two screens can never disagree. ${_payLineFor(name)}</div>${canSee('pay')?`<div class="row" style="gap:10px"><button type="button" class="btn" style="width:auto" onclick="var m=document.getElementById('profm');if(m)m.remove();go('pay')"><i class="ti ti-coin"></i> Open pay rates</button></div>`:''}<div class="sec">Scheduling rules</div><div class="faint" style="font-size:12.5px;margin:-4px 0 8px">The stuff a good manager keeps in their head. The auto-draft follows every rule you set here. Leave blank for none.</div><div style="margin-bottom:10px"><label style="font-size:12.5px;color:var(--muted)">Scheduling priority — who gets their hours first</label><select id="prPriority" style="width:100%;padding:9px 10px;margin-top:3px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px">${[['1','1 · First — protect their hours'],['2','2 · High'],['3','3 · Normal'],['4','4 · Backup — fills gaps last']].map(o=>`<option value="${o[0]}"${String(p.priority||(pos==='Trainee'?'4':'3'))===o[0]?' selected':''}>${o[1]}</option>`).join('')}</select></div><div style="margin-bottom:10px"><label style="font-size:12.5px;color:var(--muted)">Time of day they fit best — your call, not theirs</label><select id="prTimePref" style="width:100%;padding:9px 10px;margin-top:3px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px">${[['','No preference'],['early','Better in the mornings — lean early'],['late','Better later — lean closing']].map(o=>`<option value="${o[0]}"${(p.timePref||'')===o[0]?' selected':''}>${o[1]}</option>`).join('')}</select></div><div class="row" style="gap:10px;flex-wrap:wrap">${F('prOffRun','Days off in a row','number',p.offRun,'e.g. 2')}${F('prMaxDays','Max days / week','number',p.maxDays,'e.g. 5')}${F('prMaxRun','Most days in a row','number',p.maxRun,'e.g. 2')}${F('prMaxShift','Longest shift (hrs)','number',p.maxShift,'e.g. 8')}</div><div class="row" style="gap:10px;flex-wrap:wrap;margin-top:8px">${F('prMinHrs','Min hours / week','number',p.minHrs,'e.g. 36')}${F('prMaxHrs','Max hours / week','number',p.maxHrs,'e.g. 40')}</div><div style="margin-top:10px"><label style="font-size:12.5px;color:var(--muted)">Days off — the draft will never schedule these</label><div class="faint" style="font-size:12.5px;margin:2px 0 7px">Hard rule. The draft will not put them on these days for any reason. You can still add a shift by hand if they agree to cover.</div><div style="display:flex;flex-wrap:wrap;gap:6px">${dayOffBoxes}</div></div>${myRoles.length?`<div style="margin-top:12px"><label style="font-size:12.5px;color:var(--muted)">Skill day limits <span class="faint" style="font-weight:400">— trained, but not on certain days</span></label><div class="faint" style="font-size:12.5px;margin:2px 0 8px">Check the days to keep them <b>off</b> a station (e.g. off the bar on busy Saturdays). The draft still uses them elsewhere those days.</div>`+myRoles.map(st=>{ const ex=(p.skillDays&&Array.isArray(p.skillDays[st]))?p.skillDays[st]:[]; return `<div style="margin-bottom:8px"><div style="font-size:12.5px;font-weight:600;margin-bottom:4px">${esc(st)}</div><div style="display:flex;flex-wrap:wrap;gap:5px">`+DOW.map((d,i)=>`<label style="display:inline-flex;align-items:center;gap:5px;font-size:12.5px;background:var(--bg);border:1px solid var(--line2);border-radius:8px;padding:5px 8px;cursor:pointer"><input type="checkbox" class="prSkillDay" data-st="${esc(st)}" value="${i}" ${ex.indexOf(i)>=0?'checked':''}/> ${d}</label>`).join('')+`</div></div>`; }).join('')+`</div>`:''}<div class="faint" style="font-size:12.5px;margin-top:12px;padding:9px 11px;background:var(--bg);border-radius:8px">Stations, skill level, and can open / can close are all set on the <b style="color:var(--brand);cursor:pointer" onclick="document.getElementById('prm').remove();state.ctx.ttab='skills';go('team')">Team → Skills</b> grid — one place to see and set everyone at once.</div><div class="faint" style="font-size:12.5px;margin-top:10px;padding:8px 10px;background:var(--bg);border-radius:8px">Minor status is detected automatically from their birthday — hour caps and curfew limits then apply on their own.</div>${canCorract()?`<div style="margin-top:14px"><button class="btn" style="width:100%;justify-content:center;gap:7px" onclick='document.getElementById("prm").remove();openCorract(${JSON.stringify(name)})'><i class="ti ti-flag"></i> Corrective actions</button></div>`:''}<div class="row" style="gap:8px;margin-top:18px;flex-wrap:wrap"><button class="btn" style="width:auto;color:#A32D2D;border-color:#F0C9C9" onclick='document.getElementById("prm").remove();openArchive(${JSON.stringify(name)})'>No longer employed</button><button class="btn" style="width:auto;margin-left:auto" onclick="prCancel()">Cancel</button><button class="btn pri" style="width:auto" onclick='doProfile(${JSON.stringify(name)})'>Save</button></div></div>`;
...
/* The profile used to carry its own wage box that nothing in the app ever read -- nineteen
   people had a rate typed into it and none of it reached labour cost. Rather than a second
   input, show what the pay screen actually holds, fetched fresh so the two can never drift. */
window._payLineFor=function(name){
  const id='payln-'+Math.random().toString(36).slice(2,9);
  setTimeout(async function(){
    const el=document.getElementById(id); if(!el) return;
    try{
      const pr=profileOf(name)||{};
      if(+pr.salary>0){ el.textContent='Currently salaried at $'+Number(pr.salary).toLocaleString()+' a year.'; return; }
      const r=await sb.from('pay_rates').select('wage').eq('person_name',name).maybeSingle();
      const w=r.data&&+r.data.wage;
      el.textContent = w>0 ? ('Currently $'+w.toFixed(2)+' an hour.') : 'No rate set yet.';
    }catch(e){ el.textContent=''; }
  },0);
  return '<span id="'+id+'"></span>';
};
window.doProfile=async function(name){ const g=id=>((document.getElementById(id)||{}).value||'').trim(); const b=g('prBday'); const prev=profileOf(name)||{}; /* stations, skill levels and caps are edited on the Skills grid — preserve them here */ const detail=JSON.stringify(Object.assign({},prev,{bday:b?b.slice(5):'',hired:g('prHired'),mobile:g('prMobile'),home:g('prHome'),address:g('prAddr'),city:g('prCity'),state:g('prState'),zip:g('prZip'),ec_name:g('prEcName'),ec_phone:g('prEcPhone'),minHrs:g('prMinHrs'),maxHrs:g('prMaxHrs'),offRun:g('prOffRun'),maxDays:g('prMaxDays'),maxRun:g('prMaxRun'),maxShift:g('prMaxShift'),priority:g('prPriority'),timePref:((document.getElementById('prTimePref')||{}).value||''),daysOff:[...document.querySelectorAll('.prDayOff')].filter(c=>c.checked).map(c=>+c.value),skillDays:(function(){const o={};document.querySelectorAll('.prSkillDay').forEach(c=>{if(c.checked){const st=c.getAttribute('data-st');(o[st]=o[st]||[]).push(+c.value);}});return o;})()})); await sb.from('day_items').delete().eq('kind','profile').eq('title',name); await sb.from('day_items').insert({kind:'profile',title:name,on_date:null,detail,created_by:state.user.id}); const posv=(document.getElementById('prPos')||{}).value; if(posv) await setPos(name,posv); await loadProfiles(); const m=document.getElementById('prm'); if(m)m.remove(); _reRenderPeople(); };
...
/* Writing a write-up and reading one are different jobs, and Jason wants each restaurant
   to set them separately: a supervisor may be the one who documents a late shift while
   only managers and up can read the file. Neither is hard-wired to a rank any more. */
function canCoractWrite(){ return (state.profile&&state.profile.role==='admin') || (typeof myRank==='function' && myRank()>=permOf('coract_write')); }
function canCoractView(){  return (state.profile&&state.profile.role==='admin') || (typeof myRank==='function' && myRank()>=permOf('coract_view')); }
/* the tab appears for anyone who can do either */
function canCorract(){ return canCoractWrite() || canCoractView(); }
...
window.openCorract=async function(name){
  if(!canCorract()){ alert('Write-ups are limited by your restaurant\u2019s settings.'); return; }
  const entries=await loadCoract(name); await loadPositions(); const pos=posOf(name);
  const flags=coractFlags(entries);
  /* Somebody who may write a write-up but not read the file sees the form and nothing
     else -- showing them the history would make the read setting decorative. */
  const _canRead=canCoractView(), _canWrite=canCoractWrite();
  const list = !_canRead
    ? `<div class="faint" style="font-size:13.5px;padding:10px 0">Past entries are not visible to you.</div>`
    : entries.length ? entries.map(e=>{ const t=coractType(e.type), l=coractLevel(e.level); return `<div style="border:1px solid var(--line2);border-radius:12px;padding:11px 13px;margin-bottom:9px">${e.doc?`<a href="${esc(e.doc)}" target="_blank" rel="noopener" download style="display:inline-flex;align-items:center;gap:6px;float:right;font-size:12.5px;color:var(--brand);text-decoration:none;border:1px solid var(--brand-line);border-radius:8px;padding:3px 9px"><i class="ti ti-file-download"></i>Signed copy</a>`:''}<div class="row" style="gap:7px;align-items:center;flex-wrap:wrap"><span style="font-size:12.5px;font-weight:640;text-transform:uppercase;letter-spacing:.06em;color:#fff;background:${t[2]};border-radius:8px;padding:2px 8px">${esc(t[1])}</span><span style="font-size:11.5px;font-weight:700;color:${l[2]};border:1px solid ${l[2]};border-radius:8px;padding:2px 8px">${esc(l[1])}</span><span class="faint" style="font-size:12.5px">${esc(fmtDay(_d(e.on_date)))}</span><button onclick='coractDel(${e.id},${JSON.stringify(name)})' title="Delete" style="margin-left:auto;border:none;background:none;color:var(--muted);cursor:pointer;font-size:14px"><i class="ti ti-trash"></i></button></div><div style="font-size:14px;line-height:1.5;margin-top:7px;white-space:pre-wrap">${esc(e.note)}</div>${e.byName?`<div class="faint" style="font-size:12.5px;margin-top:6px">Logged by ${esc(e.byName)}</div>`:''}</div>`; }).join('') : `<div class="faint" style="font-size:14px;text-align:center;padding:14px">No entries yet.</div>`;
  ...
  m.innerHTML=`<div style="background:var(--card);width:100%;max-width:560px;border-radius:12px 16px 0 0;padding:20px 20px 26px;box-shadow:0 -8px 40px rgba(0,0,0,.22);max-height:90vh;overflow:auto"><div class="row" style="align-items:flex-start;gap:10px"><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:18px">${esc(name)}</div><div class="faint" style="font-size:12.5px">Corrective actions &middot; ${esc(pos)} &middot; ${_canRead?'confidential':'you can add, not read'}</div></div><button class="btn" style="width:auto;padding:6px 12px;flex:none" onclick='printCoract(${JSON.stringify(name)})'><i class="ti ti-printer"></i> Print record</button></div>${flags.length?`<div style="margin-top:12px;background:#FCEBEB;border:1px solid #F0C9C9;color:#8A1C1C;border-radius:8px;padding:9px 12px;font-size:12.5px;font-weight:600"><i class="ti ti-flag"></i> ${flags.map(esc).join(' &middot; ')}</div>`:''}<div class="card" style="padding:13px 14px;margin-top:14px;background:var(--bg)"><div style="font-weight:700;font-size:14px;margin-bottom:9px">Log an entry</div><div class="row" style="gap:8px;flex-wrap:wrap"><input id="caDate" type="date" value="${isoDate(new Date())}" style="padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px"/><select id="caType" style="padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px">${CORACT_TYPES.map(o=>`<option value="${o[0]}">${o[1]}</option>`).join('')}</select><select id="caLevel" style="padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px">${CORACT_LEVELS.map(o=>`<option value="${o[0]}">${o[1]}</option>`).join('')}</select></div><textarea id="caNote" placeholder="What happened — be specific and factual: what occurred, the impact, and any expectation you set." style="width:100%;min-height:80px;margin-top:8px;padding:10px;border:1px solid var(--line2);border-radius:8px;font-family:inherit;font-size:14px;line-height:1.5;color:var(--ink);background:var(--card);box-sizing:border-box"></textarea><div class="row" style="margin-top:9px;gap:10px;align-items:center;flex-wrap:wrap;display:${_canWrite?'flex':'none'}"><label style="display:inline-flex;align-items:center;gap:7px;font-size:13.5px;color:var(--ink2);cursor:pointer;border:1px dashed var(--line2);border-radius:9px;padding:7px 11px"><i class="ti ti-paperclip"></i><span id="caFileName">Attach the signed copy</span><input id="caFile" type="file" accept="image/*,.pdf" onchange="caFilePicked(this)" style="display:none"/></label><button class="btn pri" style="width:auto;margin-left:auto" onclick='coractAdd(${JSON.stringify(name)})'>Add entry</button></div></div><div class="sec" style="margin-top:16px">History <span class="faint" style="font-weight:400">&middot; ${entries.length}</span></div>${list}<div class="row" style="margin-top:16px"><button class="btn" style="width:auto;margin-left:auto" onclick="document.getElementById('cam').remove()">Close</button></div></div>`;
  document.body.appendChild(m); m.addEventListener('click',e=>{ if(e.target===m) m.remove(); });
};
window.coractAdd=async function(name){ if(!canCoractWrite())return; const d=(document.getElementById('caDate')||{}).value||isoDate(new Date()); const type=(document.getElementById('caType')||{}).value||'other'; const level=(document.getElementById('caLevel')||{}).value||'note'; const note=((document.getElementById('caNote')||{}).value||'').trim(); if(!note){ alert('Add a description of what happened.'); return; } /* A write-up the team member signed is the document that decides an unemployment
   claim. Typing the note is not the same as holding the signed page, so the
   scan rides with the entry rather than living in somebody's camera roll. */
  let _doc=null; const _f=(document.getElementById('caFile')||{}).files;
  if(_f&&_f[0]){ try{ _doc=await uploadMedia(_f[0]); }catch(e){ alert('The file did not upload \u2014 the entry was not saved. '+(e.message||'')); return; }
    if(!_doc){ alert('The file did not upload \u2014 the entry was not saved.'); return; } }
  const detail=JSON.stringify({type,level,note,doc:_doc||undefined,docName:(_f&&_f[0]?_f[0].name:undefined),by:state.user.id,byName:(state.profile&&state.profile.name)||'',created:new Date().toISOString()}); const r=await sb.from('day_items').insert({kind:'coract',title:name,on_date:d,detail,created_by:state.user.id}); if(r&&r.error)return; openCorract(name); };
window.caFilePicked=function(el){
  const lbl=document.getElementById('caFileName'); if(!lbl) return;
...
async function teamCorract(v){
  ...
  h+=`<div class="card" style="padding:13px 15px;margin-bottom:16px"><div class="row" style="gap:8px;align-items:center;flex-wrap:wrap"><span style="font-weight:600;font-size:14px">Log an action for</span><select id="caPick" style="flex:1;min-width:160px;padding:8px 10px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px"><option value="">Choose a team member…</option>${roster.map(n=>`<option value="${esc(n)}">${esc(n)} &middot; ${esc(posOf(n))}</option>`).join('')}</select><button class="btn pri" style="width:auto" onclick="var s=document.getElementById('caPick');if(s&&s.value)openCorract(s.value)">Open</button></div></div>`;
  ...
  h+=withEntries.map(nm=>{ const es=byPerson[nm]; const flags=coractFlags(es); const latest=es.slice().sort((a,b)=>(b.on_date||'').localeCompare(a.on_date||''))[0]; const inits=(nm||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase(); return `<div class="card" style="padding:12px 15px;margin-bottom:9px;display:flex;align-items:center;gap:11px;cursor:pointer" onclick='openCorract(${JSON.stringify(nm)})'><span class="av">${esc(inits)}</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(nm)}</div><div class="faint" style="font-size:12.5px">${es.length} entr${es.length===1?'y':'ies'} &middot; last ${esc(fmtDay(_d(latest.on_date)))}</div></div>${flags.length?`<span style="font-size:12.5px;font-weight:700;color:#8A1C1C;background:#FCEBEB;border:1px solid #F0C9C9;border-radius:8px;padding:3px 9px;white-space:nowrap">${flags.map(esc).join(' &middot; ')}</span>`:''}<i class="ti ti-chevron-right" style="color:var(--faint)"></i></div>`; }).join('');
  v.innerHTML=h;
}
function posSelect(name,cur){ return `<select onchange="setPos('${esc(name).replace(/'/g,"\\'")}',this.value);var d=this.parentNode.querySelector('.posdot');if(d)d.style.background=window.POSCOL(this.value)" style="padding:7px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px">${POS_PICK.map(o=>`<option value="${o}"${o===cur?' selected':''}>${o}</option>`).join('')}</select>`; }
async function schBoard(v){
  ...
  if(isAdmin){ const _rost=team.map(p=>p.name); const _isLead=n=>['Owner','Manager','Supervisor'].includes(posOf(n)); const _capO=n=>{ const c=(profileOf(n)||{}).caps; return (c&&typeof c==='object'&&'open'in c)?!!c.open:_isLead(n); }; const _capC=n=>{ const c=(profileOf(n)||{}).caps; return (c&&typeof c==='object'&&'close'in c)?!!c.close:_isLead(n); }; const _has=(n,st)=>((profileOf(n)||{}).roles||[]).includes(st); const _lvl=(n,st)=>{ const s=(profileOf(n)||{}).skillLevels; if(s&&typeof s==='object'&&(st in s))return +s[st]||0; return _has(n,st)?2:0; }; const fn=n=>esc((n||'').split(' ')[0]); const W=[];
    ...
    if(W.length){ W.sort((a,b)=>b[0]-a[0]); const nB=W.filter(w=>w[0]===2).length; h+=`<div class="card" style="padding:13px 15px;margin-bottom:14px;border-left:4px solid ${nB?'#B32D2D':'var(--amber)'}"><div style="font-weight:700;font-size:14px;margin-bottom:7px">${nB?'⛔':'⚠️'} Before you draft — ${W.length} thing${W.length>1?'s':''} to check</div>`+W.map(w=>`<div style="display:flex;gap:8px;align-items:flex-start;font-size:12.5px;padding:3px 0;color:var(--ink)"><span style="color:${w[0]===2?'#B32D2D':'var(--amber)'};font-weight:800;line-height:1.4">•</span><span>${w[1]}</span></div>`).join('')+`</div>`; }
  }
  ...
  if(isAdmin){
    ...
    if(!_issues.length && !_empty.length){
      ...
      const _rows = _issues.map(x=>`<div style="display:flex;gap:9px;align-items:baseline;padding:5px 0">
          <span style="font-weight:800;font-size:14px;min-width:82px">${esc(_dn(x.i))}</span>
          <span style="font-size:14px">${esc(x.v.why)}</span></div>`).join('')
        + (_empty.length? `<div style="display:flex;gap:9px;align-items:baseline;padding:5px 0;opacity:.75">
          <span style="font-weight:800;font-size:14px;min-width:82px">${_empty.length===1?esc(_dn(_empty[0].i)):_empty.length+' days'}</span>
          <span style="font-size:14px">Nobody scheduled yet.</span></div>` : '');
      ...
      h += `<div style="background:${_tone.bg};border-radius:12px;padding:15px 18px;margin-bottom:14px;color:${_tone.fg}">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
          <i class="ti ${_tone.ic}" style="font-size:18px"></i>
          <div style="font-weight:800;font-size:15.5px">${_issues.length+_empty.length} ${(_issues.length+_empty.length)===1?'day needs':'days need'} attention</div></div>
        ${_rows}</div>`;
    }
  }
  ...
  if(window._draftSummary){ const ds=window._draftSummary; window._draftSummary=null;
    if(ds.noAvail && ds.noAvail.length){
      h+=`<div class="card" style="padding:13px 16px;margin-bottom:14px;background:#F7EEDC;border-color:#E4CFA3">
        ...
        <button class="btn" style="width:auto;padding:7px 13px;font-size:12.5px;margin-top:9px" onclick="goEdit('avail')">Set availability</button></div>`;
    } h+=`<div class="card" style="padding:14px 16px;margin-bottom:14px;background:var(--brand-soft);border-color:var(--brand-line)"><div style="font-weight:700;margin-bottom:3px">✨ Auto-draft ready — ${ds.made} shift${ds.made!==1?'s':''} placed${ds.unfilled?', '+ds.unfilled+' still open':''}</div><div class="faint" style="font-size:12.5px">Review below, tweak anything, then Publish. Nothing reaches the team until you publish.</div>${ds.flags.length?`<div style="margin-top:9px;font-size:14px;line-height:1.7;border-top:1px solid var(--brand-line);padding-top:8px">${ds.flags.map(f=>`<div style="white-space:pre-line;margin-bottom:5px">${esc(f)}</div>`).join('')}</div>`:''}</div>`; }
  if(!shifts.length) h+=`<div class="card" style="padding:30px 22px;text-align:center;margin-bottom:14px"><div style="font-size:26px;color:var(--brand);margin-bottom:8px"><i class="ti ti-calendar-plus"></i></div><div style="font-weight:600;margin-bottom:3px">No shifts this week</div><div class="faint" style="font-size:14px">${isAdmin?'Add one below, tap Copy week, or ✨ Auto-draft to build it from a past week.':'Your shifts show here once leadership posts the schedule.'}</div></div>`;
...
h+=`<div class="board"><div class="board-grid"><div class="bh bh-team">Team</div>`+days.map((d,di)=>`<div class="bh ${isoDate(d)===todayIso?'bh-today':''}">${d.toLocaleDateString(undefined,{weekday:'short'})}<span>${d.getDate()}</span>${dayVerdict[di].label?`<span title="${esc(dayVerdict[di].why)}" style="display:block;margin-top:4px;font-size:11.5px;font-weight:700;letter-spacing:.03em;text-transform:none;padding:2px 6px;border-radius:999px;background:${_VC[dayVerdict[di].level].bg};color:${_VC[dayVerdict[di].level].fg};white-space:nowrap">${esc(dayVerdict[di].label)}</span>`:''}${(isAdmin&&peakOf[di]&&peakOf[di].cap)?`<span title="Most people on at once, against what your coverage matrix allows" style="display:block;margin-top:3px;font-size:11.5px;font-weight:600;text-transform:none;letter-spacing:0;color:${peakOf[di].peak>peakOf[di].cap?'#A8401C':'var(--muted)'};white-space:nowrap">${peakOf[di].peak}/${peakOf[di].cap} at once${peakOf[di].people!==peakOf[di].peak?` &middot; ${peakOf[di].people} people`:''}</span>`:''}${(isAdmin&&lyForDay[di])?`<span title="Same day last year" style="display:block;margin-top:3px;font-size:11.5px;font-weight:600;text-transform:none;letter-spacing:0;color:var(--muted);white-space:nowrap">LY ${money(lyForDay[di].total)}${lyForDay[di].peakH!=null?` &middot; peak ${((lyForDay[di].peakH%12)||12)+(lyForDay[di].peakH<12?'a':'p')}`:''}</span>`:''}</div>`).join('');
  ...
  { const _EK={one_on_one:{l:'1-on-1',c:'#4A9CAD'},interview:{l:'Interview',c:'#7C3AED'},leadership:{l:'Leadership',c:'#2563EB'},meeting:{l:'Meeting',c:'#64748B'}};
    ...
    if(isAdmin || (_evList&&_evList.length)){
      h+=`<div class="band" style="display:flex;align-items:center;justify-content:space-between"><span>Meetings &amp; events</span>${isAdmin?`<button onclick="addEventModal()" style="border:none;background:none;color:var(--brand);font-weight:700;font-size:12.5px;letter-spacing:.02em;cursor:pointer;text-transform:none"><i class="ti ti-plus"></i> Add event</button>`:''}</div>`;
  ...
  Object.keys(roles).sort((a,b)=>{const ia=POS_ORDER.indexOf(a),ib=POS_ORDER.indexOf(b);return (ia<0?99:ia)-(ib<0?99:ib)||a.localeCompare(b);}).forEach(role=>{
    ...
    roles[role].forEach(p=>{
      ...
      const c=hrs*_rate; wCost+=c; dayHrs[di]+=hrs; dayCost[di]+=c; const tr=s.kind==='training'; const oj=isOjr?';border-color:#F4C7C7;background:#FEECEC':''; const styleAttr=isAdmin?` style="cursor:pointer${oj}"`:(isOjr?` style="border-color:#F4C7C7;background:#FEECEC"`:''); cell+=`<div class="scard${tr?' train':''}${_bad?' conflict':''}"${_bad?` title="${esc(_badWhy)}"`:''}${isAdmin?` onclick="editShift(${s.id})" draggable="true" data-sid="${s.id}" ondragstart="schDragStart(event)" ondragend="schDragEnd(event)"`:''}${styleAttr}>${_bad?'<i class="ti ti-alert-triangle" onclick="event.stopPropagation();schWhyConflict(this)" title="Why is this red?" style="font-size:12.5px;margin-right:4px;vertical-align:-1px;cursor:pointer"></i>':''}${isOjr?'<span style="display:inline-block;background:#DC2626;color:#fff;font-size:11.5px;font-weight:800;letter-spacing:.03em;padding:1px 4px;border-radius:5px;margin-right:4px;vertical-align:middle">OJR</span>':''}${s.start_time?fmtClock(s.start_time):'shift'}${s.end_time?'–'+fmtClock(s.end_time):''}${tr?' · train':''}${s.note?`<div style="font-size:11.5px;font-weight:700;opacity:.9;letter-spacing:.02em;margin-top:1px">${esc(s.note)}</div>`:''}${isAdmin?`<span class="x" onclick="event.stopPropagation();delShift(${s.id})">×</span>`:''}${(s.role&&s.role!==posOf(p.name)&&s.role.trim())?`<div style="font-size:11.5px;font-weight:700;opacity:.75;white-space:normal;line-height:1.15;margin-top:1px">${esc(s.role)}</div>`:''}</div>`; });
        if(offRec) cell=`<div class="scard" style="background:#F1F0EE;color:#8A8A8A;border-color:#E2E0DD;font-weight:600;cursor:default" title="${esc(offRec)}"><i class="ti ti-plane" style="font-size:12.5px"></i> Time off</div>`+cell;
        const canAdd = isAdmin && !cell;
        return `<div class="daycell${iso===todayIso?' today':''}"${isAdmin?` data-nm="${esc(p.name)}" data-iso="${iso}"${_bad?` data-bad="1" data-badwhy="${esc(_badWhy)}"`:''} ondragover="schDragOver(event)" ondragleave="schDragLeave(event)" ondrop="schDrop(event)"`:''}${canAdd?` onclick="quickAdd(this)" style="cursor:pointer"`:''}>${cell}${canAdd?'<span class="addhint" style="display:block;text-align:center;color:var(--brand);opacity:.28;font-size:15.5px;font-weight:700;line-height:1">+</span>':''}</div>`;
      }).join('');
  ...
  h+=`<div class="bt bt-team" style="font-weight:600">On the floor</div>`+isoDays.map((iso,di)=>{ const t=covTarget(di); const short=t!=null&&dayPpl[di]<t; return `<div class="bt" style="font-weight:600">${dayPpl[di]||'—'}${t!=null?`<span class="faint" style="font-weight:400"> / ${t}</span>`:''}${short?`<div style="font-size:11.5px;font-weight:600;color:var(--amber)">short ${t-dayPpl[di]}</div>`:''}</div>`; }).join('');
  const _tgtPct=+covRules.targetPct||30;
  h+=`<div class="bt bt-team">Daily total</div>`+isoDays.map((iso,di)=>{ const sl=sales[iso]||fcS[iso]||0; const dc=dayCost[di]*burdenMul; const pct=(isAdmin&&sl&&dc)?Math.round(dc/sl*100):null; return `<div class="bt">${dayHrs[di]?dayHrs[di].toFixed(1)+'h':'—'}${isAdmin&&dc?`<div class="faint" style="font-weight:400;font-size:12.5px">${money(dc)}</div>`:''}${pct!=null?`<div style="font-size:12.5px;font-weight:600;color:${pct>_tgtPct?'var(--amber)':'var(--green)'}">${pct}%</div>`:''}</div>`; }).join('');
  const _wkFc=isoDays.reduce((a,iso)=>a+(fcS[iso]||0),0);
  h+=`<div class="bt bt-team" style="font-weight:400"><span class="faint" title="Expected sales for the week. Auto-figured from last year, scaled to this year's trend. Tap a number to set your own — labor % uses this until real sales come in.">Forecast<div style="font-size:11.5px;opacity:.7;font-weight:400">tap to adjust</div></span></div>`+isoDays.map(iso=>{ const es=fcS[iso]||0; const over=fcAdj[iso]>0; const auto=fcAuto[iso]||0; return `<div class="bt"><div style="display:flex;align-items:center;justify-content:center;gap:1px"><span class="faint" style="font-size:11.5px">$</span><input type="number" min="0" value="${es?Math.round(es):''}" placeholder="—" onchange="setFcAdj('${iso}',this.value,${Math.round(auto)})" onclick="this.select()" style="width:50px;font-size:12.5px;font-weight:600;text-align:center;padding:3px 2px;border:1px solid ${over?'var(--brand)':'var(--line2)'};border-radius:8px;background:var(--card);color:${over?'var(--brand)':'var(--muted)'}"/></div>${over?`<div style="font-size:11.5px;color:var(--brand);font-weight:700;cursor:pointer" onclick="resetFcAdj('${iso}')" title="Back to the forecast number">your # &middot; ↺ reset</div>`:(es?'<div class="faint" style="font-size:11.5px;opacity:.7">est.</div>':'')}</div>`; }).join('')+`</div></div>`;
  if(isAdmin && _wkFc){
    ...
    em+=`<table style="border-collapse:collapse;font-size:12.5px;margin-top:11px;width:100%"><thead><tr style="color:var(--muted);text-align:left"><th style="padding:4px 6px">Day</th>${haveLY?'<th style="padding:4px 6px;text-align:right">Last year</th>':''}<th style="padding:4px 6px;text-align:right">Forecast</th><th style="padding:4px 6px;text-align:right">You set</th></tr></thead><tbody>`;
    ...
    em+=`</tbody></table><div class="faint" style="font-size:12.5px;margin-top:8px">Type your own number in the Forecast row above to override any day — your number wins, and labor % recalculates against it.</div>`;
  ...
  if(isAdmin){ const dn=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
    ...
    const _presets=`<div class="faint" style="font-size:12.5px;margin:2px 0 7px">Quick add — tap a common one, then tweak:</div><div class="row" style="gap:8px;flex-wrap:wrap;margin-bottom:12px">${_presetBtn('Opening leader',{role:'__open__',count:1,start:'',end:'',label:'Opening lead'})}${_presetBtn('Opening support ('+_bakeStation.toLowerCase()+' at open)',{role:_bakeStation,count:1,start:'06:00',end:'08:00',label:'Opening support'})}${_presetBtn('Closing leader',{role:'__close__',count:1,start:'',end:'',label:'Closing lead'})}</div>`;
    const blkrowHTML=(bk)=>{ bk=bk||{}; const days=Array.isArray(bk.days)?bk.days:[0,1,2,3,4,5,6]; return `<tr class="blkrow"><td style="padding:3px 4px"><input type="time" class="blkStart" value="${bk.start||''}" style="width:88px;${_cc}"/></td><td style="padding:3px 4px"><input type="time" class="blkEnd" value="${bk.end||''}" style="width:88px;${_cc}"/></td><td style="padding:3px 4px"><div style="display:flex;gap:2px">${_dLet.map((L,i)=>`<label style="display:inline-flex;flex-direction:column;align-items:center;font-size:11.5px;color:var(--muted)">${L}<input type="checkbox" class="blkDay" data-d="${i}" ${days.indexOf(i)>=0?'checked':''} style="width:14px;height:14px;margin-top:1px"/></label>`).join('')}</div></td><td style="padding:3px 4px"><button onclick="this.closest('tr').remove()" title="Remove" style="border:none;background:none;color:#B32D2D;cursor:pointer;font-size:18px;line-height:1">×</button></td></tr>`; };
    h+=`<div class="faint" style="font-size:12.5px;margin:10px 0;text-align:center">Add a shift by tapping any empty day cell in the grid above — look for the <b style="color:var(--brand)">+</b>.</div>`;
    let _setupOpen=false; try{ _setupOpen=localStorage.getItem('sch_setup_open')==='1'; }catch(e){}
    h+=`<div onclick="schToggleSetup()" style="cursor:pointer;display:flex;align-items:center;gap:9px;padding:12px 15px;margin:14px 0 10px;border:1px solid var(--line2);border-radius:12px;background:var(--card)"><i class="ti ti-bulb" style="color:var(--brand);font-size:18px"></i><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:14px;color:var(--ink)">The rules behind this schedule</div><div class="faint" style="font-size:12.5px">Coverage matrix, required roles, protected rush, labour targets, pay rates &mdash; these live in <b>The Brain</b>. Open them here if you need a quick change.</div></div><i id="schSetupChev" class="ti ti-chevron-${_setupOpen?'up':'down'}" style="color:var(--brand);font-size:18px"></i></div><div id="schSetup" style="display:${_setupOpen?'block':'none'}">`;
    h+=`<div class="sec">Coverage matrix — your staffing by time of day</div><div class="card" style="padding:14px" onchange="autoSaveCov()"><div class="faint" style="font-size:12.5px;margin-bottom:10px">Changes here <b>save automatically</b> as you make them. This is <b>your</b> matrix — every restaurant's is different, built from your own sales &amp; traffic. Add your time blocks, then for each block enter how many people you need on each day. The auto-draft staggers shifts to match this curve, so you're never overstaffed at open. (Later this imports straight from your POS.)</div><label style="display:flex;align-items:center;gap:9px;font-size:14px;cursor:pointer;margin-bottom:10px"><input type="checkbox" id="covLeader" ${covRules.requireLeader?'checked':''}/> <span>Require a <b>leader</b> (manager or supervisor) to open and close every day</span></label><div class="row" style="gap:10px;align-items:center;margin-bottom:12px;flex-wrap:wrap"><span class="faint" style="font-size:12.5px">Shift length</span><span class="faint" style="font-size:12.5px">min</span><input type="number" min="2" max="12" step="0.5" id="covMin" value="${covRules.minLen||4}" style="width:54px;${_cc};text-align:center"/><span class="faint" style="font-size:12.5px">max</span><input type="number" min="3" max="14" step="0.5" id="covMax" value="${covRules.maxLen||8}" style="width:54px;${_cc};text-align:center"/><span class="faint" style="font-size:12.5px">hrs</span><span class="faint" style="font-size:12.5px;margin-left:18px">Labor&nbsp;% goal</span><input type="number" min="0" max="100" step="0.5" id="covLaborPct" value="${covRules.targetPct||''}" placeholder="e.g. 28" style="width:58px;${_cc};text-align:center"/><span class="faint" style="font-size:12.5px">%</span><span class="faint" style="font-size:12.5px;margin-left:18px" title="Payroll taxes, workers' comp, benefits on top of wages — an estimate you can set. Varies by state and wage level.">Labor burden</span><input type="number" min="0" max="60" step="1" id="covBurden" value="${covRules.burdenPct||''}" placeholder="~12" style="width:52px;${_cc};text-align:center"/><span class="faint" style="font-size:12.5px">% &mdash; the taxes &amp; benefits added on top of wages</span></div><div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:12.5px"><thead><tr style="color:var(--muted)"><th style="padding:4px 6px 6px;text-align:left">Start</th><th style="padding:4px 6px 6px;text-align:left">End</th>${dn.map(d=>`<th style="padding:4px 6px 6px">${d}</th>`).join('')}<th></th></tr></thead><tbody id="matrixBody">${_M.blocks.map(mrowHTML).join('')}</tbody></table></div><div class="row" style="gap:10px;margin-top:12px;flex-wrap:wrap"><button class="btn" style="width:auto" onclick="addMatrixRow()"><i class="ti ti-plus"></i> Add time block</button><button class="btn" style="width:auto" onclick="seedMatrixFromDemand()"><i class="ti ti-wand"></i> Build from sales data</button></div><div class="faint" style="font-size:12.5px;margin-top:7px"><b>Build from sales data</b> reads your own sales history and drops in a starting matrix — the right number of people at the right hours for each day, based on how busy you actually were. It's a first draft: edit any cell after, it saves as you go.</div><div class="sec" style="margin-top:18px;font-size:14px">Required roles by time <span class="faint" style="font-weight:400">— optional</span></div><div class="faint" style="font-size:12.5px;margin-bottom:9px">On top of the headcount above, force specific coverage: an <b>opening leader</b>, an opening <b>baker at 6</b>, or two opening leads (front &amp; back, Chick-fil-A style). The draft fills these first and only with people who qualify — and counts them <b>inside</b> your headcount, not on top of it. Leave the times blank to mean "any shift that day." The <b>label</b> shows on the drafted shift so the schedule reads plainly.</div>${_presets}<div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:12.5px"><thead><tr style="color:var(--muted)"><th style="padding:4px 6px 6px;text-align:left">Need</th><th style="padding:4px 6px 6px">#</th><th style="padding:4px 6px 6px;text-align:left">From</th><th style="padding:4px 6px 6px;text-align:left">To</th><th style="padding:4px 6px 6px;text-align:left">Days</th><th style="padding:4px 6px 6px;text-align:left">Label</th><th></th></tr></thead><tbody id="reqBody">${(covRules.roleReqs||[]).map(rqrowHTML).join('')}</tbody></table></div><div class="row" style="gap:10px;margin-top:10px;flex-wrap:wrap"><button class="btn" style="width:auto" onclick="addReqRow()"><i class="ti ti-plus"></i> Add requirement</button></div><div class="sec" style="margin-top:18px;font-size:14px">Protected rush — no mid-rush shift changes <span class="faint" style="font-weight:400">— optional</span></div><div class="faint" style="font-size:12.5px;margin-bottom:9px">The draft won't <b>start or end</b> a shift inside these windows — nobody is scheduled to arrive or leave in the middle of the chaos (hard to get someone placed and a leader has to peel off to direct them). Someone can still clock in to help last‑minute; this only controls the schedule. Example: 11:00–1:00 lunch rush.</div><div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:12.5px"><thead><tr style="color:var(--muted)"><th style="padding:4px 6px 6px;text-align:left">From</th><th style="padding:4px 6px 6px;text-align:left">To</th><th style="padding:4px 6px 6px;text-align:left">Days</th><th></th></tr></thead><tbody id="blkBody">${(covRules.blackouts||[]).map(blkrowHTML).join('')}</tbody></table></div><div class="row" style="gap:10px;margin-top:10px;flex-wrap:wrap"><button class="btn" style="width:auto" onclick="addBlkRow()"><i class="ti ti-plus"></i> Add protected window</button><button class="btn pri" style="width:auto" onclick="saveCovRules()">Save matrix &amp; rules</button><span id="covmsg" class="muted" style="font-size:14px;align-self:center"></span></div></div>`;
    h+=`<div class="sec">Projected sales (for labor %)</div><div class="card" style="padding:14px"><div class="faint" style="font-size:12.5px;margin-bottom:8px">Enter each day's expected sales — the daily total row shows labor as a % of sales, updating instantly.${window._salesCarried?` <span style="color:var(--amber)">Estimated from a recent week — edit any day for this one.</span>`:''}</div><div class="row" style="gap:10px;flex-wrap:wrap">${isoDays.map((iso,di)=>`<div style="text-align:center"><div class="faint" style="font-size:12.5px;margin-bottom:3px">${days[di].toLocaleDateString(undefined,{weekday:'short'})}</div><div class="row" style="gap:2px"><span class="faint" style="align-self:center">$</span><input type="number" id="sales-${iso}" min="0" value="${sales[iso]||''}" placeholder="0" oninput="setSales('${iso}',this.value)" style="width:72px;padding:6px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink)"/></div></div>`).join('')}</div></div>`;
    h+=`<div class="sec">Pay rates</div><div class="card" style="padding:16px;display:flex;align-items:center;gap:12px"><i class="ti ti-lock" style="font-size:18px;color:var(--muted)"></i><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">Kept off the schedule</div><div class="faint" style="font-size:12.5px">Wages don't show here, so no one reads them over your shoulder. They still power Labor $ and Labor %.</div></div><button class="btn" style="width:auto" onclick="go('pay')">Manage pay rates</button></div>`;
    h+=`</div>`;
  }
...
window.schWhyConflict=function(el){
  ...
  box.innerHTML='<div style="display:flex;gap:11px;align-items:flex-start"><i class="ti ti-alert-triangle" style="color:#A32D2D;font-size:18px;margin-top:1px;flex:none"></i><div><div style="font-weight:700;font-size:15.5px;margin-bottom:5px;color:#A32D2D">Scheduling conflict</div><div style="font-size:14px;line-height:1.55;color:var(--ink)">'+esc(why)+'.</div><div class="faint" style="font-size:12.5px;margin-top:9px">Leave it if they have agreed to cover, or drag the shift to another day.</div></div></div><div style="text-align:right;margin-top:16px"><button class="btn" style="width:auto">Got it</button></div>';
...
window.quickAdd=function(cell){ if(!cell||cell.querySelector('.qadd'))return; cell._prevHTML=cell.innerHTML; cell.innerHTML=`<div class="qadd" onclick="event.stopPropagation()" style="display:flex;flex-direction:column;gap:3px;padding:1px"><input type="time" class="qaS" value="08:00" style="font-size:12.5px;padding:2px 3px;border:1px solid var(--line2);border-radius:5px;font-family:inherit;background:var(--card);color:var(--ink)"/><input type="time" class="qaE" value="14:00" style="font-size:12.5px;padding:2px 3px;border:1px solid var(--line2);border-radius:5px;font-family:inherit;background:var(--card);color:var(--ink)"/><div style="display:flex;gap:3px"><button onclick="quickAddSave(this)" style="flex:1;font-size:11.5px;padding:3px;border:none;border-radius:5px;background:var(--brand);color:#fff;cursor:pointer;font-weight:700">Add</button><button onclick="schQuickCancel(this)" style="font-size:11.5px;padding:3px 5px;border:1px solid var(--line2);border-radius:5px;background:var(--card);color:var(--muted);cursor:pointer">&times;</button></div></div>`; const f=cell.querySelector('.qaS'); if(f)f.focus(); };
...
window.editShift=function(id){
  ...
  m.innerHTML=`<div style="background:var(--card);width:100%;max-width:460px;border-radius:12px 16px 0 0;padding:20px 20px 28px;box-shadow:0 -8px 40px rgba(0,0,0,.22)"><div style="font-weight:700;font-size:15.5px">${esc(s.person_name||'Shift')}</div><div class="faint" style="font-size:12.5px;margin-bottom:16px">${esc(fmtDay(_d(s.on_date)))}${s.role?' · '+esc(s.role):''}</div><div class="row" style="gap:10px"><div style="flex:1"><label style="font-size:12.5px;color:var(--muted)">Start</label><input id="esStart" type="time" value="${s.start_time||''}" style="width:100%;padding:11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div><div style="flex:1"><label style="font-size:12.5px;color:var(--muted)">End</label><input id="esEnd" type="time" value="${s.end_time||''}" style="width:100%;padding:11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div></div><div style="margin-top:14px"><label style="font-size:12.5px;color:var(--muted)">What is this block? <span style="opacity:.7">(optional)</span></label><input id="esNote" type="text" value="${esc(s.note||'')}" placeholder="e.g. Manager Meeting, Prep, Training" style="width:100%;padding:11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><div class="faint" style="font-size:12.5px;margin-top:5px">Someone can have more than one block in a day \u2014 open the store, then come back for a meeting. Naming it means the team knows why they are on twice.</div></div><label style="display:flex;align-items:center;gap:9px;margin:14px 0 4px;font-size:14px;cursor:pointer"><input type="checkbox" id="esTrain" ${s.kind==='training'?'checked':''}/> Training shift</label><label style="display:flex;align-items:center;gap:9px;margin:8px 0 2px;font-size:14px;cursor:pointer"><input type="checkbox" id="esOjr" ${ojr?'checked':''}/> <span><b>OJR today</b> \u2014 in charge of this day</span></label><div id="ojrTeach" style="display:none;background:var(--bg,#f4f7f8);border-left:3px solid var(--brand,#4a9cad);border-radius:8px;padding:11px 13px;margin:8px 0 2px"><div style="font-weight:700;font-size:14px">Owner Judgment Representative</div><div class="faint" style="font-size:12.5px;line-height:1.55;margin-top:3px">When more than one leader is on, one person is actually running the shift. That is the OJR. The team knows who to go to, and you know who to ask about how the shift went. Pick one per day \u2014 not per leader.</div></div><div class="row" style="gap:8px;margin-top:16px"><button class="btn" style="width:auto;color:#A32D2D;border-color:#F0C9C9" onclick="delShiftM(${s.id})">Delete</button><button class="btn" style="width:auto;margin-left:auto" onclick="document.getElementById('shedit').remove()">Cancel</button><button class="btn pri" style="width:auto" onclick='saveShift(${s.id},${JSON.stringify(s.on_date)},${JSON.stringify(s.person_name||"")})'>Save</button></div></div>`;
  document.body.appendChild(m); m.addEventListener('click',e=>{ if(e.target===m) m.remove(); });
...
window.addEventModal=function(iso,editId){ const ev=editId?(window._events||{})[editId]:null; const team=(window._team||[]).map(function(p){return p.name;}).filter(Boolean); const kinds=[['one_on_one','1-on-1'],['interview','Interview'],['leadership','Leadership meeting'],['meeting','Meeting']]; const cur=ev?ev.kind:'one_on_one'; const att=(ev&&Array.isArray(ev.attendees))?ev.attendees:[];
  ...
  m.innerHTML=`<div style="background:var(--card);width:100%;max-width:480px;border-radius:12px 16px 0 0;padding:20px 20px 28px;box-shadow:0 -8px 40px rgba(0,0,0,.22);max-height:88vh;overflow:auto"><div style="font-weight:700;font-size:15.5px;margin-bottom:14px">${ev?'Edit event':'Add an event'}</div><label style="font-size:12.5px;color:var(--muted)">Type</label><select id="evKind" style="width:100%;padding:11px;margin:4px 0 12px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${kinds.map(function(k){return `<option value="${k[0]}" ${cur===k[0]?'selected':''}>${k[1]}</option>`;}).join('')}</select><label style="font-size:12.5px;color:var(--muted)">Title</label><input id="evTitle" value="${ev?esc(ev.title||''):''}" placeholder="e.g. 1-on-1 with Presley" style="width:100%;padding:11px;margin:4px 0 12px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><div class="row" style="gap:10px"><div style="flex:1"><label style="font-size:12.5px;color:var(--muted)">Date</label><input id="evDate" type="date" value="${ev?ev.on_date:(iso||isoDate(new Date()))}" style="width:100%;padding:11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div></div><div class="row" style="gap:10px;margin-top:12px"><div style="flex:1"><label style="font-size:12.5px;color:var(--muted)">Start</label><input id="evStart" type="time" value="${ev?(ev.start_time||''):''}" style="width:100%;padding:11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div><div style="flex:1"><label style="font-size:12.5px;color:var(--muted)">End</label><input id="evEnd" type="time" value="${ev?(ev.end_time||''):''}" style="width:100%;padding:11px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/></div></div><label style="font-size:12.5px;color:var(--muted);display:block;margin-top:12px">Location / note</label><input id="evLoc" value="${ev?esc(ev.location||''):''}" placeholder="Optional — where, or a quick note" style="width:100%;padding:11px;margin:4px 0 12px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><label style="font-size:12.5px;color:var(--muted);display:block">Who's joining <span class="faint">— they'll see it and can add it to their own calendar</span></label><div id="evAtt" style="display:flex;flex-wrap:wrap;gap:6px;margin:8px 0 4px">${team.length?team.map(function(n){return `<label style="display:inline-flex;align-items:center;gap:5px;border:1px solid var(--line2);border-radius:999px;padding:5px 11px;font-size:14px;cursor:pointer"><input type="checkbox" value="${esc(n)}" ${att.indexOf(n)>=0?'checked':''}/> ${esc(n)}</label>`;}).join(''):'<span class="faint" style="font-size:12.5px">No team members on the roster yet</span>'}</div><div class="row" style="gap:8px;margin-top:18px">${ev?`<button class="btn" style="width:auto;color:#A32D2D;border-color:#F0C9C9" onclick="delEvent('${ev.id}')">Delete</button>`:''}<button class="btn" style="width:auto;margin-left:auto" onclick="document.getElementById('evmodal').remove()">Cancel</button><button class="btn pri" style="width:auto" onclick="saveNewEvent(${ev?`'${ev.id}'`:'null'})">${ev?'Save':'Add event'}</button></div></div>`;
...
window.eventDetail=function(id){ const e=(window._events||{})[id]; if(!e)return; const isAdmin=(typeof myRank==='function'&&myRank()>=3)||(typeof hasGrant==='function'&&hasGrant('schedule')); const KL={one_on_one:'1-on-1',interview:'Interview',leadership:'Leadership meeting',meeting:'Meeting'}; const KC={one_on_one:'#4A9CAD',interview:'#7C3AED',leadership:'#2563EB',meeting:'#64748B'}; const col=KC[e.kind]||'#64748B'; const att=Array.isArray(e.attendees)?e.attendees:[]; const when=fmtDay(_d(e.on_date))+(e.start_time?' · '+fmtClock(e.start_time):'')+(e.end_time?'–'+fmtClock(e.end_time):'');
  ...
  m.innerHTML=`<div style="background:var(--card);width:100%;max-width:460px;border-radius:12px 16px 0 0;padding:20px 20px 28px;box-shadow:0 -8px 40px rgba(0,0,0,.22)"><div style="display:inline-block;font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:${col};background:${col}14;padding:3px 9px;border-radius:8px;margin-bottom:9px">${KL[e.kind]||'Meeting'}</div><div style="font-weight:700;font-size:18px;line-height:1.25">${esc(e.title||KL[e.kind]||'Event')}</div><div class="faint" style="font-size:14px;margin-top:3px">${esc(when)}</div>${e.location?`<div style="font-size:14px;margin-top:8px"><i class="ti ti-map-pin" style="color:var(--muted)"></i> ${esc(e.location)}</div>`:''}${att.length?`<div style="font-size:14px;margin-top:10px;color:var(--muted)">With: <span style="color:var(--ink)">${att.map(esc).join(', ')}</span></div>`:''}<div style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn pri" style="width:auto" onclick="calAddEvent('${e.id}')"><i class="ti ti-calendar-plus"></i> Add to my calendar</button><a class="btn" style="width:auto;text-decoration:none;display:inline-flex;align-items:center;gap:5px" href="${_gcalUrl({on_date:e.on_date,start_time:e.start_time,end_time:e.end_time,title:e.title,location:e.location,note:e.note})}" target="_blank" rel="noopener"><i class="ti ti-brand-google"></i> Google</a></div><div class="row" style="gap:8px;margin-top:16px">${isAdmin?`<button class="btn" style="width:auto" onclick="document.getElementById('evdet').remove();addEventModal(null,'${e.id}')"><i class="ti ti-pencil"></i> Edit</button>`:''}<button class="btn" style="width:auto;margin-left:auto" onclick="document.getElementById('evdet').remove()">Close</button></div></div>`;
...
window.publishWeek=async function(){ const wk=state.ctx.wk||isoDate(weekStart(new Date())); const start=weekStart(wkDate(wk)); const d6=isoDate(new Date(start.getTime()+6*864e5));
  /* Publishing is the moment reachability actually matters, so it is the moment to say so.
     Only the people working this week count -- a departed name with no email is not a
     problem -- and it never appears once everyone scheduled can be reached. */
  try{
    const _rs=await sb.from('shifts').select('person_name').gte('on_date',wk).lte('on_date',d6);
    const _on=[...new Set((_rs.data||[]).map(x=>x.person_name).filter(n=>n&&n!=='__OPEN__'))];
    const _mail=n=>{ const e=((profileOf(n)||{}).email||'').trim(); return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(e); };
    const _out=_on.filter(n=>!_mail(n)).sort();
    if(_out.length){
      const _msg='No way to tell '+_out.length+' of the '+_on.length+' people working this week:\n\n'
        +_out.map(n=>'  \u2022 '+n).join('\n')
        +'\n\nThey have no email address on file, so publishing will not reach them. '
        +'Add contacts under The Brain \u2192 Reaching your team.\n\nPublish anyway?';
...
window.readMyWeek=async function(){
  ...
  try{
    ...
    if(fillRecs.length){ html+='<div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:16px 0 8px">Who can fill the open shifts</div>'; fillRecs.forEach(fr=>{ const wdN=_d(fr.o.on_date).toLocaleDateString(undefined,{weekday:'long'}); const tm=(fr.o.start_time?fmtClock(fr.o.start_time):'')+(fr.o.end_time?'–'+fmtClock(fr.o.end_time):''); html+='<div style="padding:6px 0;border-bottom:1px solid var(--line)"><div style="font-weight:600">'+esc(wdN)+' '+esc(tm)+(fr.o.role?' · '+esc(fr.o.role):'')+'</div><div class="faint" style="font-size:14px;margin-top:2px">'+(fr.who.length?('Free to take it: '+fr.who.slice(0,6).map(esc).join(', ')):'No one\'s free — everyone that day is already working, off, or unavailable.')+'</div></div>'; }); }
 ...
 return c?(('close'in c)?!!c.close:isLeader(n)):isLeader(n); };
  ...
  try{
    ...
    /* ---------- What a good week looks like ----------
       The search below already trades shifts around to fill gaps, hit weekly minimums and
       hold the labour target. It had no opinion about three things an owner cares about a
       lot, so it never fixed them however many passes it ran:

         1. Where the SECOND and third leaders go. Every day getting one is a hard rule;
            after that the extras belong on the busy days. A draft with three leaders on a
            quiet Thursday and two on the busiest Sunday is worse, and nothing said so.
         2. Days off landing together. 'Two days off, any two, but consecutive' is how
            people actually rest. Max-days and days-in-a-row cannot express it -- they cap
            work, they do not group rest.
         3. Being over the coverage matrix in a single block. The old check only compared
            against a day's peak, so an extra body at noon on a day that peaks at 8 went
            unnoticed.
    ...
    const _volMax = Math.max(1, ...Object.values(_volOf));
    const LEAD_SPREAD_PEN = 26;   // an extra leader parked on a quiet day
    const OFFRUN_PEN      = 34;   // rest days scattered instead of together
    const OVERSTAFF_PEN   = 30;   // more people on than the block asks for

    const _leadCount = iso => draft.filter(e=>e.person_name!=='__OPEN__' && e.on_date===iso && _isLead(e.person_name)).length;
    const _leadTerm = iso => { const extra=Math.max(0,_leadCount(iso)-1); return extra * LEAD_SPREAD_PEN * (1 - (_volOf[iso]||0)/_volMax); };

    const _offRunTerm = n => {
      const pr=profileOf(n)||{}; const want=+pr.offRun||0; if(!(want>0)) return 0;
      const off=isoDays.map(iso=>!(Dset[n]&&Dset[n].has(iso)));
      let best=0,run=0; off.concat(off).forEach(o=>{ run=o?run+1:0; if(run>best)best=run; });  // wrap the week
      return best>=want ? 0 : (want-Math.min(best,want)) * OFFRUN_PEN;
    };

    const _mins = t => { const m=/^(\d{1,2}):(\d{2})/.exec(t||''); return m? +m[1]*60 + +m[2] : null; };
    const _overTerm = iso => {
      const di=isoDays.indexOf(iso); if(di<0) return 0;
      const list=draft.filter(e=>e.person_name!=='__OPEN__' && e.on_date===iso);
      let pen=0;
      (covRulesMatrixBlocks||[]).forEach(b=>{
        const want=+b.n[di]||0, bs=_mins(b.s), be=_mins(b.e); if(bs==null||be==null) return;
        let peak=0;
        for(let t=bs;t<be;t+=15){ let on=0; list.forEach(e=>{ const a=_mins(e.start_time), z=_mins(e.end_time); if(a!=null&&z!=null&&a<=t&&z>t) on++; }); if(on>peak) peak=on; }
        if(peak>want) pen += (peak-want) * OVERSTAFF_PEN;
      });
      return pen;
    };

    /* Cost of the days and people a single move touches, so the delta stays cheap. */
    const _localScore = (isos, names) => {
      let v=0;
    ...
    /* The added terms are far more expensive to evaluate than the original ones -- each
       candidate move now re-scores the day it touches. The loop usually settles in a few
       dozen passes, but the 2000 guard exists for the cases that do not, and at this cost
       that would lock the browser for minutes. Stop improving after three seconds and keep
       the best week found so far: a very good schedule now beats a perfect one never. */
    const _deadline = Date.now() + 3000;
    let guard=0;
    while(guard++<2000){ if(Date.now()>_deadline) break; let best=null, bestDelta=-1e-6;
      for(let i=0;i<draft.length;i++){ const e=draft[i]; const hrs=hrsE(e); const cur=e.person_name;
        if(cur==='__OPEN__'){ for(const n of allNames){ if(!canTakeO(n,e,i))continue;
            const _before=_localScore([e.on_date],[n]);
            e.person_name=n; Dset[n].add(e.on_date);                       // try it
            const _after=_localScore([e.on_date],[n]);
            e.person_name='__OPEN__'; Dset[n].delete(e.on_date);           // put it back
            const d=-OPEN_PEN+(minPen(n,H[n]+hrs)-minPen(n,H[n]))+(laborPen(cost+hrs*(wageO[n]||0))-laborPen(cost))+(isOffO(n,e)?DAYOFF_PEN:0)+(_after-_before);
            if(d<bestDelta){ bestDelta=d; best={i,n,from:'__OPEN__',hrs}; } } }
        else { for(const n of allNames){ if(n===cur)continue; if(!canTakeO(n,e,i))continue;
            const dCost=hrs*((wageO[n]||0)-(wageO[cur]||0));
            const _before=_localScore([e.on_date],[cur,n]);
            const _curHadOther = draft.some((x,xi)=>xi!==i && x.person_name===cur && x.on_date===e.on_date);
            e.person_name=n; Dset[n].add(e.on_date); if(!_curHadOther) Dset[cur].delete(e.on_date);
            const _after=_localScore([e.on_date],[cur,n]);
            e.person_name=cur; Dset[n].delete(e.on_date); if(!_curHadOther) Dset[cur].add(e.on_date);
            const d=(minPen(cur,H[cur]-hrs)-minPen(cur,H[cur]))+(minPen(n,H[n]+hrs)-minPen(n,H[n]))+(laborPen(cost+dCost)-laborPen(cost))+((isOffO(n,e)?DAYOFF_PEN:0)-(isOffO(cur,e)?DAYOFF_PEN:0))+(_after-_before);
            if(d<bestDelta){ bestDelta=d; best={i,n,from:cur,hrs}; } } }
      }
      if(!best) break;
...
async function schOverview(v){
  ...
  else h+=`<div class="card">`+todayShifts.map(s=>{ const inits=(s.person_name||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase(); const tr=s.kind==='training'; return `<div class="row" style="padding:11px 15px;border-bottom:1px solid var(--line)"><span class="av">${esc(inits)}</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(s.person_name||'—')}</div><div class="faint" style="font-size:12.5px">${esc(s.role||'Team')}${tr?' · training':''}</div></div><div style="font-size:14px;font-weight:600;color:${tr?'var(--amber)':'var(--brand)'}">${s.start_time?fmtClock(s.start_time):'shift'}${s.end_time?'–'+fmtClock(s.end_time):''}</div></div>`; }).join('')+`</div>`;
...
async function schTeam(v){
  ...
  if(tv==='now'){
    ...
    const rowOf=(s,tag,col)=>{ const inits=(s.person_name||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase(); const pos=posOf(s.person_name); const pc=POS_COL[pos]||'#94A3B8'; return `<div class="row" style="padding:11px 15px;border-bottom:1px solid var(--line)"><span class="av" style="background:${pc}22;color:${pc}">${esc(inits)}</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(s.person_name||'—')}</div><div class="faint" style="font-size:12.5px">${esc(pos)}${s.kind==='training'?' · training':''}</div></div><div style="text-align:right"><div style="font-size:14px;font-weight:600;color:${col}">${s.start_time?fmtClock(s.start_time):''}${s.end_time?'–'+fmtClock(s.end_time):''}</div>${tag?`<div class="faint" style="font-size:12.5px">${tag}</div>`:''}</div></div>`; };
  ...
  h+=`${isAdmin?`<div style="display:flex;justify-content:flex-end;margin-bottom:10px"><button onclick="openRosterImport()" style="background:transparent;border:1px solid var(--line2,#d5dde0);border-radius:8px;padding:8px 14px;font-size:14px;font-weight:600;cursor:pointer;color:inherit"><i class="ti ti-upload"></i> Import from another scheduler</button><button onclick="go('brain')" style="margin-left:8px;background:transparent;border:1px solid var(--line2,#d5dde0);border-radius:8px;padding:8px 14px;font-size:14px;font-weight:600;cursor:pointer;color:inherit"><i class="ti ti-bulb"></i> Stations &amp; skills live in The Brain</button></div>`:''}`;
h+=`<div class="faint" style="font-size:14px;margin-bottom:14px">${team.length} people${isAdmin?' · positions are set in Onboarding':''}</div>`;
...
async function schAvail(v){
    ...
    if(reqs.length){
      ...
      reqs.forEach((x,i)=>{
        ...
        const lines=Object.keys(ch).map(wd=>{
          const f=ch[wd];
          return '<div style="font-size:14px;padding:2px 0"><b>'+esc(D[wd])+'</b> &mdash; '+(f.can_work===false? 'cannot work' : (f.note? esc(String(f.note).replace('-','\u2013')) : 'available all day'))+'</div>';
        }).join('');
        h+=`<div style="padding:14px 16px;${i?'border-top:1px solid var(--line)':''}">
          <div style="font-weight:700;font-size:14px">${esc(dispName(x.title))}</div>
          <div class="faint" style="font-size:12.5px;margin-bottom:6px">asked ${esc(x.on_date||'')}</div>
          ${lines}
  ...
  if(!mgr) h+=`<div id="avReqBar" style="display:none;gap:11px;align-items:center;flex-wrap:wrap;background:var(--bg);border-radius:12px;padding:12px 14px;margin-bottom:12px">
      <span id="avReqCount" style="font-weight:700;font-size:14px">Changes not sent yet</span>
      ...
      <button class="btn pri" style="width:auto;padding:9px 16px;font-size:14px" onclick="avSubmitRequest()">Send for approval</button></div>`;
  h+=`<div class="card" style="padding:4px 2px;margin-bottom:26px">`+AV_DOW.map((d,i)=>{ const row=(byPerson[me]||{})[i]; const can=row?row.can_work!==false:true; const note=row?(row.note||''):''; const win=parseWin(note); const status=!can?'off':((win||note==='limited')?'limited':'avail');
    return `<div style="padding:9px 12px;${i<6?'border-bottom:1px solid var(--line)':''}"><div class="row" style="gap:9px"><div style="width:38px;font-weight:600;font-size:14px">${d}</div><div class="avseg"><button class="avbtn${status==='avail'?' on':''}" onclick="avMe(${i},'avail',this)">Available</button><button class="avbtn${status==='limited'?' lim':''}" onclick="avMe(${i},'limited',this)">Limited</button><button class="avbtn${status==='off'?' off':''}" onclick="avMe(${i},'off',this)">Off</button></div></div><div class="avwin" data-wd="${i}" style="display:${status==='limited'?'flex':'none'};gap:7px;align-items:center;margin:9px 0 2px 47px;flex-wrap:wrap"><span class="faint" style="font-size:12.5px">Can work</span><input type="time" class="avfrom" value="${win?win[0]:''}" onchange="avWin(${i})" style="padding:7px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px"/><span class="faint" style="font-size:12.5px">to</span><input type="time" class="avto" value="${win?win[1]:''}" onchange="avWin(${i})" style="padding:7px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px"/></div></div>`; }).join('')+`</div>`;
  if(mgr) h+=`<div class="sec">Team availability</div><div id="avTeamGrid"><div class="faint">Loading…</div></div>`;
  v.innerHTML=h;
  /* The one grid, rendered here rather than behind a button that opened a second copy. */
  if(mgr){ const _g=document.getElementById('avTeamGrid'); if(_g){ try{ renderAvailInline(_g); }catch(e){ _g.innerHTML='<div class="faint">Could not load availability.</div>'; } } }
}
...
async function schTimeoff(v){
  ...
  const stpill=s=> s==='approved'?`<span class="pill" style="background:var(--green-soft);color:var(--green);font-size:12.5px;font-weight:600">Approved</span>` : s==='denied'?`<span class="pill" style="background:#FCEBEB;color:#A32D2D;font-size:12.5px;font-weight:600">Declined</span>` : `<span class="pill" style="background:var(--amber-soft);color:var(--amber);font-size:12.5px;font-weight:600">Pending</span>`;
...
async function schLogbook(v){
  ...
  const dt=_d(d0);
  /* The Hard / OK / Good / Great rating is gone. It was measuring temperament, not the
     shift -- the same night gets rated Hard and Good by two people, and the ones who
     always pick Great and the ones who always pick Hard cancel each other out into a
     number nobody can act on. What the next leader needs is the handover, in words. */
  const SCORES=[];
  ...
  const cell=(l,vv,c)=>`<div style="flex:1;min-width:88px;text-align:center"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em">${l}</div><div style="font-size:18px;font-weight:800;color:${c||'var(--ink)'}">${vv}</div></div>`;
  const actCell=`<div style="flex:1;min-width:96px;text-align:center"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em">Actual</div><div style="font-size:18px;font-weight:800">${isAdmin?`<input type="number" id="logActual" value="${actual||''}" placeholder="0" oninput="logSetActual('${d0}',this.value)" style="width:96px;text-align:center;font-size:18px;font-weight:800;border:1px solid var(--line2);border-radius:8px;padding:3px 6px;background:var(--card);color:var(--ink)"/>`:(actual?money(actual):'—')}</div>${varP!=null?`<div style="font-size:12.5px;font-weight:800;color:${varP>=0?'#1B7B3F':'#B32D2D'}">${varP>=0?'▲':'▼'} ${Math.abs(varP).toFixed(0)}% vs forecast</div>`:''}</div>`;
  h+=`<div class="card" style="padding:16px 16px 14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:12px">The day&rsquo;s numbers</div><div class="row" style="gap:10px;flex-wrap:wrap">${actCell}${cell('Forecast',forecast?money(forecast):'—')}${isAdmin?cell('Labor',money(cost)):''}${isAdmin?cell('Labor %',pct!=null?pct.toFixed(0)+'%':'—',pct!=null&&pct>30?'var(--amber)':'var(--green)'):''}${cell('Hours',hrs.toFixed(1))}</div><div class="faint" style="font-size:12.5px;margin-top:8px">Forecast comes from the projected sales you set on the Schedule. Enter the day's actual here at close.</div></div>`;
  h+=`<div class="card" style="padding:16px;margin-top:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:4px">Hand the shift over</div><div class="faint" style="font-size:13px;margin-bottom:11px">What the next leader needs to know before they walk in.</div><textarea id="logSummary" placeholder="Shift summary — what the next leader should know. Wins, problems, follow-ups…" style="width:100%;min-height:70px;padding:11px;border:1px solid var(--line2);border-radius:8px;font-size:14px;font-family:inherit;color:var(--ink);background:var(--card)">${esc(meta.summary||'')}</textarea><div class="row" style="margin-top:9px"><button class="btn pri" style="width:auto;margin-left:auto" onclick="logSaveMeta('${d0}')">Save the day</button><span id="logmsg" class="muted" style="font-size:14px;align-self:center;margin-left:8px"></span></div></div>`;
  ...
  h+=`<div class="card" style="padding:16px;margin-top:14px;margin-bottom:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:7px">Close out your shift</div><div class="faint" style="font-size:12.5px;margin:0 0 12px">Pick your shift, log what happened, and flag anything the next shift needs.</div><div class="row" style="gap:8px;flex-wrap:wrap;margin-bottom:8px"><select id="lgseg" style="padding:9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"><option value="Opening">Opening</option><option value="Mid">Mid</option><option value="Closing">Closing</option></select><select id="lgcat" style="padding:9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${cats.map(c=>`<option value="${c}">${c[0].toUpperCase()+c.slice(1)}</option>`).join('')}</select></div><textarea id="lgbody" placeholder="What got done, what broke, what the next shift needs to know…" style="width:100%;min-height:56px;padding:11px;border:1px solid var(--line2);border-radius:8px;font-size:14px;font-family:inherit;color:var(--ink);background:var(--card)"></textarea><div class="row" style="margin-top:8px"><button class="btn" style="width:auto;margin-left:auto" onclick="addLog('${d0}')">Add note</button></div></div>`;
  h+=`<div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:4px 2px 10px">The day&rsquo;s log</div>`;
  const logs=rlog.data||[];
  if(!logs.length) h+=`<div class="faint" style="font-size:14px;text-align:center;padding:8px">No notes logged for this day.</div>`;
  else h+=logs.map(e=>`<div class="card" style="padding:12px 15px;margin-bottom:9px;border-left:4px solid ${CAT_COL[e.category]||'#94A3B8'}${e.resolved?';opacity:.55':''}"><div class="row" style="gap:9px;margin-bottom:5px">${e.segment?`<span style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;color:var(--ink);background:#EEEDFE;border-radius:8px;padding:2px 7px;flex-shrink:0">${esc(e.segment)}</span>`:''}<i class="ti ${cico[e.category]||'ti-note'}" style="color:${CAT_COL[e.category]||'var(--brand)'};font-size:15.5px"></i><span style="font-weight:600;font-size:14px;text-transform:capitalize">${esc(e.category||'note')}</span><span class="faint" style="font-size:12.5px">· ${esc(e.author_name||'')} · ${timeAgo(e.created_at)}</span><button onclick="logToggleResolved(${e.id},${e.resolved?0:1})" title="${e.resolved?'Reopen — put back on Today':'Clear — done, drop from Today'}" style="margin-left:auto;border:1px solid var(--line2);background:var(--card);color:${e.resolved?'#1B7B3F':'var(--muted)'};cursor:pointer;font-size:12.5px;font-weight:700;border-radius:8px;padding:2px 9px">${e.resolved?'✓ Cleared':'Clear'}</button>${isAdmin?`<button onclick="delLog(${e.id},'${d0}')" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:15.5px;margin-left:6px">×</button>`:''}</div><div class="muted" style="font-size:14px;line-height:1.5;white-space:pre-wrap">${esc(e.body||'')}</div></div>`).join('');
  v.innerHTML=h;
}
...
async function schReports(v){
  ...
  let h=`<div class="print-only" style="margin-bottom:10px"><div style="font-size:18px;font-weight:800">Labor report</div><div style="font-size:14px;color:#555">${fmtDay(_d(days[0]))} – ${fmtDay(_d(days[6]))}</div></div>`;
  ...
  const card=(lbl,val,sub,col)=>`<div class="card" style="padding:14px 16px;flex:1;min-width:130px"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin-bottom:4px">${lbl}</div><div style="font-size:26px;font-weight:800;line-height:1;color:${col||'var(--ink)'}">${val}</div>${sub?`<div class="faint" style="font-size:12.5px;margin-top:4px">${sub}</div>`:''}</div>`;
  ...
  if(isAdmin){
    ...
    h+=`<div class="sec">Sales vs last year</div><div class="card" style="padding:16px"><div class="row" style="gap:18px;flex-wrap:wrap;align-items:flex-end"><div><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em">This week</div><div style="font-size:26px;font-weight:800">${totSales?money(totSales):'—'}</div></div><div><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em">Same week last year</div><div style="font-size:18px;font-weight:700;color:var(--muted)">${lyTotal?money(lyTotal):'—'}</div></div>${lyDelta!=null?`<div style="font-size:18px;font-weight:800;color:${lyDelta>=0?'#1B7B3F':'#B32D2D'}">${lyDelta>=0?'▲':'▼'} ${Math.abs(lyDelta).toFixed(1)}% ${lyDelta>=0?'up':'down'}</div>`:`<div class="faint" style="font-size:12.5px">No sales recorded for this week last year yet.</div>`}</div></div>`;
  ...
  else h+=`<div class="card">`+names.map(n=>{ const p=per[n]; const inits=(n||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase(); const ot=p.hrs>otW+1e-9; return `<div class="row" style="padding:10px 15px;border-bottom:1px solid var(--line);gap:11px"><span class="av">${esc(inits)}</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${esc(dispName(n))} <span class="faint" style="font-size:12.5px;font-weight:500">${esc(posOf(n))} · ${p.dset.size}d</span>${ot?' <span class="pill" style="background:#FCEBEB;color:#B32D2D;font-size:11.5px;font-weight:800;padding:1px 6px">OT</span>':''}</div><div style="height:6px;border-radius:5px;background:var(--line);overflow:hidden;margin-top:5px"><i style="display:block;height:100%;width:${Math.round(p.hrs/maxPH*100)}%;background:${ot?'#B32D2D':'var(--brand)'}"></i></div></div><div style="text-align:right"><div style="font-weight:700;font-size:14px">${p.hrs.toFixed(1)}h</div>${isAdmin&&p.cost?`<div class="faint" style="font-size:12.5px">${money(p.cost)}</div>`:''}</div></div>`; }).join('')+`</div>`;
  v.innerHTML=h;
}
...
async function vSales(v){
  ...
  const pctCard=(lbl,val,sub,dc)=>`<div class="card" style="padding:16px 18px;flex:1;min-width:150px"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em">${lbl}</div><div style="font-size:26px;font-weight:800;margin:3px 0">${val==null?'<span style="color:var(--faint)">—</span>':val.toFixed(1)+'%'}</div><div style="font-size:12.5px;color:${dc||'var(--muted)'};font-weight:600">${sub}</div></div>`;
  ...
  const tab=(id,lbl)=>`<button onclick="state.ctx.salesView='${id}';vSales(document.getElementById('view'))" style="border:none;background:${view===id?'var(--brand)':'transparent'};color:${view===id?'#fff':'var(--muted)'};padding:7px 16px;border-radius:8px;font-weight:600;font-size:14px;cursor:pointer">${lbl}</button>`;
  h+=`<div style="display:inline-flex;gap:4px;padding:4px;background:var(--bg);border-radius:12px;margin-bottom:14px">${tab('daily','Daily')}${tab('weekly','Weekly')}${tab('monthly','Monthly')}</div>`;
  ...
  if(view==='daily'){
    ...
    h+=`<div class="sec">Last 14 days</div><div class="card" style="padding:4px 2px">`+days.map(k=>`<div class="row" style="padding:10px 14px;border-bottom:1px solid var(--line);gap:12px;align-items:center"><div style="width:118px;font-size:14px;font-weight:600">${fmtD(k)}</div><div style="flex:1;height:9px;background:var(--line);border-radius:5px;overflow:hidden"><i style="display:block;height:100%;width:${Math.round(map[k]/mx*100)}%;background:var(--brand)"></i></div><div style="width:92px;text-align:right;font-weight:700;font-size:14px">${money(map[k])}</div></div>`).join('')+`</div>`;
  } else if(view==='weekly'){
    ...
    h+=`<div class="sec">Last 8 weeks</div><div class="card" style="padding:4px 2px">`+wk.map(w=>{ const we=new Date(_d(w).getTime()+6*864e5); return `<div class="row" style="padding:10px 14px;border-bottom:1px solid var(--line);gap:12px;align-items:center"><div style="width:150px;font-size:14px;font-weight:600">${_d(w).toLocaleDateString(undefined,{month:'short',day:'numeric'})} – ${we.toLocaleDateString(undefined,{month:'short',day:'numeric'})}</div><div style="flex:1;height:9px;background:var(--line);border-radius:5px;overflow:hidden"><i style="display:block;height:100%;width:${Math.round(weeks[w]/mx*100)}%;background:var(--brand)"></i></div><div style="width:100px;text-align:right;font-weight:700;font-size:14px">${money(weeks[w])}</div></div>`; }).join('')+`</div>`;
  } else {
    ...
    h+=`<div class="sec">Monthly — ${y} vs ${y-1}</div><div class="card" style="padding:16px"><div style="display:flex;align-items:flex-end;gap:6px;height:164px">`+tY.map((val,i)=>{ const hY=Math.round(val/maxM*118), hL=Math.round(tL[i]/maxM*118); return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px"><div style="font-size:11.5px;color:var(--muted);height:12px">${val?('$'+Math.round(val/1000)+'k'):''}</div><div style="display:flex;align-items:flex-end;gap:2px;height:122px"><div title="${y-1} ${MN[i]}: ${money(tL[i])}" style="width:7px;height:${hL}px;background:var(--line2);border-radius:5px 2px 0 0"></div><div title="${y} ${MN[i]}: ${money(val)}" style="width:9px;height:${hY}px;background:var(--brand);border-radius:5px 2px 0 0"></div></div><div class="faint" style="font-size:11.5px">${MN[i][0]}</div></div>`; }).join('')+`</div><div class="row" style="gap:14px;margin-top:10px;font-size:12.5px"><span style="display:inline-flex;align-items:center;gap:5px"><span style="width:9px;height:9px;background:var(--brand);border-radius:5px"></span>${y}</span><span style="display:inline-flex;align-items:center;gap:5px"><span style="width:9px;height:9px;background:var(--line2);border-radius:5px"></span>${y-1}</span></div></div>`;
  }
...
async function vSalesHist(v){
  ...
  let h=`<div class="card" style="padding:16px 18px;margin-bottom:16px"><div style="font-weight:700;font-size:15.5px">Hourly sales on file</div>${have?`<div style="font-size:14px;margin-top:5px">${have} day${have>1?'s':''} loaded · ${esc(fmtDay(_d(first)))} → ${esc(fmtDay(_d(last)))}</div>`:`<div class="faint" style="font-size:14px;margin-top:5px">Nothing loaded yet — import a POS export below.</div>`}<div class="faint" style="font-size:12.5px;margin-top:6px">This powers the “last year, this day, by hour” panel on the Schedule and holiday comparisons. Once your POS is connected live, this stays current automatically.</div></div>`;
  h+=`<div class="card" style="padding:18px"><div style="font-weight:700;font-size:15.5px;margin-bottom:4px">Import hourly sales</div><div class="faint" style="font-size:12.5px;margin-bottom:12px">Export an <b>hourly sales</b> report from your POS as a CSV (Toast: Reports → Sales, export), then drop it here. You'll pick which columns are the date, the hour, and the sales — I'll figure out the rest.</div><input type="file" id="hsFile" accept=".csv,text/csv" style="font-size:14px" onchange="hsPick(this)"/><div id="hsMap" style="margin-top:14px"></div></div>`;
  v.innerHTML=h;
}
...
window.hsPick=function(inp){ const f=inp.files&&inp.files[0]; if(!f)return; const rd=new FileReader(); rd.onload=()=>{ const rows=_csvParse(String(rd.result||'')); if(rows.length<2){ alert('Could not read rows from that file.'); return; } window._hsHead=rows[0]; window._hsRows=rows.slice(1); const opts=window._hsHead.map((c,i)=>`<option value="${i}">${esc(c||('Column '+(i+1)))}</option>`).join(''); const guess=kw=>{ const idx=window._hsHead.findIndex(c=>kw.test(String(c||'').toLowerCase())); return idx<0?0:idx; }; const sel=(id,lbl,g)=>`<div><div class="faint" style="font-size:12.5px;margin-bottom:2px">${lbl}</div><select id="${id}" style="padding:8px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit">${opts}</select></div>`; const m=document.getElementById('hsMap'); m.innerHTML=`<div class="faint" style="font-size:12.5px;margin-bottom:8px">${window._hsRows.length} rows found. Map the columns:</div><div class="row" style="gap:10px;flex-wrap:wrap">${sel('hsDate','Date')}${sel('hsHour','Hour / time')}${sel('hsSales','Net sales')}</div><div class="row" style="margin-top:14px"><button class="btn pri" style="width:auto" onclick="hsImport()">Import</button><span class="muted" id="hsMsg" style="font-size:14px;margin-left:10px"></span></div>`; document.getElementById('hsDate').value=guess(/date|day/); document.getElementById('hsHour').value=guess(/hour|time/); document.getElementById('hsSales').value=guess(/net|sales|amount|total|revenue/); }; rd.readAsText(f); };
...
async function vChecklists(v){
  ...
  if(tmpls.length>1) h+=`<div style="display:flex;gap:6px;overflow-x:auto;padding:2px 0 14px">`+tmpls.map(t=>{ const on=String(t.id)===clTab; const rr2=runs[String(t.id)]||{checks:{}}; const dn2=t.items.filter((_,i)=>rr2.checks[i]&&rr2.checks[i].done).length; return `<button onclick="state.ctx.clTab='${t.id}';vChecklists(document.getElementById('view'))" style="flex:none;border:1px solid ${on?'var(--brand)':'var(--line2)'};background:${on?'var(--brand)':'var(--card)'};color:${on?'#fff':'var(--ink)'};padding:8px 16px;border-radius:999px;font-weight:600;font-size:14px;cursor:pointer;white-space:nowrap">${esc(t.title)}${dn2?` <span style="opacity:.75">${dn2}/${t.items.length}</span>`:''}</button>`; }).join('')+`</div>`;
  tmpls.filter(t=>String(t.id)===clTab).forEach(t=>{ const run=runs[String(t.id)]||{checks:{}}; const checks=run.checks; const done=t.items.filter((_,i)=>checks[i]&&checks[i].done).length; const pct=t.items.length?Math.round(done/t.items.length*100):0;
    h+=`<div class="card" style="padding:0;margin-bottom:16px;overflow:hidden"><div style="padding:13px 16px;display:flex;align-items:center;gap:10px"><div style="font-weight:700;font-size:15.5px;flex:1">${esc(t.title)}</div><div class="faint" style="font-size:12.5px">${done}/${t.items.length}</div><button onclick="clPrint(${t.id})" class="faint no-print" style="border:none;background:none;cursor:pointer;font-size:15.5px" title="Print this section"><i class="ti ti-printer"></i></button>${isAdmin?`<button onclick="clEdit(${t.id})" class="faint" style="border:none;background:none;cursor:pointer;font-size:14px">Edit</button>`:''}</div><div style="height:6px;background:var(--line)"><i style="display:block;height:100%;width:${pct}%;background:${pct===100?'#1B7B3F':'var(--brand)'}"></i></div>`;
    t.items.forEach((it,i)=>{ const c=checks[i]||{}; h+=`<div style="display:flex;align-items:flex-start;gap:11px;padding:11px 16px;border-top:1px solid var(--line)"><input type="checkbox" ${c.done?'checked':''} onclick="clCheck(${t.id},${i},this.checked)" style="width:19px;height:19px;margin-top:1px;cursor:pointer"/><div style="flex:1;min-width:0"><div style="font-size:14px;${c.done?'color:var(--muted);text-decoration:line-through':''}">${esc(it)}</div>${c.done&&c.by?`<div class="faint" style="font-size:12.5px;margin-top:2px">✓ ${esc(c.by)}${c.at?' · '+timeAgo(c.at):''}</div>`:''}</div>${c.photo?`<img src="${c.photo}" onclick="clPhotoView(${t.id},${i})" style="width:38px;height:38px;object-fit:cover;border-radius:8px;cursor:pointer"/>`:`<button onclick="clPhotoPick(${t.id},${i})" title="Add photo proof" style="border:1px solid var(--line2);background:var(--card);border-radius:8px;width:38px;height:38px;cursor:pointer;color:var(--muted)"><i class="ti ti-camera"></i></button>`}</div>`; });
    h+=`</div>`;
  });
...
window.clPhotoView=function(tid,idx){ const c=(window._clRuns[String(tid)]||{checks:{}}).checks[idx]||{}; if(!c.photo)return; const m=document.createElement('div'); m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px'; m.onclick=()=>m.remove(); m.innerHTML=`<img src="${c.photo}" style="max-width:100%;max-height:100%;border-radius:12px"/>`; document.body.appendChild(m); };
...
window.clPrint=function(tid){
  ...
  const rows=t.items.map(it=>`<tr><td style="width:24px;height:32px;border:1.6px solid #111"></td><td style="padding:5px 12px;font-size:15.5px;border-bottom:1px solid #ddd">${esc(it)}</td></tr>`).join('');
  const win=window.open('','_blank','width=720,height=920'); if(!win){ alert('Allow pop-ups for this site to print the checklist.'); return; }
  win.document.write(`<html><head><title>${esc(t.title)} checklist</title><meta charset="utf-8"></head><body style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#111;margin:0;padding:34px 38px"><div style="border-bottom:3px solid #4A9CAD;padding-bottom:12px;margin-bottom:16px"><div style="font-size:15.5px;letter-spacing:2.5px;text-transform:none;color:var(--ink);font-weight:600;letter-spacing:-.012em">Sidewalk &middot; Checklist</div><div style="font-size:26px;font-weight:800;margin-top:2px">${esc(t.title)}</div></div><div style="margin-bottom:16px;font-size:14px">Date: <span style="display:inline-block;border-bottom:1px solid #111;width:150px">&nbsp;</span>&nbsp;&nbsp;&nbsp; Completed by: <span style="display:inline-block;border-bottom:1px solid #111;width:200px">&nbsp;</span></div><table style="width:100%;border-collapse:collapse">${rows}</table><div style="margin-top:26px;font-size:12.5px;color:#888;letter-spacing:.5px">SIDEWALK Coffee + Bagel &middot; Post-shift verification</div></body></html>`);
  win.document.close(); win.focus(); setTimeout(function(){ try{win.print();}catch(e){} }, 350);
};
window.clEdit=async function(tid){ const r=await sb.from('day_items').select('*').eq('id',tid).maybeSingle(); let d={}; try{d=JSON.parse((r.data&&r.data.detail)||'{}');}catch(e){} const items=(d.items||[]).join('\n'); const m=document.createElement('div'); m.id='clem'; m.style.cssText='position:fixed;inset:0;background:rgba(16,24,40,.45);z-index:9999;display:flex;align-items:flex-end;justify-content:center'; m.onclick=e=>{if(e.target===m)m.remove();}; m.innerHTML=`<div style="background:var(--card);width:100%;max-width:460px;border-radius:12px 16px 0 0;padding:20px 20px 26px"><div style="font-weight:700;font-size:15.5px;margin-bottom:4px">Edit “${esc((r.data||{}).title||'')}”</div><div class="faint" style="font-size:12.5px;margin-bottom:10px">One task per line.</div><textarea id="clItems" style="width:100%;min-height:220px;padding:11px;border:1px solid var(--line2);border-radius:8px;font-size:14px;font-family:inherit;color:var(--ink);background:var(--card)">${esc(items)}</textarea><div class="row" style="gap:8px;margin-top:14px"><button class="btn" style="width:auto;color:#A32D2D;border-color:#F0C9C9" onclick="clDelTmpl(${tid})">Delete list</button><button class="btn" style="width:auto;margin-left:auto" onclick="document.getElementById('clem').remove()">Cancel</button><button class="btn pri" style="width:auto" onclick="clSaveTmpl(${tid})">Save</button></div></div>`; document.body.appendChild(m); };
...
async function schWhoson(v){
  ...
  h+=`<div class="card" style="padding:14px 16px;margin-bottom:14px"><span style="font-size:26px;font-weight:800;color:var(--brand)">${onNow.length}</span> <span class="faint">on the clock right now</span>${onNow.length?` — ${onNow.map(s=>esc(s.person_name.split(' ')[0])).join(', ')}`:''}${next.length?`<div class="faint" style="font-size:12.5px;margin-top:6px">Next in: ${next.map(s=>esc(s.person_name.split(' ')[0])+' @'+fmtClock(s.start_time)).join(' · ')}</div>`:''}</div>`;
  if(!shifts.length) h+=`<div class="card" style="padding:22px;text-align:center"><div class="faint">No one scheduled today.</div></div>`;
    ...
    h+=`<div class="card" style="padding:14px 12px 16px;overflow-x:auto"><div style="min-width:520px;position:relative"><div style="position:relative;height:16px;margin-left:96px;border-bottom:1px solid var(--line)">`+marks.map(hh=>`<span style="position:absolute;left:${(hh-mn)/span*100}%;font-size:11.5px;color:var(--muted);transform:translateX(-50%)">${((hh%12)||12)}${hh<12?'a':'p'}</span>`).join('')+`</div>`;
    shifts.forEach(s=>{ const a=parseClock(s.start_time),b=parseClock(s.end_time); const L=Math.max(0,(a-mn)/span*100), W=Math.max(2,(b-a)/span*100); const isNow=nowH>=a&&nowH<b; const c=POSCOL(posOf(s.person_name)); h+=`<div style="display:flex;align-items:center;height:30px;margin-top:5px"><div style="width:92px;font-size:12.5px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:0 14px 0 2px">${esc(s.person_name.split(' ')[0])}</div><div style="position:relative;flex:1;height:22px;background:var(--bg);border-radius:5px"><div style="position:absolute;left:${L}%;width:${W}%;height:100%;border-radius:5px;background:${isNow?c:c+'59'};display:flex;align-items:center;padding:0 6px;font-size:11.5px;color:#fff;font-weight:600;white-space:nowrap;overflow:hidden">${fmtClock(s.start_time)}–${fmtClock(s.end_time)}</div></div></div>`; });
  ...
  else h+=`<div class="card">`+to.map(t=>`<div class="row" style="padding:10px 15px;border-bottom:1px solid var(--line);gap:10px"><i class="ti ti-plane" style="color:var(--brand)"></i><div style="flex:1"><div style="font-weight:600;font-size:14px">${esc(t.person_name)}</div><div class="faint" style="font-size:12.5px">${esc(fmtDay(_d(t.start_date)))}${t.end_date&&t.end_date!==t.start_date?' – '+esc(fmtDay(_d(t.end_date))):''}${t.reason?' · '+esc(t.reason):''}</div></div></div>`).join('')+`</div>`;
  ...
  else h+=`<div class="card">`+hols.map(x=>`<div class="row" style="padding:10px 15px;border-bottom:1px solid var(--line);gap:10px"><i class="ti ti-lock" style="color:#B32D2D"></i><div style="flex:1"><div style="font-weight:600;font-size:14px">${esc(x.name)}</div><div class="faint" style="font-size:12.5px">${esc(fmtDay(_d(x.date)))} · closed</div></div></div>`).join('')+`</div>`;
  v.innerHTML=h;
}
...
async function vSetup(v){
  ...
  let h=`<div class="card" style="padding:0;overflow:hidden;margin-bottom:18px"><div style="padding:15px 18px;border-bottom:1px solid var(--line)"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">Get set up</div><div style="height:6px;border-radius:5px;background:var(--line);overflow:hidden"><i style="display:block;height:100%;width:${Math.round(secDone/sections.length*100)}%;background:${secDone===sections.length?'#1B7B3F':'var(--brand)'}"></i></div><div class="faint" style="font-size:12.5px;margin-top:6px">${secDone} of ${sections.length} done · you can change anything later</div></div>`;
  sections.forEach((s,i)=>{ const next=i===firstOpenSec;
    h+=`<div style="display:flex;gap:12px;align-items:center;padding:13px 18px;border-top:1px solid var(--line)"><div style="width:26px;height:26px;border-radius:50%;flex:none;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:14px;${s.ok?'background:#1B7B3F;color:#fff':'background:var(--card);color:var(--brand);border:1.5px solid var(--brand-line)'}">${s.ok?'✓':(i+1)}</div><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:14px">${s.t}${next?` <span style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);background:var(--brand-soft);padding:1px 7px;border-radius:5px;margin-left:5px">Start here</span>`:''}</div><div class="faint" style="font-size:12.5px;margin-top:2px;line-height:1.45">${s.d}</div></div>${s.sub?'':`<button class="btn${next?' pri':''}" style="width:auto;flex:none;padding:6px 13px;font-size:14px" onclick="${s.go}">${s.cta}</button>`}</div>`;
    if(s.sub){ const firstOpenSub=sched.findIndex(x=>!x.ok);
      h+=`<div style="padding:2px 18px 8px 56px">`+sched.map((ss,j)=>{ const sn=j===firstOpenSub; return `<div style="display:flex;gap:10px;align-items:center;padding:8px 0;border-top:1px solid var(--line)"><div style="width:20px;height:20px;border-radius:50%;flex:none;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12.5px;${ss.ok?'background:#1B7B3F;color:#fff':'background:var(--card);color:var(--brand);border:1px solid var(--brand-line)'}">${ss.ok?'✓':(j+1)}</div><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">${ss.t}</div><div class="faint" style="font-size:12.5px">${ss.d}</div></div>${ss.inline==='labor'?`<div class="row" style="gap:5px;align-items:center;flex:none"><input id="setupGoal" type="number" min="0" max="100" step="0.5" value="${cov.targetPct||''}" placeholder="28" style="width:52px;text-align:center;padding:6px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit"/><span class="faint">%</span><button class="btn pri" style="width:auto;padding:6px 12px" onclick="setupSaveGoal()">Save</button></div>`:`<button class="btn${sn?' pri':''}" style="width:auto;flex:none;padding:5px 11px;font-size:12.5px" onclick="${ss.go}">${ss.cta}</button>`}</div>`; }).join('')+`</div>`;
    }
  });
  v.innerHTML=h;
}
...
async function teamCerts(v){
  ...
  if(!certs.length){ h+=`<div class="card" style="padding:28px;text-align:center"><div class="muted" style="font-size:14px">Nothing logged yet. Add a card above and you’ll get a heads-up before it expires.</div></div>`; }
    ...
    return `<div class="card" style="padding:12px 15px;margin-bottom:9px"><div class="row" style="gap:10px;align-items:center"><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:14px">${esc(c.person_name)}</div><div class="muted" style="font-size:14px">${esc(c.cert_type)}${c.note?' · '+esc(c.note):''}</div></div><span class="pill" style="background:${bg};color:${fg};font-size:12.5px;font-weight:800;padding:3px 10px;white-space:nowrap">${lbl}</span><button onclick="delCert('${c.id}')" title="Remove" style="border:none;background:none;color:var(--faint);cursor:pointer;font-size:15.5px">×</button></div></div>`; }).join(''); }
  v.innerHTML=h;
}
...
async function teamAreas(v){
  ...
  const av=n=>`<span style="width:30px;height:30px;border-radius:50%;flex:none;background:var(--brand-soft);color:var(--brand);display:inline-flex;align-items:center;justify-content:center;font-size:12.5px;font-weight:700">${esc((n||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase())}</span>`;
  ...
  const avSm=n=>`<span style="width:24px;height:24px;border-radius:50%;flex:none;background:var(--brand-soft);color:var(--brand);display:inline-flex;align-items:center;justify-content:center;font-size:11.5px;font-weight:700">${esc((n||'?').split(/\s+/).map(w=>w[0]||'').join('').slice(0,2).toUpperCase())}</span>`;
  ...
  if(mgmt && owners.length){ h+=`<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:6px">`+owners.map(o=>`<div class="card" style="padding:12px 15px;display:flex;align-items:center;gap:10px;border:1px solid var(--brand);background:var(--brand-soft)">${av(o)}<div><div style="font-weight:700;font-size:14px">${esc(o)}</div><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em">Owner</div></div></div>`).join('')+`</div><div style="width:2px;height:14px;background:var(--line2);margin:0 0 6px 24px"></div>`; }
  if(!areas.length){
    if(isAdmin){ h+=`<div class="card" style="padding:20px 22px"><div style="font-weight:700;font-size:15.5px;margin-bottom:5px">Set up who owns what</div><div class="faint" style="font-size:14px;line-height:1.55;margin-bottom:14px">Give each part of the business one point of contact. The starter set drops in five common ones, Culture, Facilities, Training, Baking and Service, that you can rename, reassign or remove. Or build your own from scratch.</div><button class="btn pri" style="width:auto" onclick="areaAddStarter()"><i class="ti ti-wand"></i> Use the starter set</button> <button class="btn" style="width:auto" onclick="areaAdd()"><i class="ti ti-plus"></i> Build my own</button></div>`; }
  ...
  h+=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px">`+areas.map(a=>{
    if(isAdmin && editing===a.id){
      return `<div class="card" style="padding:15px 16px;border:1px solid var(--brand)"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin-bottom:6px">Name</div><input id="aeN_${a.id}" value="${esc(a.name)}" style="width:100%;padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px;font-weight:600;box-sizing:border-box"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin:10px 0 6px">Description</div><textarea id="aeD_${a.id}" rows="2" placeholder="What this covers" style="width:100%;padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px;box-sizing:border-box;resize:vertical">${esc(a.desc)}</textarea><div class="row" style="gap:8px;margin-top:10px"><button class="btn pri" style="width:auto" onclick="areaEditSave(${a.id})">Save</button><button class="btn" style="width:auto" onclick="areaEditCancel()">Cancel</button></div></div>`;
    }
    const node=a.owner?`<div class="row" style="gap:8px;align-items:center">${av(a.owner)}<div style="min-width:0"><div style="font-weight:600;font-size:14px">${esc(a.owner)}</div><div class="faint" style="font-size:12.5px">${esc(posOf(a.owner)||'')}</div></div></div>`:`<div class="faint" style="font-size:12.5px">No point of contact yet</div>`;
    ...
    const helping=showHelping?`<div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin:12px 0 2px">Helping</div>${helpersHtml||(isAdmin?'<div class="faint" style="font-size:12.5px;margin-top:4px">No one yet</div>':'')}${addHelp}`:'';
    const ctrls=isAdmin?`<button onclick="areaEditStart(${a.id})" title="Edit name & description" style="border:none;background:none;color:var(--muted);cursor:pointer;font-size:15.5px"><i class="ti ti-pencil"></i></button><button onclick="areaDelete(${a.id})" title="Remove" style="border:none;background:none;color:var(--muted);cursor:pointer;font-size:15.5px"><i class="ti ti-trash"></i></button>`:'';
    return `<div class="card" style="padding:15px 16px"><div class="row" style="align-items:center;gap:9px"><i class="ti ${esc(a.icon)}" style="font-size:18px;color:var(--brand)"></i><div style="font-weight:700;font-size:15.5px;flex:1;min-width:0">${esc(a.name)}</div>${ctrls}</div>${a.desc?`<div class="faint" style="font-size:12.5px;line-height:1.45;margin-top:4px">${esc(a.desc)}</div>`:''}<div style="margin-top:11px;padding-top:11px;border-top:1px dashed var(--line2)"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin-bottom:5px">Point of contact</div>${node}${sel}${helping}</div></div>`;
  }).join('')+`</div>`;
