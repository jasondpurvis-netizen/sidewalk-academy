...
window.openProfile=function(name){
  ...
  const F=(id,lbl,type,val,ph)=>`<div style="flex:1;min-width:120px"><label style="font-size:12.5px;color:var(--muted)">${lbl}</label><input id="${id}" type="${type||'text'}" value="${val?esc(String(val)):''}" placeholder="${ph||''}" style="width:100%;margin-top:3px"/></div>`;
...
h+=`<div class="board"><div class="board-grid"><div class="bh bh-team">Team</div>`+days.map((d,di)=>`<div class="bh ${isoDate(d)===todayIso?'bh-today':''}">${d.toLocaleDateString(undefined,{weekday:'short'})}<span>${d.getDate()}</span>${dayVerdict[di].label?`<span title="${esc(dayVerdict[di].why)}" style="display:block;margin-top:4px;font-size:11.5px;font-weight:700;letter-spacing:.03em;text-transform:none;padding:2px 6px;border-radius:999px;background:${_VC[dayVerdict[di].level].bg};color:${_VC[dayVerdict[di].level].fg};white-space:nowrap">${esc(dayVerdict[di].label)}</span>`:''}${(isAdmin&&peakOf[di]&&peakOf[di].cap)?`<span title="Most people on at once, against what your coverage matrix allows" style="display:block;margin-top:3px;font-size:11.5px;font-weight:600;text-transform:none;letter-spacing:0;color:${peakOf[di].peak>peakOf[di].cap?'#A8401C':'var(--muted)'};white-space:nowrap">${peakOf[di].peak}/${peakOf[di].cap} at once${peakOf[di].people!==peakOf[di].peak?` &middot; ${peakOf[di].people} people`:''}</span>`:''}${(isAdmin&&lyForDay[di])?`<span title="Same day last year" style="display:block;margin-top:3px;font-size:11.5px;font-weight:600;text-transform:none;letter-spacing:0;color:var(--muted);white-space:nowrap">LY ${money(lyForDay[di].total)}${lyForDay[di].peakH!=null?` &middot; peak ${((lyForDay[di].peakH%12)||12)+(lyForDay[di].peakH<12?'a':'p')}`:''}</span>`:''}</div>`).join('');
  ...
  h+=`<div class="bt bt-team" style="font-weight:400"><span class="faint" title="Expected sales for the week. Auto-figured from last year, scaled to this year's trend. Tap a number to set your own — labor % uses this until real sales come in.">Forecast<div style="font-size:11.5px;opacity:.7;font-weight:400">tap to adjust</div></span></div>`+isoDays.map(iso=>{ const es=fcS[iso]||0; const over=fcAdj[iso]>0; const auto=fcAuto[iso]||0; return `<div class="bt"><div style="display:flex;align-items:center;justify-content:center;gap:1px"><span class="faint" style="font-size:11.5px">$</span><input type="number" min="0" value="${es?Math.round(es):''}" placeholder="—" onchange="setFcAdj('${iso}',this.value,${Math.round(auto)})" onclick="this.select()" style="width:50px;font-weight:600;text-align:center"/></div>${over?`<div style="font-size:11.5px;color:var(--brand);font-weight:700;cursor:pointer" onclick="resetFcAdj('${iso}')" title="Back to the forecast number">your # &middot; ↺ reset</div>`:(es?'<div class="faint" style="font-size:11.5px;opacity:.7">est.</div>':'')}</div>`; }).join('')+`</div></div>`;
  if(isAdmin && _wkFc){
...
window.quickAdd=function(cell){ if(!cell||cell.querySelector('.qadd'))return; cell._prevHTML=cell.innerHTML; cell.innerHTML=`<div class="qadd" onclick="event.stopPropagation()" style="display:flex;flex-direction:column;gap:3px;padding:1px"><input type="time" class="qaS" value="08:00" style=""/><input type="time" class="qaE" value="14:00" style=""/><div style="display:flex;gap:3px"><button onclick="quickAddSave(this)" style="flex:1;font-size:11.5px;padding:3px;border:none;border-radius:5px;background:var(--brand);color:#fff;cursor:pointer;font-weight:700">Add</button><button onclick="schQuickCancel(this)" style="font-size:11.5px;padding:3px 5px;border:1px solid var(--line2);border-radius:5px;background:var(--card);color:var(--muted);cursor:pointer">&times;</button></div></div>`; const f=cell.querySelector('.qaS'); if(f)f.focus(); };
...
async function schLogbook(v){
  ...
  h+=`<div class="card" style="padding:16px;margin-top:14px;margin-bottom:14px"><div style="font-size:15.5px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:7px">Close out your shift</div><div class="faint" style="font-size:12.5px;margin:0 0 12px">Pick your shift, log what happened, and flag anything the next shift needs.</div><div class="row" style="gap:8px;flex-wrap:wrap;margin-bottom:8px"><select id="lgseg" style=""><option value="Opening">Opening</option><option value="Mid">Mid</option><option value="Closing">Closing</option></select><select id="lgcat" style="">${cats.map(c=>`<option value="${c}">${c[0].toUpperCase()+c.slice(1)}</option>`).join('')}</select></div><textarea id="lgbody" placeholder="What got done, what broke, what the next shift needs to know…" style="width:100%;min-height:56px"></textarea><div class="row" style="margin-top:8px"><button class="btn" style="width:auto;margin-left:auto" onclick="addLog('${d0}')">Add note</button></div></div>`;
...
async function teamAreas(v){
  ...
  h+=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px">`+areas.map(a=>{
    if(isAdmin && editing===a.id){
      return `<div class="card" style="padding:15px 16px;border:1px solid var(--brand)"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin-bottom:6px">Name</div><input id="aeN_${a.id}" value="${esc(a.name)}" style="width:100%;font-weight:600;box-sizing:border-box"><div class="faint" style="font-size:15.5px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin:10px 0 6px">Description</div><textarea id="aeD_${a.id}" rows="2" placeholder="What this covers" style="width:100%;box-sizing:border-box;resize:vertical">${esc(a.desc)}</textarea><div class="row" style="gap:8px;margin-top:10px"><button class="btn pri" style="width:auto" onclick="areaEditSave(${a.id})">Save</button><button class="btn" style="width:auto" onclick="areaEditCancel()">Cancel</button></div></div>`;
    }
    ...
    const sel=isAdmin?`<select onchange="areaSetOwner(${a.id},this.value)" style="width:100%;margin-top:8px"><option value="">${a.owner?'Change point of contact…':'Assign a point of contact…'}</option>${roster.map(n=>`<option value="${esc(n)}"${a.owner===n?' selected':''}>${esc(n)} &middot; ${esc(posOf(n))}</option>`).join('')}</select>`:'';
    const hlp=a.helpers||[];
    ...
    const addHelp=isAdmin?`<select onchange="areaAddHelper(${a.id},this.value);this.value='';" style="width:100%;margin-top:8px"><option value="">+ Add someone who helps…</option>${roster.filter(n=>n!==a.owner&&!hlp.includes(n)).map(n=>`<option value="${esc(n)}">${esc(n)} &middot; ${esc(posOf(n))}</option>`).join('')}</select>`:'';
