...
window.readMyWeek=async function(){
  ...
  try{
    ...
    if(fillRecs.length){ html+='<div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin:16px 0 8px">Who can fill the open shifts</div>'; fillRecs.forEach(fr=>{ const wdN=_d(fr.o.on_date).toLocaleDateString(undefined,{weekday:'long'}); const tm=(fr.o.start_time?fmtClock(fr.o.start_time):'')+(fr.o.end_time?'–'+fmtClock(fr.o.end_time):''); html+='<div style="padding:6px 0;border-bottom:1px solid var(--line)"><div style="font-weight:600">'+esc(wdN)+' '+esc(tm)+(fr.o.role?' · '+esc(fr.o.role):'')+'</div><div class="faint" style="font-size:13px;margin-top:2px">'+(fr.who.length?('Free to take it: '+fr.who.slice(0,6).map(esc).join(', ')):'No one\'s free — everyone that day is already working, off, or unavailable.')+'</div></div>'; }); }
...
async function schReports(v){
  ...
  const card=(lbl,val,sub,col)=>`<div class="card" style="padding:14px 16px;flex:1;min-width:130px"><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin-bottom:4px">${lbl}</div><div style="font-size:24px;font-weight:800;line-height:1;color:${col||'var(--ink)'}">${val}</div>${sub?`<div class="faint" style="font-size:11.5px;margin-top:4px">${sub}</div>`:''}</div>`;
  ...
  if(isAdmin){
    ...
    h+=`<div class="sec">Sales vs last year</div><div class="card" style="padding:16px"><div class="row" style="gap:18px;flex-wrap:wrap;align-items:flex-end"><div><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em">This week</div><div style="font-size:24px;font-weight:800">${totSales?money(totSales):'—'}</div></div><div><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em">Same week last year</div><div style="font-size:20px;font-weight:700;color:var(--muted)">${lyTotal?money(lyTotal):'—'}</div></div>${lyDelta!=null?`<div style="font-size:17px;font-weight:800;color:${lyDelta>=0?'#1B7B3F':'#B32D2D'}">${lyDelta>=0?'▲':'▼'} ${Math.abs(lyDelta).toFixed(1)}% ${lyDelta>=0?'up':'down'}</div>`:`<div class="faint" style="font-size:12.5px">No sales recorded for this week last year yet.</div>`}</div></div>`;
...
async function vSales(v){
  ...
  const kpi=(lbl,val,d,note)=>{ const dc=d==null?'var(--muted)':(d>=0?'#1B7B3F':'#B32D2D'); const arrow=d==null?'':(d>=0?'▲':'▼'); return `<div class="card" style="padding:16px 18px;flex:1;min-width:150px"><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em">${lbl}</div><div style="font-size:26px;font-weight:800;margin:3px 0">${val==null?'<span style="color:var(--faint)">—</span>':money(val)}</div><div style="font-size:12.5px;color:${dc};font-weight:600">${note||(d==null?'no last-year data':arrow+' '+Math.abs(d).toFixed(1)+'% vs last year')}</div></div>`; };
  ...
  const pctCard=(lbl,val,sub,dc)=>`<div class="card" style="padding:16px 18px;flex:1;min-width:150px"><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em">${lbl}</div><div style="font-size:26px;font-weight:800;margin:3px 0">${val==null?'<span style="color:var(--faint)">—</span>':val.toFixed(1)+'%'}</div><div style="font-size:12.5px;color:${dc||'var(--muted)'};font-weight:600">${sub}</div></div>`;
...
async function vSetup(v){
  ...
  let h=`<div class="card" style="padding:0;overflow:hidden;margin-bottom:18px"><div style="padding:15px 18px;border-bottom:1px solid var(--line)"><div style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);margin-bottom:8px">Get set up</div><div style="height:6px;border-radius:4px;background:var(--line);overflow:hidden"><i style="display:block;height:100%;width:${Math.round(secDone/sections.length*100)}%;background:${secDone===sections.length?'#1B7B3F':'var(--brand)'}"></i></div><div class="faint" style="font-size:12px;margin-top:6px">${secDone} of ${sections.length} done · you can change anything later</div></div>`;
  sections.forEach((s,i)=>{ const next=i===firstOpenSec;
    h+=`<div style="display:flex;gap:12px;align-items:center;padding:13px 18px;border-top:1px solid var(--line)"><div style="width:26px;height:26px;border-radius:50%;flex:none;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:13px;${s.ok?'background:#1B7B3F;color:#fff':'background:var(--card);color:var(--brand);border:1.5px solid var(--brand-line)'}">${s.ok?'✓':(i+1)}</div><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:14.5px">${s.t}${next?` <span style="font-size:15px;font-weight:600;letter-spacing:-.012em;text-transform:none;color:var(--ink);background:var(--brand-soft);padding:1px 7px;border-radius:5px;margin-left:5px">Start here</span>`:''}</div><div class="faint" style="font-size:12.5px;margin-top:2px;line-height:1.45">${s.d}</div></div>${s.sub?'':`<button class="btn${next?' pri':''}" style="width:auto;flex:none;padding:6px 13px;font-size:13px" onclick="${s.go}">${s.cta}</button>`}</div>`;
...
async function teamAreas(v){
  ...
  if(mgmt && owners.length){ h+=`<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:6px">`+owners.map(o=>`<div class="card" style="padding:12px 15px;display:flex;align-items:center;gap:10px;border:1px solid var(--brand);background:var(--brand-soft)">${av(o)}<div><div style="font-weight:700;font-size:14px">${esc(o)}</div><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em">Owner</div></div></div>`).join('')+`</div><div style="width:2px;height:14px;background:var(--line2);margin:0 0 6px 24px"></div>`; }
  if(!areas.length){
  ...
  h+=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:12px">`+areas.map(a=>{
    if(isAdmin && editing===a.id){
      return `<div class="card" style="padding:15px 16px;border:1px solid var(--brand)"><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin-bottom:6px">Name</div><input id="aeN_${a.id}" value="${esc(a.name)}" style="width:100%;padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:14px;font-weight:600;box-sizing:border-box"><div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin:10px 0 6px">Description</div><textarea id="aeD_${a.id}" rows="2" placeholder="What this covers" style="width:100%;padding:8px 9px;border:1px solid var(--line2);border-radius:8px;background:var(--card);color:var(--ink);font-family:inherit;font-size:13px;box-sizing:border-box;resize:vertical">${esc(a.desc)}</textarea><div class="row" style="gap:8px;margin-top:10px"><button class="btn pri" style="width:auto" onclick="areaEditSave(${a.id})">Save</button><button class="btn" style="width:auto" onclick="areaEditCancel()">Cancel</button></div></div>`;
    }
    ...
    const helping=showHelping?`<div class="faint" style="font-size:15px;font-weight:600;text-transform:none;letter-spacing:-.012em;margin:12px 0 2px">Helping</div>${helpersHtml||(isAdmin?'<div class="faint" style="font-size:12px;margin-top:4px">No one yet</div>':'')}${addHelp}`:'';
